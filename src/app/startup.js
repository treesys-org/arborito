...
export function ensureHeavyShellLoaded() {
    if (!_heavyShellPromise) {
        _heavyShellPromise = Promise.all([
            ...
            import('../features/tour/api/product-tour-start-bridge.js'),
        ]).then(() => undefined);
    }
...
export function runStartup() {
    ...
    if (isFirstVisitOnboarding()) {
        ...
        onGdprNetworkConsentGranted(armEmojiRendering);
        setTimeout(armEmojiRendering, 45000);
    } else {
