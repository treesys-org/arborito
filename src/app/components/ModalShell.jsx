...
import { resolveMobileDockHubLayout } from '../../shared/ui/mobile-fullbleed-modals.js';
...
export function ModalShell({
    ...
    const backdropPosCls = (layout === 'dock-bottom' && mobile) ? 'fixed' : 'fixed inset-0';
    const dockBottomStyle = (layout === 'dock-bottom' && mobile)
        ? { top: 0, left: 0, right: 0, bottom: 'var(--arborito-chrome-dock-gap,4.25rem)', width: '100%', maxWidth: '100vw', height: 'auto', minHeight: 0, maxHeight: 'none', boxSizing: 'border-box' }
        : undefined;

    const immersiveFlag = layout === 'immersive' ? ' arborito-modal--immersive' : '';
    const hubDockRoot = /\barborito-modal--(?:arcade|forum|certificates-hub)\b/u.test(`${rootFlags} ${o.rootFlags || ''}`);
...
export function DockModalShell({
    ...
    const requestedLayout = layoutProp ?? shellOpts.layout ?? 'dock';
    ...
    if (hubChrome) {
        inner = (
            <>
                <DockHubShell mobile={mob} hero={hero} toolbar={toolbar} rootClass={rootClass} skipBodyWrap={skipBodyWrap}>
                    {children}
                </DockHubShell>
                {footer}
            </>
