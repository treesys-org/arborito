...
/** Modal backdrop and panel shell. */
export function ModalShell({
    ...
    const scrim = scrimProp || o.scrim || (layout === 'immersive' ? 'black' : 'translucent');
    /* Overlay from CSS `--arborito-modal-overlay`. Only `black` paints solid (gamesplay). */
    const scrimCls = scrim === 'none' ? ''
     ...
     * the forest header. `#modal-backdrop` has no baseline z-index in CSS, so this
     * is the only thing that stacks the modal above the shell. */
...
/** Modal with optional dock hub chrome (hero, toolbar, footer). */
...
/** Trusted HTML (calloutHtml, modalHeroHtml strings, …). */
