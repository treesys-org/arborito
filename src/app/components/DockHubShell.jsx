...
/**
 * Shared inner shell for dock hub modals (hero, optional toolbar, scroll body).
 * Optional `overlay` paints above hero + body (full panel; not clipped to the scroll body).
 */
export function DockHubShell({
    mobile: mobileProp,
    hero,
    toolbar,
    children,
    ...
    const mobileShellCls = compact
        ? 'arborito-dock-hub-shell arborito-dock-hub-shell--compact flex flex-col min-h-0 overflow-hidden'
        : 'arborito-dock-hub-shell flex flex-col flex-1 min-h-0 h-full overflow-hidden';
    ...
    if (mobile) {
        ...
        return (
            <div className={`${mobileShellCls}${extra} relative`}>
                {heroNode}
                {toolbar}
                {bodyWrap}
                {footer}
