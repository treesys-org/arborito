...
export function ModalHost() {
    ...
    }, [state.modal, state.viewMode, state.modalOverlay, route.kind, route.suspenseKey, shellStore]);
    ...
    }, [route.kind, route.suspenseKey, state.modalOverlay, shellStore]);
    ...
    const showAuthorLicense = state.modalOverlay?.type === 'author-license';

    return (
        <div ref={hostRef} data-arborito-panel="modals">
            {showContent ? <ModalRouteContent route={route} ui={state.ui} /> : null}
            {showAuthorLicense ? <ModalAuthorLicense key="author-license" /> : null}
        </div>
    );
}
