...
import { countCareDue, openArcadeCare } from '../utils/care-reminders.js';
import { computeCareStats } from '../utils/study-stats.js';
...
class ArboritoProgressWidget extends HTMLElement {
    ...
    buildCompactBodyHtml(ctx, layout = {}) {
        ...
        return `
            ...
            <section class="arborito-mochila-block arborito-mochila-block--care" aria-label="${escAttr(ui.arcadeTabCare || 'Cuidados')}">
                <div class="arborito-mochila-block__head">
                    <span class="arborito-mochila-block__title">🍂 ${escAttr(ui.arcadeTabCare || 'Cuidados')}</span>
                    ${ctx.dueCount > 0 ? `<span class="arborito-mochila-block__badge arborito-mochila-block__badge--due">${ctx.dueCount}</span>` : ''}
                </div>
                <p class="arborito-mochila-course__meta m-0 mb-2">
                    <span>${ctx.careStats.reviewedToday} ${escAttr(ui.careReviewedToday || 'regados hoy')}</span>
                    <span class="arborito-mochila-dot" aria-hidden="true">·</span>
                    <span>${ctx.careStats.inInterval} ${escAttr(ui.careInInterval || 'en intervalo')}</span>
                    <span class="arborito-mochila-dot" aria-hidden="true">·</span>
                    <span>${ctx.careStats.avgHealth}% ${escAttr(ui.careHealth || 'salud')}</span>
                </p>
                ${ctx.dueCount > 0 ? `<button type="button" class="arborito-mochila-btn arborito-mochila-btn--primary w-full js-mochila-care">${escAttr(ui.arcadeWaterBtn || 'Regar')} (${ctx.dueCount})</button>` : ''}
            </section>
    ...
    render() {
        ...
        const compactHtml = this.buildCompactBodyHtml(
            compactCtx,
            desktopMochilaTrim ? { omitActions: true } : {}
        );
