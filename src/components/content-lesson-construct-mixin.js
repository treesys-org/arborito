...
import { getToc, tocIdOrdinalBefore } from './content-toc.js';
...
} from '../utils/lesson-section-slices.js';
import { bodyMarkdownHasQuizV2Block, isQuizV2ChallengeComplete } from '../utils/quiz-v2-status.js';
...
import { ensureAndBindQuizV2Wizard, bindQuizV2WizardDelegation } from '../utils/quiz-v2-wizard.js';
...
export const contentLessonConstructMethods = {
    ...
    _readReviewMetaFromConstructInputs() {
        const chk = this.querySelector('#inp-lesson-spaced-review');
        const repaso = !!(chk && chk.checked);
        const node = this.currentNode;
        const parsed = node && node.content ? parseArboritoFile(node.content) : { meta: {} };
        const base = readSpacedReviewFromMeta(parsed.meta);
        const src = store.value.activeSource;
        return {
            repaso,
            id: repaso ? base.id || (node ? String(node.id) : '') : '',
            grupo: repaso ? base.grupo || (src && src.name ? String(src.name) : '') : '',
            tags: repaso ? base.tags || [] : []
        };
    },

    _bindLessonReviewMeta() {
        const sync = () => {
            if (!this.currentNode) return;
            const patch = this._readReviewMetaFromConstructInputs();
            this._reviewMetaDraft = { nodeId: this.currentNode.id, ...patch };
            this.lessonSpacedReviewValues = { repaso: patch.repaso };
            this._refreshLessonSaveButton();
        };
        const el = this.querySelector('#inp-lesson-spaced-review');
        if (el) {
