...
class ArboritoProductTour extends HTMLElement {
    constructor() {
        ...
        this._onState = () => {
            ...
            const sourcesPickerOnly = this._sourcesPickerOnlyTour && modalType === 'sources';
            ...
            if (store.value.modal || store.state.modalOverlay) {
                if (
                    (inSourcesContinuation || sourcesPickerOnly) &&
                    modalType === 'sources' &&
    ...
    _applyStepText() {
        ...
        if (mascotEl && step) {
            let m = this._mode === 'construction' ? '🦉⛑️' : '🦉';
            const t = step.target;
            if (t === 'graph' || t === 'mob-home' || t === 'home') m = '🗺️';
