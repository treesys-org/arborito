...
import { iconArboritoRootSvg } from '../sidebar-utils.js';
...
function openMobileRenameSheet(graph, node, initialName) {
    ...
    sheet.innerHTML = `<div class="arborito-mobile-rename-sheet__panel w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-2xl border-t border-slate-200 dark:border-slate-700 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl" role="dialog" aria-label="${escAttr(label)}">
        <p class="text-[10px] font-black uppercase tracking-wider text-slate-500 m-0 mb-2">${escHtml(label)}</p>
        <input type="text" class="arborito-mobile-rename-sheet__input w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 text-slate-900 dark:text-white text-base mb-3" value="${escAttr(initialName || '')}" />
        <div class="flex gap-2 justify-end">
            <button type="button" class="arborito-mobile-rename-sheet__cancel px-4 py-2.5 rounded-xl font-bold text-sm text-slate-600 dark:text-slate-300">${escHtml(ui.cancel || 'Cancelar')}</button>
            <button type="button" class="arborito-mobile-rename-sheet__save px-5 py-2.5 rounded-xl font-bold text-sm bg-indigo-600 text-white">${escHtml(ui.save || 'Guardar')}</button>
        </div>
    </div>`;
...
function startConstructionRename(graph, node, name) {
...
function constructionPanelTitleMarkup(graph, current, ui) {
    ...
    if (isConstruct && canWrite && current.type !== 'root') {
        ...
        return `<span class="mobile-panel-title-row flex items-center gap-1 min-w-0 flex-1">
            ...
            <button type="button" class="mobile-panel-rename-btn shrink-0 text-base leading-none p-1 rounded-lg hover:bg-slate-200/80 dark:hover:bg-slate-700/80" aria-label="${escAttr(renameLbl)}" title="${escAttr(renameLbl)}">✏️</button>
        </span>`;
    }
    const versionSlot =
        current.type === 'root' &&
...
function bindPanelRenamePencil(graph, headEl, node) {
...
export function applyMobileConstructionChromeOnly(root) {
    ...
    if (head) {
        ...
        head.innerHTML = `<div class="mobile-panel-header">${backBtnHtml}${headEmoji}${actionsHtml}${titleCell}</div>
            ${parentToolsHtml}
            ${moveHereHtml}`;
        ...
        bindPanelRenamePencil(this, head, tailCurrent);
...
export function renderMobilePrototypeTree(root) {
        ...
        pathNodes.forEach((node, index) => {
            ...
            if (index === 0 && node.type === 'root') {
                ...
                knot.innerHTML = iconArboritoRootSvg({ size: 68, className: 'mobile-knot__svg arborito-root-knot-mark' });
            } else {
                ...
                if (children.length === 0) {
                    if (current.hasUnloadedChildren) {
                        ...
                        if (isConstructEmpty && canWriteEmpty) {
                            ...
                            bindPanelRenamePencil(this, headElEmpty || panel, current);
                            ...
                            bindPanelRenamePencil(this, headElExplore || panel, current);
                    ...
                    bindPanelRenamePencil(this, headElMain || panel, current);
                    ...
                    children.forEach((child) => {
                        ...
                        const nameBlock = renamingRow
                            ...
                            : isConstruct && canWrite
                              ? `<div class="mobile-child-name-row flex items-center gap-1 min-w-0"><div class="mobile-child-name mobile-child-name-slot flex-1 min-w-0" title="${escAttr(cname)}">${nameLine}</div><button type="button" class="mobile-child-rename-btn shrink-0 text-sm leading-none p-0.5 rounded hover:bg-slate-200/60 dark:hover:bg-slate-700/60" aria-label="${escAttr(renameLbl)}" title="${escAttr(renameLbl)}">✏️</button></div>`
                        ...
                        this.bindMobileTap(row, async (e) => {
                            try {
                                ...
                                const fromRenameBtn =
                                    t && typeof t.closest === 'function' && t.closest('.mobile-child-rename-btn');
