...
export function buildVersionSwitchHTML() {
        ...
        const localBtn = `<button type="button" id="${VERSION_TOGGLE_ID}" class="arborito-timeline-local-pill shrink-0 flex items-center gap-2 px-2.5 py-1 rounded-full border-2 shadow-sm text-[10px] font-black w-full min-w-0" data-arborito-version-kind="local" aria-label="${escAttr(`${vp.chipLabel}: ${vp.chipSub}`)}" title="${escAttr(ui.releasesModalTitle || ui.menuVersion || 'Versions')}">
                <span class="text-sm" aria-hidden="true">${vp.icon}</span>
                <span class="min-w-0 flex flex-col items-start leading-tight flex-1">
                    <span class="text-[8px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">${escHtml(vp.chipLabel)}</span>
                    <span class="arborito-timeline-local-sub line-clamp-2">${escHtml(vp.chipSub)}</span>
                </span>
            </button>`;
        if (vp.isLocal) return localBtn;
        return `<div class="arborito-version-chip-host relative shrink-0 overflow-visible w-full min-w-0">
            <button type="button" id="${VERSION_TOGGLE_ID}" class="arborito-timeline-chip arborito-timeline-chip--btn flex items-start gap-2 rounded-2xl border-2 shadow-sm px-2.5 py-2 min-w-0 w-full" data-arborito-version-kind="${vp.versionKind}" aria-expanded="${this._versionMenuOpen}" aria-haspopup="listbox" aria-label="${escAttr(ariaLabel)}">
                ...
                <span class="min-w-0 flex-1 text-left space-y-0.5">
                    ...
                    <span class="block text-[10px] font-semibold leading-snug line-clamp-2 text-slate-700 dark:text-slate-200">${escHtml(
                        treeName
                    )}</span>
                    <span class="block text-[11px] font-bold leading-snug line-clamp-2">${escHtml(vp.chipSub)}</span>
                </span>
