...
import { addScrollSafeClickDelegation, bindMobileTap } from '../../utils/mobile-tap.js';
import { DOCK_SHEET_BODY_WRAP, modalWindowCloseXHtml, modalNavBackHtml } from '../../utils/dock-sheet-chrome.js';
import { getModuleStaticGameReadiness } from '../../utils/quiz-v2-status.js';
...
export const ArcadeUI = {
        ...
        const inner = `
                    ...
                    <div id="main-header" class="${mainHeaderClass}">
                        ...
                        <div class="flex-1 min-w-0">
                            <h2 class="arborito-mmenu-subtitle m-0 ${isMobile ? '' : 'leading-tight'}">${escHtml(ui.arcadeTitle)}</h2>
                            <p class="text-xs text-slate-600 dark:text-slate-400 mt-1 ${isMobile ? 'font-medium' : 'font-semibold'}">${escHtml(ui.arcadeDesc)}</p>
                        </div>
    ...
    renderGamesList(container, ui) {
        ...
        if (allGames.length === 0) {
            html += `<div class="p-8 text-center text-slate-400 italic">${escHtml(ui.noResults)}</div>`;
        } else {
            html += allGames.map((g, idx) => {
                ...
                const gIdAttr = escAttr(String(g.id != null ? g.id : ''));
                return `
                    ...
                    <div class="flex items-center gap-4 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h4 class="font-bold text-slate-800 dark:text-white truncate flex items-center gap-2">
                                ${gName}
                                ${g.isOfficial ? `<span class="text-[9px] bg-blue-100 text-blue-700 dark:bg-blue-900/45 dark:text-blue-300 px-1.5 py-0.5 rounded uppercase">${escHtml(ui.arcadeOfficialBadge)}</span>` : ''}
                            </h4>
                            <p class="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[200px] md:max-w-xs">${gSub}</p>
                            <p class="text-[9px] text-slate-400 mt-0.5 uppercase tracking-wide">${gRepo}</p>
                        </div>
                    </div>
        ...
        <div class="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <div class="flex gap-2">
                <input id="inp-custom-game" type="text" placeholder="${escAttr(ui.arcadePlaceholder || '')}" class="flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-sm text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-orange-500 placeholder:text-slate-400">
    ...
    renderGarden(container, ui) {
        ...
        const healthyIds = [];
        const now = Date.now();
        const dueOnly = this.gardenDueOnly !== false;
        ...
        let html = `
        <label class="flex items-center gap-2 mb-4 text-xs font-medium text-slate-600 dark:text-slate-400 cursor-pointer select-none">
            <input type="checkbox" id="arcade-garden-due-only" class="rounded border-slate-300"${dueOnly ? ' checked' : ''} />
            <span>${escHtml(uiSafe.arcadeGardenDueOnly || 'Solo cuidados pendientes')}</span>
        </label>`;
        ...
        if (dueIds.length === 0 && healthyIds.length === 0) {
            ...
            <div class="p-12 text-center flex flex-col items-center">
                ...
                <h3 class="text-lg font-black text-slate-700 dark:text-white mb-2">${escHtml(uiSafe.arcadeGardenEmptyTitle || 'Jardín vacío')}</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs">${escHtml(uiSafe.arcadeGardenEmptyDesc || '')}</p>
            </div>`;
        } else {
            // Withered Section
            if (dueIds.length > 0) {
                ...
                <div class="space-y-2 mb-6">
                    ${dueIds.map(id => {
                        ...
                        <div class="flex items-center justify-between p-3 bg-white dark:bg-slate-900 border border-red-200 dark:border-red-900/30 rounded-xl group hover:border-red-400 transition-colors">
                            ...
                            <button data-action="water-node" data-id="${escAttr(id)}" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg shadow-md hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
            ...
            if (healthyIds.length > 0) {
                html += `<h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">${escHtml(uiSafe.arcadeThrivingTitle || '')} (${healthyIds.length})</h4>`;
    ...
    renderStorage(container, ui) {
        ...
        <div class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 mb-6">
            <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-xs font-mono text-slate-500">${escHtml(stats.arcade.usedFmt)} / 3.5 MB</span>
            </div>
        ...
        <div class="flex justify-between items-center mb-3">
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest">${escHtml(ui.arcadeSavedGames)}</h3>
        ...
        <div class="space-y-2">
            ${stats.arcade.games.length === 0 ? `<div class="p-8 text-center text-slate-400 italic text-sm">${escHtml(ui.arcadeNoSavedGameData)}</div>` : 
    ...
    renderSetupContent(container, ui) {
        ...
        <div class="flex-1 flex flex-col min-h-0">
            ...
            <div class="mb-4">
                ...
                ${staticWarnHint ? `<p class="text-[10px] text-rose-700/90 dark:text-rose-300/90 mt-2 p-2 rounded-lg border border-rose-200/80 dark:border-rose-800/40 bg-rose-50/70 dark:bg-rose-950/25 leading-relaxed m-0">${escHtml(staticWarnHint)}</p>` : ''}
                ${isDynamic ? `<p class="text-[10px] text-amber-700/90 dark:text-amber-300/80 mt-2 p-2.5 rounded-lg border border-amber-200/80 dark:border-amber-800/40 bg-amber-50/70 dark:bg-amber-950/20 leading-relaxed m-0">${escHtml(ui.arcadeAiExperimentalDisclaimer || ui.sageExperimentalDisclaimer || '')}</p>` : ''}
            </div>

            <label class="block text-[10px] font-bold text-slate-400 uppercase mb-2">${escHtml(ui.arcadeSelectModule)}</label>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar bg-slate-50 dark:bg-slate-950/50 rounded-xl border border-slate-100 dark:border-slate-800 p-2 space-y-1">
                ${filteredNodes.map(n => {
                    ...
                    let typeBadge = `<span class="text-[9px] bg-slate-200 text-slate-600 px-1.5 rounded uppercase font-bold tracking-wider">${escHtml(ui.tagModule)}</span>`;
                    if (isLeaf) typeBadge = `<span class="text-[9px] bg-purple-100 text-purple-700 px-1.5 rounded uppercase font-bold tracking-wider">${escHtml(ui.tagLesson)}</span>`;
                    if (isExam) typeBadge = `<span class="text-[9px] bg-red-100 text-red-700 px-1.5 rounded uppercase font-bold tracking-wider">${escHtml(ui.tagExam)}</span>`;
                    ...
                    const gameBadge =
                        isStatic && nodeReadiness && nodeReadiness.staticReady
                            ? `<span class="text-[9px] bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200 px-1.5 rounded uppercase font-bold tracking-wider shrink-0" title="${escAttr(ui.arcadeNodeGameReadyTooltip || 'Questionnaire ready for static games')}">🎮</span>`
                            : '';
        ...
        <div class="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800 shrink-0">
            ...
            <p class="text-[10px] text-center text-slate-400 mt-2">${escHtml(ui.arcadeDisclaimer)}</p>
        </div>`;
    }
};
