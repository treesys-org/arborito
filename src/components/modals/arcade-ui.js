    ...
    renderGarden(container, ui) {
        ...
        if (dueIds.length === 0 && healthyIds.length === 0) {
            ...
            <div class="p-12 text-center flex flex-col items-center">
                ...
                <h3 class="text-lg font-black text-slate-700 dark:text-white mb-2">${escHtml(ui.arcadeGardenEmptyTitle)}</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs">${escHtml(ui.arcadeGardenEmptyDesc)}</p>
            </div>`;
        } else {
            // Withered Section
            if (dueIds.length > 0) {
                ...
                <div class="bg-red-50 dark:bg-red-900/10 p-4 rounded-xl border border-red-100 dark:border-red-900/30 mb-4 flex items-center gap-3">
                    ...
                    <p class="text-xs text-red-800 dark:text-red-300 font-medium">${escHtml(ui.arcadeWitheredMsg)}</p>
                </div>
                <div class="space-y-2 mb-6">
                    ${dueIds.map(id => {
                        ...
                        const daysText = escHtml(ui.arcadeWitheredDays.replace('{days}', String(daysOverdue)));
                        ...
                        <div class="flex items-center justify-between p-3 bg-white dark:bg-slate-900 border border-red-200 dark:border-red-900/30 rounded-xl group hover:border-red-400 transition-colors">
                            ...
                            <button data-action="water-node" data-id="${escAttr(id)}" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg shadow-md hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
                                <span>💧</span> ${escHtml(ui.arcadeWaterBtn)}
                            </button>
                        </div>`;
                    }).join('')}
                </div>`;
                ...
                <div class="bg-green-50 dark:bg-green-900/10 p-4 rounded-xl border border-green-100 dark:border-green-900/30 mb-6 flex items-center gap-3">
                    <span class="text-2xl">🌻</span>
                    <div>
                        <p class="text-sm font-black text-green-700 dark:text-green-300">${escHtml(ui.arcadeHealthyTitle)}</p>
                        <p class="text-xs text-green-600 dark:text-green-400">${escHtml(ui.arcadeHealthyMsg)}</p>
                    </div>
                </div>`;
            }
            ...
            if (healthyIds.length > 0) {
                html += `<h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">${escHtml(ui.arcadeThrivingTitle)} (${healthyIds.length})</h4>`;
                html += `<div class="space-y-2 opacity-80">
                    ${healthyIds.map(item => {
                        ...
                        const name = escHtml(nameRaw);
                        const icon = escHtml(node ? (node.icon || '📄') : '📄');
                        const rainText = escHtml(ui.arcadeNextRain.replace('{days}', String(daysLeft)));
