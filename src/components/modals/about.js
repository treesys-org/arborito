...
class ArboritoModalAbout extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (!this.embedded && modalState) {
            ...
            if (modalState.showImpressumDetails) this.showImpressumDetails = true;
        }
        ...
        if (this.activeTab === 'manifesto') {
            ...
            <div class="animate-in fade-in slide-in-from-bottom-2">
                ...
                <div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/40 p-4 text-center mb-6">
                    <button type="button" class="w-full py-3 rounded-xl border-2 border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-bold text-sm cursor-not-allowed opacity-75" disabled aria-describedby="about-arborito-donate-expl">${escHtml(ui.arboritoDonateCta || '')}</button>
                    <p id="about-arborito-donate-expl" class="text-[11px] text-slate-500 dark:text-slate-400 mt-2 mb-0 leading-snug">${escHtml(ui.arboritoDonateNotAccepting || '')}</p>
                </div>
            ...
            const impressumDetailsDecoded = escHtml(injectOperatorEmailToken(ui.impressumDetails || '', ui));
            return `
            <div class="animate-in fade-in slide-in-from-bottom-2">
                <div class="flex items-center gap-3 mb-6">
                    <span class="text-3xl">⚖️</span>
