...
class ArboritoModalArcade extends HTMLElement {
    constructor() {
        ...
        /** Garden tab: show only due care lessons when true. */
        this.gardenDueOnly = true;
    ...
    async connectedCallback() {
        ...
        if (modal && typeof modal === 'object' && modal.initialTab) {
            this.activeTab = modal.initialTab;
        }
        this.updateContent();
    ...
    updateContent() {
        ...
        if (this.selectedGame) {
            ...
            if (this.activeTab === 'garden') {
                ...
                const dueChk = content.querySelector('#arcade-garden-due-only');
                if (dueChk) {
                    dueChk.onchange = () => {
                        this.gardenDueOnly = dueChk.checked;
                        this.scheduleUpdateContent();
                    };
                }
            }
    ...
    launchGame() {
            ...
            if (readiness && !readiness.staticReady) {
                ...
                store.notify(
                    ui.arcadeStaticNoQuizWarn ||
                        'Este módulo no tiene lecciones con cuestionario completo (Quiz V2). Añade @core_concept, @main_question, @correct_answer y @trap en al menos una lección, o usa modo dinámico.'
                );
                return;
            }
        }
        ...
        store.setModal({
            ...
            aiMode: this.aiMode // 'static' or 'dynamic'
        });
    }
