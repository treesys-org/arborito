...
class ArboritoModalTreeInfo extends HTMLElement {
    ...
    renderShell() {
        ...
        const backdropCls = mobile
            ? `arborito-tree-info-full fixed inset-0 z-[140] flex flex-col p-0 m-0 bg-slate-950 h-[100dvh] min-h-[100dvh]${fade} duration-300${instantCls}`
            : `fixed inset-0 z-[140] flex items-center justify-center bg-slate-950 p-4${fade} duration-500 arborito-modal-root${instantCls}`;
        const panelCls = mobile
            ...
            : 'arborito-float-modal-card arborito-float-modal-card--narrow bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 cursor-auto max-h-[90dvh]';
