            ...
            <div id="publish-diff-backdrop" class="${backdropCls}">
                <div class="${panelCls}">
                    ...
                    <div class="p-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2 justify-end items-center">
                        ...
                        ${
                            !noBaseline
                                ? `<button type="button" id="btn-publish-diff-publish" class="px-4 py-2 rounded-xl text-sm font-black inline-flex items-center gap-2 ${noChanges ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-600 shadow-inner' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg active:scale-95'}" ${noChanges ? 'disabled' : ''}>
