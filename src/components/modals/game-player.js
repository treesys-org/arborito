...
import { parseLessonMetaTagsFromContent } from '../../utils/lesson-meta-tags.js';
...
class ArboritoModalGamePlayer extends HTMLElement {
    ...
    async prepareCurriculum() {
        ...
        if (this.aiMode === 'static') {
            let quizReady = 0;
            for (const node of this.playlist) {
                if (
                    !node.content &&
            ...
            if (quizReady === 0) {
                const ui = store.ui;
                throw new Error(
                    ui.gameStaticNoQuizError ||
                        'No playable questionnaires in this module. Add Quiz V2 tags (@core_concept, @main_question, @correct_answer, @trap) to at least one lesson, or switch to dynamic mode.'
                );
            }
    ...
    render() {
        ...
        const aiModeBadge = this.aiMode === 'dynamic'
            ? `<span class="ml-2 text-[9px] bg-purple-600 text-white px-1.5 py-0.5 rounded font-bold uppercase tracking-wider" title="${String(ui.arcadeAiModeDynamicDesc || '').replace(/"/g, '&quot;')}">AI</span>`
            : `<span class="ml-2 text-[9px] bg-orange-600 text-white px-1.5 py-0.5 rounded font-bold uppercase tracking-wider cursor-help" title="${String(staticPlayTip).replace(/"/g, '&quot;')}">Static</span>`;
        ...
        const mobilePlayHeader = `
            <header class="arborito-game-player-toolbar grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-x-1 sm:gap-x-2 px-2 sm:px-4 py-1 sm:py-3 pt-[max(0.2rem,env(safe-area-inset-top))] sm:pt-[max(0.5rem,env(safe-area-inset-top))] bg-slate-900 border-b border-slate-800 text-white shrink-0 shadow-md shadow-black/30">
                ...
                <h2 class="arborito-game-player-title justify-self-center text-center font-bold text-[0.7rem] sm:text-sm md:text-base px-1 sm:px-2 truncate min-w-0 max-w-[85vw] sm:max-w-md [text-shadow:0_1px_8px_rgb(0_0_0/_0.85)]">
                    <span class="arborito-game-player-emoji mr-0.5 sm:mr-2">🎮</span> ${title || ui.gameDefaultTitle}${aiModeBadge}
                </h2>
