...
class ArboritoModalSources extends HTMLElement {
    constructor() {
        ...
        this._overlayParentCount = 3;
        this._overlayChildCount = 3;
    ...
    _readPlantSkeletonFromOverlay(root) {
        const el = root || this;
        const parentsInp = el.querySelector('#inp-skeleton-parents');
        const childrenInp = el.querySelector('#inp-skeleton-children');
        const parentCount = parentsInp ? Number(parentsInp.value) : Number(this._overlayParentCount);
        const childrenPerParent = childrenInp ? Number(childrenInp.value) : Number(this._overlayChildCount);
        if (parentsInp) this._overlayParentCount = parentCount;
        if (childrenInp) this._overlayChildCount = childrenPerParent;
        if (!Number.isFinite(parentCount) || parentCount < 1 || !Number.isFinite(childrenPerParent) || childrenPerParent < 1) {
            return null;
        }
        return {
            parentCount: Math.min(50, Math.max(1, Math.round(parentCount))),
            childrenPerParent: Math.min(50, Math.max(1, Math.round(childrenPerParent)))
        };
    }
    ...
    _wirePlantOverlay(container) {
        ...
        if (inp) {
            ...
            inp.addEventListener('keydown', (ev) => {
                ...
                const skeleton = this._readPlantSkeletonFromOverlay(container);
                void plantNewTree(this, n, skeleton);
            });
        }
        const stopBubble = (ev) => {
            if (ev && typeof ev.stopPropagation === 'function') ev.stopPropagation();
        ...
        if (btnOk) {
            bindMobileTap(btnOk, async (ev) => {
                ...
                const n = (inp?.value || this._overlayPlantNameDraft || '').trim();
                const skeleton = this._readPlantSkeletonFromOverlay(container);
                await plantNewTree(this, n, skeleton);
            });
        }
        setTimeout(() => {
            if (inp) inp.focus();
    ...
    renderOverlay(ui) {
        ...
        if (this.overlay === 'plant') {
            const keep =
                ...
                container.querySelector('#inp-skeleton-parents');
            if (!keep) {
                const ph = escapeHtmlAttr(ui.treeNamePlaceholder || 'Name your tree...');
                const val = escapeHtmlAttr(this._overlayPlantNameDraft || '');
                const topicVal = escapeHtmlAttr(String(this._overlayParentCount ?? '3'));
                const lessonVal = escapeHtmlAttr(String(this._overlayChildCount ?? '3'));
                container.innerHTML = `
                <div class="w-full max-w-sm text-center px-2">
                    <h3 class="text-xl font-black mb-2 dark:text-white">${ui.plantTree}</h3>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mb-4 leading-relaxed">${escapeHtmlText(ui.skeletonBuilderDesc || ui.plantTreeDesc || '')}</p>
                    <input id="inp-new-tree-name" type="text" placeholder="${ph}" value="${val}" autocomplete="off" class="w-full bg-slate-100 dark:bg-slate-800 rounded-xl px-4 py-3 text-base font-bold mb-3 focus:ring-2 focus:ring-green-500 outline-none dark:text-white" autofocus>
                    <div class="grid grid-cols-2 gap-3 mb-4 text-left">
                        <label class="block">
                            <span class="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400">${escapeHtmlText(ui.skeletonParentCountLabel || 'Topics')}</span>
                            <input id="inp-skeleton-parents" type="number" min="1" max="50" value="${topicVal}" class="w-full mt-1 bg-slate-100 dark:bg-slate-800 rounded-xl px-3 py-2.5 text-sm font-bold focus:ring-2 focus:ring-green-500 outline-none dark:text-white">
                        </label>
                        <label class="block">
                            <span class="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400">${escapeHtmlText(ui.skeletonChildCountLabel || 'Lessons / topic')}</span>
                            <input id="inp-skeleton-children" type="number" min="1" max="50" value="${lessonVal}" class="w-full mt-1 bg-slate-100 dark:bg-slate-800 rounded-xl px-3 py-2.5 text-sm font-bold focus:ring-2 focus:ring-green-500 outline-none dark:text-white">
                        </label>
                    </div>
    ...
    async handleActionFromElement(btn) {
        ...
        if (action === 'confirm-plant') {
            ...
            if (inp) this._overlayPlantNameDraft = inp.value || this._overlayPlantNameDraft || '';
            const n = (inp?.value || this._overlayPlantNameDraft || '').trim();
            const skeleton = this._readPlantSkeletonFromOverlay(this);
            await plantNewTree(this, n, skeleton);
        }
