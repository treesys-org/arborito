...
export class ArboritoSageUICore extends HTMLElement {
    render() {
        ...
        const stateKey = JSON.stringify({
             ...
             mode: this.mode,
             stableHordeModels: this.stableHordeModels,
             ...
             stableHordeError: this.stableHordeConnectionError
        });

        if (stateKey === this.lastRenderKey) return;
        this.lastRenderKey = stateKey;

        // CHECK FOR GDPR CONSENT (model download in browser — wllama / GGUF)
        if (this.mode !== 'settings' && provider === 'browser' && !this.hasConsent) {
            // Prevent double-rendering of consent dialog
            if (this._consentRendered) return;
            this._consentRendered = true;
            this.renderConsent();
            return;
        }
        // Reset flag when consent is granted or provider changes
