...
import { getManualSections } from '../../utils/manual-sections.js';
import { buildManualTileNavHtml } from '../../utils/manual-tile-nav.js';
...
class ArboritoModalManual extends HTMLElement {
    ...
    updateSidebarUI() {
        const tileBtns = this.querySelectorAll('.manual-tile-btn');
        tileBtns.forEach((btn) => {
            ...
            btn.classList.toggle('ring-indigo-400/40', isActive);
    ...
    render() {
        ...
        const layoutKey = embedded
            ? this.hasAttribute('show-nav')
                ? 'embed-nav'
                : 'embed'
            : shouldShowMobileUI()
              ? 'mob'
        ...
        if (embedded) {
            const tileNav = this.hasAttribute('tile-nav');
            const withNav = this.hasAttribute('show-nav') && !tileNav;
            const embedNav = withNav
                ? `<nav class="arborito-manual-embed-nav shrink-0 flex flex-row md:flex-col gap-0.5 p-2 max-h-[30vh] md:max-h-none md:w-40 overflow-x-auto md:overflow-y-auto custom-scrollbar border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-950/50" aria-label="${titleText}">${sidebarHtml}</nav>`
                : '';
            const tileNavHtml = tileNav
                ? `<nav class="arborito-manual-tile-nav shrink-0 p-2.5 max-h-[38vh] md:max-h-none md:w-[11.5rem] overflow-y-auto custom-scrollbar border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700 bg-slate-50/90 dark:bg-slate-950/60" aria-label="${titleText}">${buildManualTileNavHtml(ui, this.activeSection)}</nav>`
                : '';
            const sideCol = tileNavHtml || embedNav;
            const embedBody = sideCol
                ? `<div class="arborito-manual-embed-root flex flex-col md:flex-row flex-1 min-h-0 w-full min-w-0 h-full overflow-hidden">${sideCol}<div id="manual-content" class="flex-1 bg-white dark:bg-slate-900 overflow-y-auto custom-scrollbar p-3 md:p-4 scroll-smooth min-h-0 min-w-0 overscroll-contain">${contentHtml}</div></div>`
                : `<div class="arborito-manual-embed-root flex flex-col flex-1 min-h-0 w-full min-w-0 h-full overflow-hidden"><div id="manual-content" class="flex-1 bg-white dark:bg-slate-900 overflow-y-auto custom-scrollbar p-4 scroll-smooth min-h-0 overscroll-contain">${contentHtml}</div></div>`;
            this.innerHTML = embedBody;
            if (tileNav) {
                this.querySelectorAll('.manual-tile-btn').forEach((btn) => {
