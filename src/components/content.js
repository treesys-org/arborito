...
import {
    ...
    lessonContentHasCompleteQuiz,
    parseAllChallengesFromLessonContent
} from '../utils/quiz-v2-status.js';
...
import { insertBlockInEditor } from '../utils/editor-commands.js';
import { readLessonMetaTags as readSpacedReviewFromMeta } from '../utils/lesson-meta-tags.js';
import { updateCareFromQuiz } from '../utils/care-schedule.js';
import {
    pickStudyQuizMode,
    pickNextQuizMode,
    normalizeChallenge,
    QUIZ_MODE_STEPS,
    QUIZ_MODE_RECALL
} from '../utils/quiz-v2-schema.js';
import {
    RECALL_ADVANCE_MS,
    validateClozeAnswers,
    validateChipsOrder,
    validateStepsOrder
} from '../utils/quiz-v2-player.js';
...
class ArboritoContent extends ArboritoComponent {
    constructor() {
        ...
        /** @type {{ key: string, quizIds: string[], currentIndex: number, awaitingAdvance?: boolean, finished?: boolean } | null} */
        this.quizSession = null;
        ...
        /** Header draft for spaced-review meta */
        this._reviewMetaDraft = null;
        this.lessonSpacedReviewValues = null;
        /** @type {boolean} */
    ...
    _evaluateQuizSession(session) {
        ...
        if (
            ...
            if (mem && !mem.isDue && mem.interval > 0) {
                const ui = store.ui || {};
                const tpl = ui.careNextInDays || 'Próximo cuidado en {days} días.';
    ...
    render() {
        ...
        const allBlocks = sanitizeStudentViewBlocks(
            enrichBlocksWithMetaQuiz(
                contentForParse,
                parseContent(parsedForBlocks.body || contentForParse)
            ),
            isExam
        );
        const toc = annotateTocWithQuizSections(allBlocks, getToc({ content: contentForParse }));
        const filteredToc = getFilteredToc(toc, this.tocFilter);

        const activeBlocks = getActiveBlocks(allBlocks, toc, this.activeSectionIndex);
        this._syncQuizSessionForSection(allBlocks, toc);

        if (!constructEdit && toc.length > 0) {
            this.visitedSections.add(this.activeSectionIndex);
        }
        ...
        if (constructEdit && this.currentNode) {
            ...
            const reviewBase = readSpacedReviewFromMeta(parsedHdr.meta);
            if (this._reviewMetaDraft && this._reviewMetaDraft.nodeId === this.currentNode.id) {
                this.lessonSpacedReviewValues = { ...reviewBase, ...this._reviewMetaDraft };
            } else {
                this.lessonSpacedReviewValues = reviewBase;
            }
        } else {
            this.lessonSpacedReviewValues = null;
        }
