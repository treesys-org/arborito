...
class ArboritoToastStack extends HTMLElement {
    connectedCallback() {
        this.style.cssText = 'position:fixed;inset:0;z-index:2147483000;pointer-events:none;';
    ...
    _paint() {
        ...
        this.innerHTML = `
            <div class="arborito-toast-stack fixed top-0 left-0 right-0 z-[9999] flex justify-center pt-[max(0.75rem,env(safe-area-inset-top))] px-3 pointer-events-none" aria-live="${live}" role="${isErr ? 'alert' : 'status'}">
