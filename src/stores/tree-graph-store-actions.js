...
export function openBranchVersionSwitcherAction(e, rootEl = null) {
    ...
    if (!store) return;
    store.syncTreeContextFromMobilePath?.();
    store.openCurriculumSwitcherVersions?.(rootEl);
}

export function openTreeLibraryFromPanelAction(e) {
    e?.preventDefault?.();
    e?.stopPropagation?.();
    const store = storeRef();
    if (!store) return;
    const fromConstructionTree =
        store.state.constructionMode && store.state.constructionEditFocus === 'tree';
    store.setModal?.({
        type: 'sources',
        focusTab: 'trees',
        ...(fromConstructionTree ? { constructionTreeLibrary: true } : {}),
    });
}
...
export function shouldShowMoveHereInPanelAction(currentFolder) {
    ...
    if (!pidStr || !store.state.constructionMode || !fileSystem.features.canWrite) return false;
...
export function runMoveHereInPanelAction(folderNode, e) {
    ...
    void store.moveNode?.(moving, String(folderNode.id)).then(() => {
...
export function startPanelTitleRenameAction(node) {
    const store = getArboritoStore();
    if (!store?.state.constructionMode || !fileSystem.features.canWrite || !node || node.type === 'root') return;
    store.setInlineRenameNodeId?.(node.id);
    selectMobileNodeAction(node.id);
    store.setGraphMoveMode?.(false);
    bumpGraphUiRevisionAction();
...
export function wireInlineRenameInputAction(node, inp) {
    ...
    const finish = async (commit) => {
        ...
        if (!commit) {
            ...
            await store.renameGraphNodeFromConstruction?.(node, inp.value);
            store.setInlineRenameNodeId?.(null);
        }
        bumpGraphUiRevisionAction();
        schedulePersistTreeUiState(store);
