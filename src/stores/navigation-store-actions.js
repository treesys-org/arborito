 ...
 * @param {object} node
 * @param {{ forceOverlay?: boolean }} [opts]
 */
export function openEditorAction(node, opts = {}) {
    ...
    if (isLesson) {
        ...
        if (opts.forceOverlay) {
            queueMicrotask(() => store.dispatchEvent(new CustomEvent('arborito-lesson-magic-open')));
        }
        return;
    }
