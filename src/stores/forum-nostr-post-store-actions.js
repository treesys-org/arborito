...
export async function publishSignedForumMessageV3Action(sourceId, treeRef, signed, threadForPlace) {
    ...
    try {
        ...
        try {
            const prevRef = await store.nostr.loadThreadPageRefV3({ ...treeRef, threadId: signed.threadId, pageKey });
    ...
    if ((store.webtorrent && store.webtorrent.available ? store.webtorrent.available() : false)) {
        ...
        store.nostr.putThreadPageRefV3({
            ...treeRef,
        ...
        try {
            const refs = await store.nostr.loadForumSearchRefsV3(treeRef);
    ...
    store.nostr.putForumSearchRefV3({
        ...treeRef,
        pageKey,
...
export function addForumThreadAction(sourceId, opts = {}) {
    ...
    if (treeRef && th && isNostrNetworkAvailable()) {
        try {
            store.nostr.addThreadV3({ ...treeRef, placeId: th.nodeId, thread: th });
        } catch (e) {
...
export async function addForumMessageAction(sourceId, payload) {
    ...
    try {
        const bans = await store.nostr.loadForumBansV3({ ownerPub: treeRef.pub, universeId: treeRef.universeId });
    ...
    * queue so they can answer in real time. */
    ...
    try {
        const signed = await store.nostr.signForumMessage({
            pair,
            message: {
                ...msg,
        ...
        if (holdForApproval) {
            store.nostr.putPendingForumMessageV3({
                ...treeRef,
                messageId: signed.id,
                record: signed
            });
