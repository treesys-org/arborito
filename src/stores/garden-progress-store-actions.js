...
export function recordRecentLessonAction(nodeId, index, visitedSet, contentRaw, quizPassed) {
    return shell()?.userStore?.settings?.recordRecentLesson?.(nodeId, index, visitedSet, contentRaw, quizPassed);
}
...
export async function syncNetworkProgressNowAction(persistencePayload) {
    ...
    try {
        ...
        const encrypted = await net.encryptForSelf({ pair, data: payload });
        const peers = Array.isArray(net.peers) ? net.peers : [];
        net.putUserProgressReplicated({
            ...treeRef,
            userPub: pair.pub,
...
export async function loadNetworkProgressIntoUserStoreAction(treeRef) {
    ...
    try {
        ...
        const net = await getConnectedNostr(store);
        if (!net) return false;
        const rec = await net.getUserProgress({ ...treeRef, userPub: pair.pub });
        if (!rec?.ct) return false;
        const data = await net.decryptForSelf({ pair, encrypted: rec.ct });
