 ...
 * @param {import('./shell-store.js').ShellStore} store
 * @returns {Promise<boolean>} true when language was applied; false if aborted (e.g. leave cancel)
 */
export async function setLanguageOnStore(store, lang, opts = {}) {
    ...
    if (normalizeAppLangCode(store.state.lang) === target && store.state.i18nData) return true;
    ...
    /* Ask before applying lang / reloading curriculum (incl. language modal). */
    if (!uiOnly) {
        try {
