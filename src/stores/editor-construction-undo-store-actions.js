...
function persistAfterConstructionHistory(store, next) {
    try {
        if (fileSystem.isLocalComposedTree()) {
            persistActiveComposedBranchFromRaw(store, next, { notifyOnFailure: true });
        ...
        console.warn('[Arborito] construction history persist failed', e);
        store.notify?.(
            store.ui?.constructionComposedPersistFailed ||
                'Could not save this history step on this device.',
            true
        );
    }
}
...
function captureComposedTreeMeta(store) {
    if (store?.state?.activeSource?.type !== 'composed-tree') return null;
    ...
    try {
        return {
            treeId,
            branchRefs: JSON.parse(JSON.stringify(entry.branchRefs || [])),
            branchSetHash: entry.branchSetHash ?? null,
        };
    } catch {
        return null;
    }
...
function restoreComposedTreeMeta(store, meta) {
    if (!store || !meta?.treeId) return;
    try {
        store.userStore?.updateTree?.(meta.treeId, {
            branchRefs: JSON.parse(JSON.stringify(meta.branchRefs || [])),
            branchSetHash: meta.branchSetHash,
        });
    } catch {
        /* ignore */
    }
...
/** Stash branchRefs *before* composed playlist mutations so undo can restore them. */
export function stashComposedUndoMetaBeforePlaylistEditAction() {
    const store = shell();
    if (!store) return;
    store._pendingComposedUndoMeta = captureComposedTreeMeta(store);
}
...
/**
 * Leave confirm before undo/redo. Drops undo frames created by a leave-Save
 * so Undo still reverts the prior structure edit, not the save itself.
 * @param {object} store
 * @returns {Promise<boolean>}
 */
async function confirmLeaveBeforeConstructionHistory(store) {
    store._flushPendingConstructionUndo?.();
    const stackLen = store._constructionUndoStack?.length || 0;
    ...
    store._flushPendingConstructionUndo?.();
    while ((store._constructionUndoStack?.length || 0) > stackLen) {
        store._constructionUndoStack.pop();
    }
...
export function _normalizeConstructionHistoryEntryAction(entry) {
    ...
    if (!store) return undefined;
            ...
            return { snap: entry, at: Date.now(), by: '', summary: '', composedMeta: null };
...
export function _makeConstructionHistoryEntryAction(snap, summary = '', composedMeta) {
    const store = shell();
    if (!store) return undefined;

            const meta =
                composedMeta !== undefined ? composedMeta : store._pendingComposedUndoMeta || null;
            store._pendingComposedUndoMeta = null;
            return {
                snap: store._cloneGraphSnap(snap),
                at: Date.now(),
                by: store._constructionHistoryAuthor(),
                summary: summary || '',
                composedMeta: meta ? store._cloneGraphSnap(meta) : null,
            };
...
export async function undoConstructionEditAction() {
    const store = shell();
    if (!store) return undefined;

            if (!(await confirmLeaveBeforeConstructionHistory(store))) return false;

            store._flushPendingConstructionUndo?.();
            const raw = store._constructionUndoStack?.pop();
            if (!raw) return false;
            const hist = store._normalizeConstructionHistoryEntry(raw);
            const snap = hist.snap;
            ...
            try {
                ...
                persistAfterConstructionHistory(store, next);
                if (hist.composedMeta) restoreComposedTreeMeta(store, hist.composedMeta);
                store.dispatchEvent(new CustomEvent('construction-undo-changed'));
                return true;
            } finally {
...
export async function redoConstructionEditAction() {
    const store = shell();
    if (!store) return undefined;
...
export const storeConstructionUndoMethods = {
    ...
    stashComposedUndoMetaBeforePlaylistEdit: stashComposedUndoMetaBeforePlaylistEditAction,
};
