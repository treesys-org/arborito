            ...
            const applyToNode = (n) => {
                ...
                if (n.id === nodeId) {
                    if (n.type === 'branch' || n.type === 'root') {
                        ...
                        if (folderMeta.icon) n.icon = folderMeta.icon;
                        if (folderMeta.description != null) n.description = folderMeta.description;
                        if (folderMeta.certifiable) n.isCertifiable = true;
                        else if ('certifiable' in folderMeta) n.isCertifiable = false;
                        if (metaFromEditor && typeof metaFromEditor === 'object' && 'isCertifiable' in metaFromEditor) {
                            n.isCertifiable = !!metaFromEditor.isCertifiable;
                        }
                    } else {
                        n.content = rawFileContent;
...
export function getActivePublicTreeRefAction() {
    ...
    if (String(u).startsWith('branch://')) {
        const id = String(u).slice('branch://'.length);
        const entry = store.userStore?.state?.branches?.find((t) => t.id === id);
        const pub = String(entry?.publishedNetworkUrl || '').trim();
        if (pub) return parseNostrTreeUrl(pub);
    }
