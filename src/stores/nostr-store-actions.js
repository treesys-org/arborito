...
export function getMyTreeNetworkRoleAction() {
    ...
    const sessionUser = normalizeUsername(store._authSession?.username || '');
    if (sessionUser) {
        const byName = store.state.treeCollaboratorRolesByUsername?.[sessionUser];
        if (byName === 'editor' || byName === 'proposer') return byName;
    }
    return null;
}
...
export async function inviteNostrCollaboratorAction({ inviteePub, inviteeUsername, role } = {}) {
    ...
    if (!pub && username) {
        ...
        try {
            ...
            pub = String(resolved.networkUserPub || resolved.accountSignerPub).trim();
