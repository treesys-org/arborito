...
import { resolveComposedRefIdFromMobilePath } from '../features/forest/api/compose-tree-graph.js';
...
import {
    confirmConstructionTreeLoadIfNeeded,
    setConstructionEditFocus,
    maybePromptConstructionEditTarget,
    enforceConstructionPathLock,
} from '../features/editor/api/construction-enter-flow.js';
...
export async function loadComposedTreeAction(treeIdOrSource, forceRefresh = true, opts = {}) {
    ...
    if (!store) return undefined;
            ...
            const prevComposedId = fileSystem.isLocalComposedTree()
                ? String(fileSystem.composedTreeId() || '').trim()
                : '';
            const hadBranchLock = !!String(store.state.constructionLockedBranchRefId || '').trim();
            const skipLoadConfirm = !!(opts && opts.skipConstructionLoadConfirm);

            if (store.state.constructionMode && !skipLoadConfirm) {
                try {
                    const proceed = await confirmConstructionTreeLoadIfNeeded(treeId);
            ...
            /* Creating/opening a composed tree while already constructing must re-ask
             * which branch to edit — otherwise the playlist root looks like explore. */
            if (ok && store.state.constructionMode) {
                const treeChanged = prevComposedId !== String(treeId).trim();
                if (treeChanged || !hadBranchLock) {
                    try {
                        setConstructionEditFocus(null);
                        await maybePromptConstructionEditTarget();
                    } catch (e) {
                        console.error('[Arborito] construction edit-pick after composed load', e);
                    }
                } else {
                    try {
                        enforceConstructionPathLock();
...
export async function editComposedTreeInteractiveAction(treeId) {
    ...
    if (!store) return undefined;
            ...
            const hash = computeBranchSetHashSync(branchRefs);
            const dup = findLocalTreeWithSameHash(store.userStore.state.trees, hash, treeId);
            if (dup) {
                store.notify(ui.sourcesDuplicateTreeLocal || 'You already have a tree with the same branches.', true);
                return false;
            }
            if (store.state.activeSource?.type === 'composed-tree' && store.state.activeSource.treeId === treeId) {
                store.stashComposedUndoMetaBeforePlaylistEdit?.();
            }
...
export async function addBranchToTreeInteractiveAction(branchId) {
    ...
    if (!store) return undefined;
            ...
            const addRefsToTree = async (treeId) => {
                ...
                const hash = computeBranchSetHashSync(branchRefs);
                const dupTree = findLocalTreeWithSameHash(store.userStore.state.trees, hash, treeId);
                if (dupTree) {
                    store.notify(ui.sourcesDuplicateTreeLocal || 'You already have a tree with the same branches.', true);
                    return false;
                }
...
export const storeForestMethods = {
