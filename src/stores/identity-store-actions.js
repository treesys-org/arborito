...
export function setNostrRelayUrlsAction(peers) {
    const store = shell();
    if (!store) return;
    const normalized = persistUserNostrRelays(peers);
    void (async () => {
        try {
            const net = await getConnectedNostr(store);
            if (net) {
                net.setPeers(normalized);
...
export async function restoreOrPublishUserPairEscrowAction(username) {
    ...
    try {
        const blobs =
            typeof net.listAccountUserPairEscrowBlobs === 'function'
                ? await net.listAccountUserPairEscrowBlobs(name)
                : [await net.loadAccountUserPairEscrowOnce(name)].filter(Boolean);
        const list = Array.isArray(blobs) ? blobs.filter(Boolean) : [];
        ...
        /* Escrow exists but decrypt failed: do NOT mint/publish a new pair —
         * that would fork progress namespaces across devices forever. */
        if (list.length) {
            console.warn(
                'User-pair escrow present but unreadable; keeping local pair without overwrite'
            );
            await publishNetworkUserPubIndexAction(store, name);
            return;
        }
        if (secret) {
            ...
            if (local?.pub && local?.priv) {
                try {
                    ...
                    console.warn('User-pair escrow republish failed', e2);
                }
            }
        }
        await publishNetworkUserPubIndexAction(store, name);
