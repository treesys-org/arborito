...
export async function ensureTreeForumThreadWeekLoadedAction(threadId, weekKey) {
    ...
    if (!net) return false;
    try {
        const pageRef = await net.loadThreadPageRefV3({ ...ref, threadId: tid, pageKey: wk });
        ...
        * another pubkey. Verification is STRICT: signature + field
        * binding + per-message proof-of-work bound to this tree/thread
        * are all required, so peers cannot inject unsigned or zero-cost
        * messages into a page. */
        const msgs = [];
        for (const m of fresh) {
            try {
                if (await net.verifyForumMessage({ message: m, treeRef: { pub: ref.pub, universeId: ref.universeId } })) msgs.push(m);
            } catch {
