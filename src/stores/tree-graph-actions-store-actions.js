...
import { runGraphNodeAction as runGraphNodeActionImpl } from '../features/tree-graph/api/logic/graph-node-actions.js';
...
export function runGraphNodeActionAction(node, act, rootEl = null) {
    const store = shell();
    if (!store) return undefined;
    return runGraphNodeActionImpl.call(createActionContext(store, rootEl), node, act);
}
...
export function openUnifiedCurriculumSwitcherAction(rootEl = null) {
    ...
    if (!store) return undefined;
    const g = graphUiOf(store);
    if (g.curriculumSwitcherTreesOnly && !hasOtherTreeSwitcherSource()) {
        const ui = store.ui || {};
        store.notify(ui.treeSwitcherOnlyOne || 'No other tree or branch. More in Trees.', true);
        return undefined;
    }
    const vpGate = getVersionPresentation(
        store.state.activeSource,
        store.state.availableReleases || [],
        store.ui
    );
    const showVersionTab = switcherShowsVersionTab(vpGate, store.state, g);
    const allowConstructionEmpty =
        store.state.constructionMode &&
        fileSystem.features.canWrite &&
        (vpGate.isLocal || vpGate.isComposed || fileSystem.isNostrTreeSource());
    if (
        !g.curriculumSwitcherTreesOnly &&
        !hasOtherTreeSwitcherSource() &&
        !showVersionTab &&
        !allowConstructionEmpty
    ) {
        const ui = store.ui || {};
        store.notify(ui.treeSwitcherOnlyOne || 'No other tree or branch. More in Trees.', true);
        return undefined;
