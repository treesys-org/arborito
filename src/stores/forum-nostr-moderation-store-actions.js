...
export async function getForumModerationModeForActiveTreeAction(treeRef) {
    ...
    let mode = 'free';
    try {
        mode = await store.nostr.loadForumModerationPolicyV3({ ...treeRef });
    } catch (e) {
...
export async function setForumModerationPolicyModeAction(mode) {
    ...
    try {
        await store.nostr.putForumModerationPolicyV3({
            ...treeRef,
            adminPair,
...
export async function listForumPendingSummariesForActiveTreeAction() {
    ...
    try {
        const ids = await store.nostr.listPendingForumMessageIdsV3({ ...treeRef });
        const out = [];
        for (const id of ids) {
            const rec = await store.nostr.loadPendingForumMessageV3({ ...treeRef, messageId: id });
...
export async function approveForumPendingMessageAction(sourceId, messageId) {
    ...
    if (!mid) return false;
    try {
        const signed = await store.nostr.loadPendingForumMessageV3({ ...treeRef, messageId: mid });
        ...
        store.nostr.clearPendingForumMessageV3({ ...treeRef, messageId: mid });
