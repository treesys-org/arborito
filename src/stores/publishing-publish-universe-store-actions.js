...
export async function publishActiveTreeToNostrUniverseAction({ universeId = null, reuseNostrTreeUrl = null } = {}) {
    const store = shell();
    ...
    await yieldToPaint();
    await store.nostr.publishBundle({ pair, universeId: id, bundle });
...
export async function publishComposedTreeToNostrAction({ treeId = null, universeId = null, reuseNostrTreeUrl = null } = {}) {
    const store = shell();
    if (!store) return undefined;
    const ui = store.ui;
    if (typeof store.isSignedIn === 'function' && !store.isSignedIn()) {
        await store.showDialog({
            ...
            'To publish a tree to the network, open Profile and sign in with your sync code first.',
            bodyHtml: false,
    ...
    await ensureConnectedNostr(store);
    const { buildArboritoTreeBundleObject } = await import('../features/trees/api/arborito-tree-bundle.js');
    const { computeBranchSetHash } = await import('../features/trees/api/branch-set-hash.js');
