...
export async function setForumBanForActiveTreeAction({ targetPub, banned }) {
    ...
    if (!tp) return false;
    try {
        await store.nostr.putForumBanV3({
        ...
        if (banned) {
            ...
            try {
                await store.nostr.deleteAccountByAdmin({
                    pub: treeRef.pub,
                    universeId: treeRef.universeId,
                    userPub: tp,
                    adminPair: actorPair
...
try {
    const rec = await store.nostr.signDeletion({ adminPair: pair, kind: 'delete_message', targetId: messageId });
    store.nostr.putDeletedMessage({ ...treeRef, messageId, record: rec });
} catch (e) {
...
export function moderateDeleteForumMessageAction(sourceId, messageId, meta = {}) {
    ...
    if (treeRef && ok && isNostrNetworkAvailable()) {
        try {
            ...
            if (adminPair) store.nostr.deleteMessage({ ...treeRef, messageId, adminPair });
        } catch (e) {
...
export function moderateDeleteForumThreadAction(sourceId, threadId, meta = {}) {
    ...
    if (treeRef && ok && isNostrNetworkAvailable()) {
        try {
            const adminPair = store.getNostrPublisherPair(treeRef.pub);
            if (adminPair) store.nostr.deleteThread({ ...treeRef, threadId, adminPair });
        } catch (e) {
