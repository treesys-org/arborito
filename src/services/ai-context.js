...
function buildNodeHeader(store, node, { maxChars = 600 } = {}) {
    if (!node) return '';
    const ui = (store && store.ui);
    const name = String(node.type === 'root' ? ((ui && ui.navHome) || 'Home') : (node.name || '')).trim();
    const parts = [];
...
export function buildContextPack({
    ...
    const { evidence } = extractEvidence((node && node.content) || '', userText, { maxChars: evidenceBudget, alwaysIncludeLead: true });

    let contextStr = '';
    if (includeGlobalHeader) {
        contextStr += buildGlobalContextHeader({ lang: ((store && store.state) ? store.state.lang : undefined), appName: ((store && store.ui) ? store.ui.appTitle : undefined) || 'Arborito' }) + '\n\n';
    }
