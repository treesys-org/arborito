...
export function buildQrSignalingPayload(sessionId, pubkey, relays = []) {
    ...
    url.searchParams.set('auth_sid', sessionId);
    url.searchParams.set('auth_pub', pubkey);
    if (Array.isArray(relays) && relays.length > 0) {
        url.searchParams.set('auth_rel', btoa(relays.slice(0, 5).join(',')));
    }
    url.searchParams.set('auth_ts', Date.now().toString());
    return url.toString();
}
...
export function parseQrSignalingPayload(text) {
    try {
        ...
        if (raw.startsWith('http://') || raw.startsWith('https://')) {
            ...
            if (url.searchParams.has('auth_sid') && url.searchParams.has('auth_pub')) {
                const sid = url.searchParams.get('auth_sid');
                const pub = url.searchParams.get('auth_pub');
                let relays = [];
                const relRaw = url.searchParams.get('auth_rel');
                if (relRaw) {
                    try { relays = atob(relRaw).split(',').filter(Boolean); } catch {}
                }
                let ts = Date.now();
                const tsRaw = url.searchParams.get('auth_ts');
                if (tsRaw) ts = Number(tsRaw);

                if (sid && pub && /^[0-9a-fA-F]{64}$/.test(pub)) {
                    return { sessionId: sid, pubkey: pub, relays, timestamp: ts };
                }
                return null;
            }
        }
