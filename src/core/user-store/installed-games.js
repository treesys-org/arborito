...
import { getDefaultArcadeGameCatalog } from './_helpers.js';
import { isOfficialArboritoGamesCatalogUrl } from '../../features/arcade/api/arcade-games-cdn.js';
...
export const installedGamesMixin = {
    ...
    ensureDefaultArcadeGameCatalog() {
        const def = getDefaultArcadeGameCatalog();
        let repos = this.state.gameRepos.filter((r) => {
            if (r.id === def.id) return true;
            if (isOfficialArboritoGamesCatalogUrl(r.url)) return false;
            return true;
        });
        ...
        if (idx === -1) {
            ...
            const row = repos[idx];
            const updated = { ...row, url: def.url, name: def.name, isOfficial: true };
            if (row.url !== updated.url || row.name !== updated.name || !row.isOfficial) {
                repos[idx] = updated;
                changed = true;
            }
            if (idx > 0) {
    ...
    saveGameData(gameId, key, value) {
        ...
        this.state.gameData[gameId]._sys_updated = Date.now();
        this.persist();
    },
    ...
    removeGameRepo(id) {
        if (id === getDefaultArcadeGameCatalog().id) return false;
