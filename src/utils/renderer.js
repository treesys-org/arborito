...
function renderExternalMediaPlaceholder(b, ui) {
    ...
    return `
        <div class="arborito-media-blocked my-10 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 p-6 text-center">
            <p class="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">${ui.mediaBlockedTitle || 'External media'}</p>
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">${ui.mediaBlockedHint || ''}</p>
            <span class="inline-block text-2xl mb-2" aria-hidden="true">${kind === 'video' ? '▶' : kind === 'audio' ? '🎵' : '🖼'}</span>
            <p class="text-xs text-slate-400 dark:text-slate-500 mb-4">${label}</p>
...
export const ContentRenderer = {
    renderBlock(b, ui, context) {
        ...
        if (b.type === 'code') return `
            <div class="my-6 rounded-2xl bg-[#1e1e1e] border border-slate-700 overflow-hidden shadow-xl text-sm group not-prose">
                <div class="flex items-center justify-between px-4 py-2 bg-[#2d2d2d] border-b border-black/20">
                    ...
                    <span class="text-xs text-slate-500 font-mono uppercase">${ui.codeTerminalLabel || 'Terminal'}</span>
        ...
        if (b.type === 'game') {
            const title = (b.label || ui.gameRecommendedTitle || ui.mobileArcadeCta || 'Arcade').trim();
            const prefix =
                (ui.gameCtaPrefix || ui.gameActivityLabel || 'Actividad interactiva').trim();
            ...
            const btnLabel = ui.gamePlayNow || ui.arcadeSetup || 'Play';
            const hint =
            ...
            const topicsLabel =
                topics.length > 0
                    ? `<div class="text-[11px] text-slate-500 dark:text-slate-400 font-mono break-all select-text">topics: <span class="opacity-80">${topics.join(', ')}</span></div>`
                    : '';
            return `
                ...
                <div class="relative z-10 flex flex-col gap-4">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center flex-wrap">
                                <h3 class="m-0 text-lg font-black text-slate-900 dark:text-white tracking-tight">${prefix}: ${title}</h3>
                                ${optPill}
                            </div>
                            <p class="m-0 mt-1 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">${hint}</p>
                        </div>
                    </div>
                    <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                        <button type="button" class="btn-game-launch px-5 py-3 rounded-xl font-black uppercase tracking-wider text-sm bg-slate-900 text-white hover:opacity-90 active:scale-[0.99] transition ${!url ? 'opacity-50 cursor-not-allowed' : ''}"
                            ...
                            data-topics="${topics.join(',').replace(/"/g, '&quot;')}"
                            ${disabled}>
                            ${btnLabel}
                        </button>
                        ${url ? `<div class="flex flex-col gap-1">
                            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-mono break-all select-text">cartridge: <span class="opacity-80">${url}</span></div>
                            ${topicsLabel}
                        </div>` : `<div class="text-[11px] text-red-600 dark:text-red-300 font-bold">${ui.gameMissingUrl || 'Missing game URL'}</div>`}
                    </div>
                </div>
            </div>`;
        }
