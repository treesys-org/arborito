 ...
 * @param {object} ui
 */
export function formatSageQuizStatus(quiz, ui) {
...
/**
 * @param {ReturnType<typeof getSageQuizContent>} quiz
 * @param {'full'|'question'|'answer'|'traps'} view
 * @param {object} ui
 */
export function formatSageQuizDisplay(quiz, view, ui) {
    if (!quiz || !quiz.hasQuiz) {
        return ui.sageQuizEmpty || 'No questionnaire filled in this lesson yet.';
    }
    const L = {
        concept: ui.sageQuizStudentConcept || ui.lessonQuizV2Label || ui.editorBlockCoreConcept || 'Core concept',
        def: ui.sageQuizStudentDefinition || ui.editorBlockCoreConcept || 'Definition',
        q: ui.sageQuizStudentQuestion || ui.editorBlockQuizQuestion || 'Question',
        ok: ui.sageQuizStudentCorrect || ui.editorBlockCorrect || 'Correct answer',
        trap: ui.sageQuizStudentDistractors || ui.lessonQuizV2AddTrap || 'Distractors'
    };
    if (view === 'question') {
        return quiz.question || ui.sageQuizNoQuestion || '(No question yet)';
    }
    if (view === 'answer') {
        return quiz.correct
            ? `${ui.sageQuizCorrectPrefix || 'Correct answer:'} ${quiz.correct}`
            : ui.sageQuizNoAnswer || '(No correct answer yet)';
    }
    if (view === 'traps') {
        if (!quiz.traps.length) return ui.sageQuizNoTraps || '(No distractors yet)';
        return quiz.traps.map((t, i) => `${i + 1}. ${t}`).join('\n');
    }
    const lines = [];
    if (quiz.coreConcept) lines.push(`${L.concept}: ${quiz.coreConcept}`);
    if (quiz.shortDef) lines.push(`${L.def}: ${quiz.shortDef}`);
    if (quiz.question) lines.push(`${L.q}: ${quiz.question}`);
    if (quiz.correct) lines.push(`${L.ok}: ${quiz.correct}`);
    if (quiz.traps.length) {
        lines.push(`${L.trap}:`);
        quiz.traps.forEach((t) => lines.push(`  • ${t}`));
    }
    return lines.join('\n') || ui.sageQuizEmpty || '';
}
...
export const SAGE_AI_MODE_KEY = 'arborito-sage-ai-mode';
