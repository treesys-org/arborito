 ...
 * @param {boolean} lightChrome — dock sheet (móvil / mismo look que about); false = dropdown oscuro escritorio
 */
export function getSearchResultsListHTML(state, ui, isBookmarks = false, lightChrome = false) {
    ...
    if (isSearching) {
        if (lightChrome) {
            return `
             <div class="text-center text-slate-500 dark:text-slate-400 py-8 flex flex-col items-center gap-2">
                 <div class="w-8 h-8 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin"></div>
                 <span class="text-sm font-bold text-emerald-600 dark:text-emerald-400/90 animate-pulse">${ui.editorProcessing || 'Processing...'}</span>
             </div>`;
        }
        return `
    ...
    if (results.length === 0) {
        if (isBookmarks) {
            const emptyCls = lightChrome
                ? 'text-center text-slate-600 dark:text-slate-400 py-12 flex flex-col items-center gap-2'
                : 'text-center text-slate-500 py-12 flex flex-col items-center gap-2';
        ...
        const noResCls = lightChrome
            ? 'text-center text-slate-500 dark:text-slate-400 py-8 flex flex-col items-center gap-2'
            : 'text-center text-slate-400 py-8 flex flex-col items-center gap-2';
        return `<div class="${noResCls}"><span class="text-2xl opacity-50">🍃</span><span>${ui.noResults}</span></div>`;
    }
    ...
    const header = isBookmarks
        ? lightChrome
            ? `<div class="px-4 py-2 bg-slate-100/90 dark:bg-slate-800/70 text-[10px] font-bold text-sky-700 dark:text-sky-300 uppercase tracking-widest border-b border-slate-200 dark:border-slate-700">Recent Lessons (In Progress)</div>`
            : `<div class="px-4 py-2 bg-slate-800/50 text-[10px] font-bold text-sky-400 uppercase tracking-widest border-b border-slate-700">Recent Lessons (In Progress)</div>`
        : '';
    ...
    const iconBox = lightChrome
        ? 'w-10 h-10 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-200 flex items-center justify-center text-xl shrink-0 group-hover:scale-105 transition-transform shadow-sm'
        : 'w-10 h-10 rounded-lg bg-[#0f172a] border border-slate-700 text-slate-300 flex items-center justify-center text-xl shrink-0 group-hover:scale-105 transition-transform shadow-md';
    const titleCls = lightChrome ? 'font-bold text-slate-800 dark:text-white truncate text-base' : 'font-bold text-slate-100 truncate text-base';
    ...
    const progressCls = lightChrome
        ? 'text-[10px] text-green-600 dark:text-green-400 font-mono ml-2'
        : 'text-[10px] text-green-400 font-mono ml-2';
        ...
        sortedResults
            .map((res) => {
                ...
                if (isBookmarks && res.progress !== undefined) {
                    progressIndicator = `<span class="${progressCls}">${res.progress}% Read</span>`;
                }
                const deleteAction = isBookmarks
                    ? `
                <button type="button" class="btn-delete-bookmark w-14 flex items-center justify-center border-l ${delBorder} text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-500/10 transition-colors text-lg" data-id="${res.id}" title="Remove Bookmark" aria-label="Remove Bookmark">🗑️</button>
            `
                    : '';
