...
import { buildQuizV2WizardHtml, readQuizV2Wizard } from './quiz-v2-wizard.js';
import { normalizeChallenge, serializeChallengeExtras, applyChallengeMetaLine, CHALLENGE_EXTRA_KEYS } from './quiz-v2-schema.js';
...
export const BLOCKS = {
    ...
    quizv2: (coreConcept = '', shortDef = '', question = '', correct = '', traps = []) => {
        let ch;
        ...
        return buildQuizV2WizardHtml(ch);
    },
    section: (title = "") => {
        const ui = store.ui;
        return `
        ...
        if (readingMeta) {
            ...
            if (trim.startsWith('@')) {
                ...
                if (idx > -1) {
                    ...
                    } else if (challengeFields.includes(key)) {
                        // Challenge schema fields
                        if (!meta.challenge) meta.challenge = { traps: [] };
                        if (CHALLENGE_EXTRA_KEYS.has(key)) {
                            applyChallengeMetaLine(meta.challenge, key, val);
                        } else if (key === 'trap') {
                            if (val) meta.challenge.traps.push(val);
                        } else {
                            meta.challenge[key] = val;
                        }
                    } else {
                        // Standard metadata
                        if (key === 'repaso') {
                            meta.repaso = ['1', 'true', 'yes', 'on', 'si', 'sí'].includes(val.toLowerCase());
                        } else if (key === 'id') {
                            meta.idTarjeta = val;
                        } else if (key === 'grupo') {
                            meta.grupo = val;
                        } else if (key === 'tags' || key === 'etiquetas') {
                            meta.tags = val
                                .split(',')
                                .map((t) => t.trim())
                                .filter(Boolean);
                        } else if (['title', 'icon', 'description', 'order', 'discussion'].includes(key)) {
                            meta[key] = val;
                        } else {
...
export function markdownToVisualHTML(md, opts = {}) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Quiz V2 (Challenge Schema for static game mode)
        if (line.startsWith('@core_concept:')) {
            let ch = normalizeChallenge({ core_concept: line.substring(14).trim() });
            while (i + 1 < lines.length) {
                const next = lines[i + 1].trim();
                if (!next.startsWith('@')) {
                    if (next === '') {
                ...
                const cidx = next.indexOf(':');
                if (cidx === -1) break;
                const key = next.substring(1, cidx).trim().toLowerCase();
                const val = next.substring(cidx + 1).trim();
                const known = [
                    'short_definition',
                    'main_question',
                    'correct_answer',
                ...
                if (!known.includes(key)) break;
                if (key === 'trap') ch.traps.push(val);
                else if (CHALLENGE_EXTRA_KEYS.has(key)) ch = applyChallengeMetaLine(ch, key, val);
                else ch[key] = val;
                i++;
            }
            html += BLOCKS.quizv2(ch);
            continue;
        }
...
export function visualHTMLToMarkdown(rootElement) {
    ...
    for (const node of rootElement.childNodes) {
        ...
        // Blockquotes (Simple)
        if (node.tagName === 'BLOCKQUOTE' && (!node.classList || !node.classList.contains('arborito-callout-edit'))) { 
            md += `> ${node.innerText}\n\n`; continue; 
        }
        ...
        // Quiz V2 (Game-ready quiz with Challenge Schema)
        if (node.classList && node.classList.contains('arborito-quizv2-edit')) {
            const ch = readQuizV2Wizard(node);
            if (ch.core_concept || ch.correct_answer) {
                if (ch.core_concept) md += `@core_concept: ${ch.core_concept}\n`;
                if (ch.short_definition) md += `@short_definition: ${ch.short_definition}\n`;
                if (ch.main_question) md += `@main_question: ${ch.main_question}\n`;
                if (ch.correct_answer) md += `@correct_answer: ${ch.correct_answer}\n`;
                ch.traps.forEach((t) => {
                    if (t) md += `@trap: ${t}\n`;
                });
                serializeChallengeExtras(ch).forEach((line) => {
                    md += `${line}\n`;
                });
                md += '\n';
            }
...
export function reconstructArboritoFile(meta, bodyMD) {
    ...
    if (meta.repaso) out += `@repaso: si\n`;
    if (meta.idTarjeta) out += `@id: ${meta.idTarjeta}\n`;
    if (meta.grupo) out += `@grupo: ${meta.grupo}\n`;
    if (meta.tags && meta.tags.length) {
        out += `@tags: ${meta.tags.join(', ')}\n`;
    }

    // Challenge Schema fields (for static game mode)
    if (meta.challenge) {
        const c = meta.challenge;
        if (c.core_concept) out += `@core_concept: ${c.core_concept}\n`;
        if (c.short_definition) out += `@short_definition: ${c.short_definition}\n`;
        if (c.main_question) out += `@main_question: ${c.main_question}\n`;
        if (c.correct_answer) out += `@correct_answer: ${c.correct_answer}\n`;
        if (c.traps && c.traps.length) {
            c.traps.forEach((trap) => {
                if (trap) out += `@trap: ${trap}\n`;
            });
        }
        serializeChallengeExtras(c).forEach((line) => {
            out += `${line}\n`;
        });
    }
