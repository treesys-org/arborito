...
export function getMediaConsentModalMarkup(ui, pending) {
    ...
    <div id="arborito-media-consent-root" class="fixed inset-0 z-[220] flex items-center justify-center p-4" role="presentation" data-pending-origins="${escapeHtmlAttr(originsJson)}">
        ...
        <div class="relative z-10 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 shadow-2xl p-6 text-left" role="dialog" aria-modal="true" aria-labelledby="arborito-media-consent-title">
            ...
            <div class="mb-4 max-h-48 overflow-y-auto custom-scrollbar">${rows}</div>
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">${ui.mediaConsentRememberHint || ''}</p>
            <label class="flex items-start gap-3 cursor-pointer mb-6 text-sm text-slate-700 dark:text-slate-200 select-none">
                <input type="checkbox" id="chk-media-consent-remember" class="mt-1 rounded border-slate-400 dark:border-slate-500" />
                <span>${ui.mediaConsentDontAskAgain || ''}</span>
            </label>
...
/** @deprecated */
export function persistMediaConsent(allow, remember) {
    if (!allow) return;
    if (remember) {
        try {
            localStorage.setItem(MEDIA_CONSENT_STORAGE_KEY_V1, JSON.stringify({ allow: true, remember: true }));
        ...
        try {
            sessionStorage.setItem(MEDIA_SESSION_KEY_V1, '1');
        } catch {
            /* ignore */
        }
    }
