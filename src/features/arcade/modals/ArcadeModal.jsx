...
export function ModalArcade({ embed, dockEmbed = false, dockEmbedActive = false }) {
    const {
        ui,
        dismissModal,
        setModal,
    ...
    const [showAddGameSheet, setShowAddGameSheet] = useState(false);
    const [arcadeRevision, setArcadeRevision] = useState(0);
    const isPreparingRef = useRef(false);
    const actionCtxRef = useRef(null);

    const bumpArcade = useCallback(() => setArcadeRevision((n) => n + 1), []);
    ...
    const handleArcadeAction = useCallback(async (action, fields = {}) => {
        ...
        if (!ctx) return;
        await runArcadeAction(ctx, action, fields);
    }, []);
    ...
    const dismissArcadeOrSheet = useCallback(() => {
        if (showAddGameSheet) {
            void handleArcadeAction('close-add-game-sheet');
            return;
        }
        handleClose();
    }, [showAddGameSheet, handleArcadeAction, handleClose]);

    useDockHubEmbedClose(dismissArcadeOrSheet, dockEmbedActive);
    ...
    useEffect(() => {
        const gameId = modal?.preSelectedGameId;
        if (!gameId || selectedGame || isLoading) return;
        const game = findGameById(gameId);
        if (!game) return;
        void prepareLaunch(game, modal?.preSelectedNodeId || null);
    }, [modal?.preSelectedGameId, modal?.preSelectedNodeId, selectedGame, isLoading, findGameById, prepareLaunch]);
    ...
    const launchGame = useCallback(() => {
        ...
        setModal({
            type: 'game-player',
            url: finalUrl,
            title: localizedArcadeGameName(ui, selectedGame),
            moduleId: selectedNodeId,
            aiMode,
            gameEntryUrl: selectedGame.path,
            gameVersion: selectedGame.version || '',
            offlineGameId: String(selectedGame.id),
            playOffline: userStore.isGameOffline(selectedGame.id),
            returnArcade: buildArcadeReturnContext({
                dockEmbed: dockEmbed || modal?.dockUi,
                activeTab,
            }),
        });
    }, [
    ...
    const tabs = (
        <div id="main-tabs" className="arborito-tab-strip" role="tablist" aria-label={ui.arcadeTitle || 'Arcade'}>
            <button
                type="button"
                role="tab"
    ...
    const hero = selectedGame ? (
        <ModalHubHero
            ...
            leadingIcon={selectedGame.icon || '🕹️'}
            tagClass="btn-close"
            onClose={dismissArcadeOrSheet}
        />
    ) : (
        <ModalHubHero
            ui={ui}
    ...
    /* skipBodyWrap: setup keeps sticky Start CTA (overflow hidden). Lists need their own scroll host. */
    const modalContentClass = selectedGame
        ? 'flex flex-col min-h-0 flex-1 overflow-hidden'
        : 'flex flex-col min-h-0 flex-1 overflow-y-auto custom-scrollbar';

    const addGameOverlay = showAddGameSheet ? (
        <ArcadeAddGameSheet
            ui={ui}
            onCancel={() => handleArcadeAction('close-add-game-sheet')}
            onSubmit={(url) => handleArcadeAction('add-game', { url })}
        />
    ) : null;
    ...
    if (dockEmbed) {
        return (
            <DockHubPanelEmbed
                panelId="modal-arcade"
                hero={hero}
                toolbar={selectedGame ? undefined : tabs}
                overlay={addGameOverlay}
            >
                <div id="modal-content" className={modalContentClass}>
                    {content}
                </div>
            </DockHubPanelEmbed>
        );
    ...
    return (
        <div data-arborito-panel="modal-arcade" data-embed={embed ? '1' : undefined}>
            <DockModalShell
                ...
                onBackdropClick={dismissArcadeOrSheet}
            >
                <div id="modal-content" className={modalContentClass}>
                    {content}
                </div>
