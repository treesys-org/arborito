        ...
        <>
                    ...
                    <>
                        ...
                        <div className="space-y-2 mb-4">
                            {dueIds.map((id) => {
                                ...
                                const nameRaw = node
                                    ? node.name
                                    : (ui.arcadeMemoryUnknownModule || 'Lesson {shortId}').replace(
                                          '{shortId}',
                                          `${id.substring(0, 8)}…`
                                      );
                                const icon = node ? node.icon || '📄' : '📄';
                                const daysText = (ui.arcadeWitheredDays || '{days} days').replace(
                                    '{days}',
                                    String(daysOverdue)
                                );
                    ...
                    <>
                        ...
                        <div className="space-y-2">
                            {healthyIds.map((item) => {
                                ...
                                const icon = node ? node.icon || '📄' : '📄';
                                const rainText = (ui.arcadeNextRain || '{days} days').replace(
                                    '{days}',
                                    String(daysLeft)
                                );
            ...
            <details className="garden-howto mb-4">
                <summary className="garden-howto__head">
                    {ui.gardenHowtoTitle || 'How does the garden work?'}
                </summary>
                <ul className="garden-howto__list">
                    <li>{ui.gardenHowtoXp || 'Each lesson or review adds lumens (☀️).'}</li>
                    <li>
                        {ui.gardenHowtoSeeds ||
                            'Each module you touch is planted as a seed in your plot.'}
                    </li>
                    <li>
                        {ui.gardenHowtoCare ||
                            'If time passes without review, the seed withers 🍂. Reviewing heals it.'}
                    </li>
                    <li>
                        {ui.gardenHowtoShop ||
                            'Spend lumens on decorations (butterflies, mushrooms, lanterns) that appear behind the tree.'}
                    </li>
                </ul>
            </details>
            ...
            <div className="mb-4">
                <h4 className="arborito-eyebrow arborito-eyebrow--md mb-2">
                    {ui.gardenPlotTitle || ui.gardenTitle || 'My plot'}
                </h4>
                <GardenPlot ui={ui} />
            </div>
