...
function darkShellProps(panelClass, tone, size = 'compact auto-h') {
    ...
    return {
        layout: mob ? 'centered' : 'immersive',
        shellOpts: {
            scrim: 'black',
            enter: 'fade',
            z: 80,
            panelSize: mob ? size : undefined,
            panelTone: tone,
            panelClass: `${panelClass} relative`.trim(),
            rootFlags: mob
                ? 'arborito-modal--immersive arborito-modal-immersive--center arborito-game-immersive-scrim'
                : 'arborito-modal--immersive arborito-game-player--desktop-widescreen arborito-game-immersive-scrim',
            bareBackdrop: !mob,
        },
    };
}

/** Shared immersive centered shell for game-player blocking screens (§4 MODAL_STANDARDS). */
function GamePlayerImmersiveScreen({
    ui,
    title,
    tone = 'default',
    panelClass = '',
    ...
    const mob = shouldShowMobileUI();
    const shellTone = tone === 'danger' ? 'danger-dark' : 'dark';
    const headbar = mob ? (
        <MobToolbar
            ui={ui}
            title={title}
    ...
    return (
        <ModalCenteredShell
            {...darkShellProps(panelClass, shellTone, panelSize)}
...
export function GamePlayerAiDownloadScreen({ ui, progressRaw, pct, onClose }) {
    ...
    return (
        <GamePlayerImmersiveScreen
            ui={ui}
            title={ui.sageDownloading || 'Loading…'}
...
export function GamePlayerConsentScreen({ ui, onGrant, onClose }) {
    ...
    const consentNotice = fillSageAiConsentTokens(ui.gameConsentAiNotice || '', isDesktop);
    const licenseText = ui.sageAiThirdPartyLicenses
        ? fillSageAiConsentTokens(ui.sageAiThirdPartyLicenses, isDesktop)
        : '';

    const bodyPadding = mob
        ? 'px-3 sm:px-4 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]'
        : 'p-6 pt-2';
    const footerPadding = mob
        ? 'px-3 sm:px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]'
        : 'px-6 pt-3 pb-6';

    const body = (
        <div className={`flex flex-col min-h-0 flex-1 overflow-hidden arborito-dialog-sheet-body ${bodyPadding}`}>
            <div
                className={`arborito-dialog-body-block arborito-callout-actions shrink-0 mb-3 ${mob ? 'items-start text-left w-full' : 'items-center text-center w-full max-w-md mx-auto'}`}
            >
                <Callout tone="blue" size="sm" layout="stack" extraClass="w-full">
                    <p className="text-xs leading-relaxed font-medium m-0">{consentNotice}</p>
                </Callout>
                {licenseText ? (
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-snug w-full m-0">
                        {licenseText}
                    </p>
                ) : null}
                <Callout tone="amber" size="sm" icon="⚠️" layout="stack" extraClass="w-full">
    ...
    const footer = (
        <div className={`arborito-modal-footer shrink-0 ${footerPadding}`}>
            <div className={`arborito-action-row w-full ${mob ? 'arborito-action-row--stack-mobile' : ''}`}>
    ...
    const backdropClick = (e) => {
        ...
        if (isModalBackdropEmptyTap(bd, e)) onClose();
    };

    const shellOpts = {
        z: 80,
        enter: mob ? 'dock' : 'fade-fast',
        scrim: 'translucent',
        rootFlags: mob ? 'arborito-modal--immersive arborito-game-immersive-scrim' : '',
    };
...
export function GamePlayerPlayHeader({ ui, title, aiMode, staticQuizLessonCount, onClose }) {
    ...
    const aiModeBadge =
        aiMode === 'dynamic' ? (
            ...
            >
                {ui.arcadeAiModeDynamic || 'Dynamic'}
            </span>
        ) : (
            <span
