...
function afterGameFrameMounted() {
...
export function useGamePlayerModal(embed) {
    ...
    const close = useCallback(() => {
        if (sessionXPRef.current > 0) {
            notify(`+${sessionXPRef.current} ${ui.xpUnit} — ${ui.gameSessionComplete}`);
        }
        setModal('arcade');
    }, [ui]);
    ...
    const setupBridge = useCallback(() => {
        ...
        window.__ARBORITO_GAME_BRIDGE__ = {
            ...
            aiChat: async (promptMessages, lessonCtx) => {
                ...
                const prevMode = (ai && ai.contextMode) || 'sage-tree';
                update({ ai: { ...ai, contextMode: 'game' } });
                try {
                    return await sageAi.chat(promptMessages, ctxNode);
                } finally {
                    update({ ai: { ...ai, contextMode: prevMode } });
                }
            },
            save: (key, value) => {
                try {
    ...
    const prepareCurriculum = useCallback(async () => {
        ...
        if (modalState?.careDueOnly) {
            ...
            if (filtered.length === 0) {
                throw new Error(
                    ui.carePlaylistEmpty || 'No hay cuidados pendientes en este módulo.'
                );
            }
            playlist.splice(0, playlist.length, ...filtered);
            cursorIndexRef.current = 0;
    ...
    const runPlaySession = useCallback(async () => {
        ...
        setupBridge();
        setIsPreparing(false);
        await afterGameFrameMounted();
        await loadGame(bundlePromise);
    }, [startBundlePrefetch, prepareCurriculum, setupBridge, loadGame]);
    ...
    const initializeSession = useCallback(async () => {
        ...
        if (webUnavailable) {
            setNeedsConsent(false);
            setAiError(ui.gameAiWebUnavailable || ui.sageWebAiUnavailableShort);
            setIsPreparing(false);
            return;
        }
