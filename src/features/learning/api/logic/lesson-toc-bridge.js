...
export function attachLessonTocBridgeMethods(bridge, { patchPanel, scheduleUpdate }) {
    ...
    bridge._lessonTocMoveAt = (idx, action) => {
        ...
        const next = moveTocSectionByAction(body, idx, action, tocBefore);
        if (next === body) return;

        const tocAfter = getToc({ content: next });
        const movedId = tocBefore[idx]?.id;
        let newActive = bridge.activeSectionIndex;
        if (movedId) {
