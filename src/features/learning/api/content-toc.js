...
function isQuestionIdSatisfied(qid, getQuizState, requireCorrect) {
    const st = getQuizState(qid);
    if (!st || !st.finished) return false;
    if (!requireCorrect) return true;
...
export function isSectionQuizSatisfied(sectionIndex, blocks, toc, getQuizState, opts = {}) {
    ...
    return questionIds.every((qid) => isQuestionIdSatisfied(qid, getQuizState, requireCorrect));
}
...
export function isTocSectionProgressDone(
    ...
    if (questionIds.length > 0) return quizSatisfied;
