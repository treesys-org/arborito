...
/** All questions in a block answered correctly. */
export function isQuizBlockPassed(block, quizStates) {
    const ids = expandQuizBlock(block).map((q) => q.id || 'quiz');
    if (!ids.length) return true;
    return ids.every((id) => {
        const st = getQuizState(quizStates, id);
        return !!(st.finished && st.correct);
    });
}
...
function mergeQuizPassRecord(prev, quizStates, ids) {
    const next = { ...(prev || {}) };
    for (const id of ids || []) {
        const st = getQuizState(quizStates, id);
        if (st.finished && st.correct) next[id] = true;
    }
    return next;
}
...
export function buildStartQuizState(id, block, prev) {
    ...
    const attempt = isRetry ? (prev.attemptCount || 0) + 1 : 0;
    const mode = ch
        ? isRetry && prev.v2Mode
    ...
    const stepsOrder =
        mode === QUIZ_MODE_STEPS && ch && ch.steps.length >= 2
            ? shuffleStepsForQuiz(ch.steps, `${id}:${attempt}`)
            : null;
    return {
        started: true,
        finished: false,
...
export function evaluateQuizSession(ctx, session) {
    ...
    if (isExam) {
        ...
        for (const qid of ids) {
            const st = getQuizState(ctx.quizStates, qid);
    ...
    if (ctx.currentNode) {
        const quizPassRecord = isExam
            ? ctx.quizPassRecord || {}
            : mergeQuizPassRecord(ctx.quizPassRecord, ctx.quizStates, ids);
...
export function buildAnswerQuizPatch(ctx, id, isCorrect) {
    ...
    if (blockKey) {
        ...
        if (ctx.currentNode && remembered && !isExam) {
            persistLessonReadingPosition(store, {
                nodeId: ctx.currentNode.id,
                index: ctx.activeSectionIndex,
