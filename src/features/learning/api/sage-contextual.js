...
import { isQuizChallengeComplete, normalizeChallenge, findQuizBlocks } from './quiz-schema.js';
...
export function getSageQuizContent(node) {
    ...
    if (!node || node.type !== 'leaf' && node.type !== 'exam' || !node.content) return empty;

    const { body } = parseArboritoFile(node.content);
    const fenced = findQuizBlocks(body || node.content);
    if (!fenced.length) return empty;

    const challenges = fenced.map((f) => normalizeChallenge(f.challenge));
    const complete = challenges.every((c) => isQuizChallengeComplete(c));
    const primary = challenges[0];
    const traps = Array.isArray(primary.traps) ? primary.traps.filter((t) => String(t || '').trim()) : [];
    const coreConcept = String(primary.core_concept || '').trim();
    const shortDef = String(primary.short_definition || '').trim();
    const question = String(primary.main_question || '').trim();
    const correct = String(primary.correct_answer || '').trim();
    const hasQuiz = challenges.some(
        (c) =>
            !!(c.core_concept || c.main_question || c.correct_answer || c.short_definition || (c.traps || []).length)
    );
    let questionCount = 0;
    for (const c of challenges) {
        if (c.items?.length) questionCount += c.items.length;
        else if (c.main_question || c.correct_answer) questionCount += 1;
    }

    return {
        hasQuiz,
        complete,
        quizCount: fenced.length,
        questionCount,
        isExam: isExamLesson(node),
        coreConcept,
        shortDef,
        question,
        correct,
...
export function getSageSupportResponse(action, ui, ctx = {}) {
    ...
    if (action === 'quiz-exam-author') {
        return ui.sageQuizExamAuthorHelp || '';
    }
    if (action === 'exam-student-help') {
        return ui.sageExamStudentHelp || '';
    }
    return '';
}
