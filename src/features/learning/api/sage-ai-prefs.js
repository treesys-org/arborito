...
const SAGE_LLAMA_VULKAN_KEY = 'arborito_sage_llama_vulkan';

/** Desktop: prefer Vulkan llama.cpp build + GPU layers when the platform supports it.
 * Default ON when unset (unsupported platforms still use the CPU binary). */
export function resolveLlamaVulkan() {
    try {
        const raw = localStorage.getItem(SAGE_LLAMA_VULKAN_KEY);
        if (raw === '0' || raw === 'false') return false;
        if (raw === '1' || raw === 'true') return true;
    } catch (_) {}
    return true;
...
export function writeLlamaVulkan(enabled) {
    try {
        localStorage.setItem(SAGE_LLAMA_VULKAN_KEY, enabled ? '1' : '0');
    } catch (_) {}
}

export function resetLlamaVulkanPref() {
    try {
        localStorage.removeItem(SAGE_LLAMA_VULKAN_KEY);
    } catch (_) {}
}
