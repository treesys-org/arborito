...
import { collectTreeRagEvidence, buildSageActiveLessonContext, expandSageRagQuery, resolveSageQueryTarget, preloadRagLessonContent } from './sage-tree-rag.js';
...
async function buildSageContextBlock({ mode, lang, lastMsg, contextNode, messages, preset }) {
    if (isTrivialGreeting(lastMsg)) return '';

    const budgets = resolveContextPresetBudgets(preset);

    /* Game mode: inject the active lesson into the system prompt. Tree-wide RAG
     * is skipped (games already embed context in user messages via ask.json). */
    if (mode === 'game') {
        const lessonNode =
            contextNode && (contextNode.content || isLessonNode(contextNode))
                ? { ...contextNode, type: contextNode.type || 'leaf' }
                : null;
        if (!lessonNode?.content) return '';
        const activeLesson = buildSageActiveLessonContext(store, lessonNode, lang, budgets.lesson);
        return activeLesson ? clipContextToWindow(activeLesson, preset) : '';
    }

    if (mode !== 'sage-tree' && mode !== 'architect') return '';
    const parts = [];
    ...
    if (treeRoot || rawGraph) {
        ...
        await preloadRagLessonContent(store, rawGraph, lang, ragOpts, 2);
        const rag = collectTreeRagEvidence(rawGraph, lang, ragOpts);
...
class HybridAIService {
    ...
    async chat(messages, contextNode = null, onStream = null) {
        ...
        try {
            ...
            let contextBlock = await buildSageContextBlock({
                mode,
                lang,
                lastMsg,
                contextNode,
