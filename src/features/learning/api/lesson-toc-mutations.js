...
export function getTocLineRanges(body) {
    ...
    const tocBlocks = [];
    blocks.forEach((b) => {
        if (['h1', 'section', 'h2', 'subsection', 'h3', 'h4', 'h5', 'h6'].includes(b.type)) {
            tocBlocks.push(b);
        }
    });
    ...
    for (let k = 0; k < headingIndices.length; k++) {
        ...
        let headingRaw = rawLine;
        if (isSectionFenceLine(t)) headingRaw = `@@section@@${fencedTitleAt(lines, start)}`;
        else if (isSubsectionFenceLine(t)) headingRaw = `@@subsection@@${fencedTitleAt(lines, start)}`;
        ranges.push({
            id,
            startLine: start,
            endLine: end,
...
function headingPrefixFromLine(line) {
    const t = line.trim();
 ...
 */
function moveTocSectionRange(body, fromIdx, toIdx, mode = 'reorder', desiredLevel = null) {
    const ranges = getTocLineRanges(body);
...
function tocEffectiveOutlineLevel(ranges, toc, tocIdx) {
    const r = ranges[tocIdx];
    if (!r) return 99;
    const L = tocRangeOutlineLevel(r);
    const itemLevel = toc?.[tocIdx]?.level;
    if (itemLevel != null && itemLevel > L) return itemLevel;
    return L;
}

/** Move availability for one TOC index (same indices as `getToc`). */
export function tocSectionMoveAvailability(body, tocIdx, toc) {
    const none = { canUp: false, canDown: false, canOutdent: false, canIndent: false };
    const ranges = getTocLineRanges(body);
    const r = ranges[tocIdx];
    const item = toc?.[tocIdx];
    if (!r || r.isQuiz || item?.isQuiz) return none;
    const L = tocRangeOutlineLevel(r);
    if (!L || L === 99) return none;

    let prev = tocIdx - 1;
    while (prev >= 0 && tocRangeOutlineLevel(ranges[prev]) > L) prev--;
    const canUp = prev >= 0 && tocRangeOutlineLevel(ranges[prev]) === L;

    const subEnd = movedSubtreeExclusiveEnd(ranges, tocIdx);
    const next = subEnd;
    const canDown = next < ranges.length && tocRangeOutlineLevel(ranges[next]) === L;

    const effectiveL = tocEffectiveOutlineLevel(ranges, toc, tocIdx);
    const canOutdent = effectiveL > 2;
    const prevEffectiveL = tocIdx > 0 ? tocEffectiveOutlineLevel(ranges, toc, tocIdx - 1) : 0;
    const canIndent = tocIdx > 0 && effectiveL < 6 && effectiveL <= prevEffectiveL;
 ...
 * @param {string} body
 * @param {number} tocIdx
 * @param {'up'|'down'|'indent'|'outdent'} action
 * @param {{ level?: number }[] | null | undefined} [toc]
 */
export function moveTocSectionByAction(body, tocIdx, action, toc) {
    const ranges = getTocLineRanges(body);
    if (!Number.isFinite(tocIdx) || tocIdx < 0 || tocIdx >= ranges.length) return body;
    const r = ranges[tocIdx];
    if (!r || r.isQuiz) return body;

    const avail = tocSectionMoveAvailability(body, tocIdx, toc);
    const L = tocRangeOutlineLevel(r);
    const effectiveL = tocEffectiveOutlineLevel(ranges, toc, tocIdx);

    if (action === 'up') {
        if (!avail.canUp) return body;
        let prev = tocIdx - 1;
        while (prev >= 0 && tocRangeOutlineLevel(ranges[prev]) > L) prev--;
        return reorderTocSectionRange(body, tocIdx, prev);
    }
    if (action === 'down') {
        if (!avail.canDown) return body;
        const subEnd = movedSubtreeExclusiveEnd(ranges, tocIdx);
        const nextEnd = movedSubtreeExclusiveEnd(ranges, subEnd);
        return reorderTocSectionRange(body, tocIdx, nextEnd);
    }
    if (action === 'outdent') {
        if (!avail.canOutdent) return body;
        return setTocSectionLevel(body, tocIdx, effectiveL - 1);
    }
    if (action === 'indent') {
        if (!avail.canIndent) return body;
        return setTocSectionLevel(body, tocIdx, effectiveL + 1);
    }
    return body;
}
...
export function setTocSectionLevel(body, tocIndex, targetLevel) {
    ...
    // Outline mapping: depth=0 => '##' (H2). Never go below H2.
...
export function renameTocSection(body, tocIndex, newTitle, emojiPrefix) {
    ...
    if (kind === 'section') {
        replaceFencedBlockAt(lines, r.headingLine, 'section', { title: combined });
    } else if (kind === 'subsection') {
        replaceFencedBlockAt(lines, r.headingLine, 'subsection', { title: combined });
    } else {
...
function firstMajorPartRangeAfterIntro(ranges) {
    for (let i = 1; i < ranges.length; i++) {
        ...
        if (isSubsectionFenceLine(raw)) continue;
...
function tocHeadingEmojiPrefix(headingRaw) {
    if (!headingRaw) return '';
    const t = headingRaw.trim();
    let inner = t;
    if (t.startsWith('###### ')) inner = t.slice(7);
