...
function modeIsPlayable(c, mode) {
    switch (mode) {
        case QUIZ_MODE_CLOZE:
            /* Cloze targets definition words; skip when a distinct main question is authored. */
            return !!(
                c.short_definition &&
                c.cloze_indices.length > 0 &&
...
export function isQuizChallengeComplete(challenge) {
    ...
    if (Array.isArray(c.items) && c.items.length) {
        return c.items.every((item) => isQuizChallengeComplete(item));
    }
...
export function getChallengeValidationHints(challenge) {
    ...
    if (c.correct_answer && playable.length === 0) {
        const need = [];
        if (!c.traps.length && !c.skip_multiple) {
            need.push({
...
export function pickStudyQuizMode(challenge, blockId, salt = '') {
    ...
    const attempt = Number.isFinite(Number(salt)) ? Number(salt) : 0;
...
export function expandQuizBlock(block) {
    ...
    if (items.length) {
        return items.map((item, i) => challengeToQuizBlock(item, `${baseId}:${i}`));
    }
...
export function parseQuizBlock(bodyLinesOrText) {
    ...
    for (const raw of lines) {
        ...
        const itemField = line.match(/^\s{2,}([a-zA-Z_]+)\s*:\s*(.*)$/);
        ...
        /* Nested traps/steps inside an item (4+ spaces) — before itemStart. */
        const itemList = line.match(/^\s{4,}-\s+(.*)$/);
        if (itemList && listKey === 'items' && currentItem && itemListKey) {
            const v = itemList[1].trim();
            if (v) currentItem[itemListKey].push(v);
            continue;
        }

        /* New item row: exactly two spaces before the dash (see serializeQuizBlock). */
...
export function serializeQuizBlock(c) {
    ...
    if (Array.isArray(n.items) && n.items.length) {
        ...
        for (const item of n.items) {
            ...
            const playable = getPlayableModes(ni);
            ...
            if (ni.skip_multiple) lines.push('    skip_multiple: yes');
            if (ni.skip_ordering) lines.push('    skip_ordering: yes');
        }
        lines.push('@/quiz');
        return lines.join('\n');
    }
