...
import { ContentRenderer } from '../tree-graph/renderer.js';
import { getEditorToolbarInnerHtml } from '../editor/editor-toolbar-html.js';
import {
    getMediaConsentModalMarkup,
    getPendingExternalMediaDetails,
    isMediaSrcBlocked
} from '../privacy-gdpr/third-party-media.js';
import { NODE_PROPERTY_EMOJIS } from '../tree-graph/node-property-emojis.js';
import { getQuizV2RenderBlockFromContent, lessonContentHasCompleteQuiz } from './quiz-v2-status.js';
...
export function renderContentHtml(ctx, allBlocks, toc, filteredToc, activeBlocks, progress, isExam, onExamIntro) {
    ...
    const saveBtnHtml = constructEdit ? `<button type="button" id="btn-lesson-save" data-arbor-tour="lesson-edit-save" class="max-sm:absolute max-sm:top-3 max-sm:right-4 max-sm:z-[136] arborito-lesson-save-btn arborito-cta-emerald shrink-0 px-6 py-2.5 sm:px-5 sm:ml-auto rounded-lg text-sm font-black transition-[transform,box-shadow] active:scale-[0.98] border border-emerald-700/30 shadow-sm">Guardar</button>` : '';
    ...
    const lessonTitleBlockConstruct = `
                        <div class="arborito-lesson-meta-hit group flex items-start gap-2 w-full min-w-0 min-h-0 rounded-xl -mx-1 px-1 py-0.5 text-left transition-colors hover:bg-amber-100/40 dark:hover:bg-amber-950/25" data-arbor-tour="lesson-edit-meta">
                            ...
                            <div class="min-w-0 flex-1 flex flex-col gap-0.5">
                                <div class="flex items-start gap-1.5 min-w-0">
                                    ...
                                    ${isExam ? `<span class="arborito-pill arborito-pill--chip arborito-pill--red arborito-pill--bordered shrink-0 align-middle">${ui.tagExam || 'EXAM'}</span>` : ''}
                                </div>
    ...
    const lessonTitleBlockRead = `
                        ...
                        <div class="min-w-0 flex-1">
                            <h1 class="line-clamp-3 flex flex-wrap items-center gap-1.5">${escHtml(ctx.currentNode.name)}${isExam ? ` <span class="arborito-pill arborito-pill--chip arborito-pill--red arborito-pill--bordered align-middle">${escHtml(ui.tagExam || 'EXAM')}</span>` : ''}</h1>
                            ${careFeedback}
    ...
    <aside class="${containerClasses.join(' ')} transform translate-x-0 ${constructEdit ? 'arborito-lesson-aside--construct-edit' : ''}">
        ...
        <div class="relative z-20 flex items-center justify-between px-4 pt-1">
            <span class="arborito-eyebrow">${ui.lessonProgress}</span>
