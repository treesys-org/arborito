...
export function useSageVoice({ mode, onRerender, inputRef }) {
        ...
        const body =
            kind === 'mic'
                ? ui.sageVoiceMicDownloadConsentBody ||
                  `Para usar el micrófono se descarga Whisper (~${estMb} MB), una sola vez. ¿Continuar?`
                : ui.sageVoiceDownloadConsentBody ||
                  `Se descargará la voz Piper (~${estMb} MB). ¿Continuar?`;
        const licenseNote = ui.sageVoiceThirdPartyLicenses || ui.sageAiThirdPartyLicenses || '';
        const title =
            kind === 'mic'
                ? ui.sageVoiceMicDownloadConsentTitle || '¿Activar micrófono?'
                : ui.sageVoiceDownloadConsentTitle || '¿Activar voz (Piper)?';
        setVoiceConsent({
            kind,
            mode: consentMode,
            title,
    ...
    const speakMessageAtIndex = useCallback(
        ...
        [ensureTtsAssetsOrPromptConsent, onRerender]
    );
            ...
            store.update({
                ai: {
                    ...
                    messages: [
                        ...
                        {
                            role: 'assistant',
                            content: ui.sageVoiceBusy || 'Espera a que termine la respuesta del Sabio.',
                        },
                    ],
                },
            });
            ...
            store.update({
                ai: {
                    ...
                    messages: [
                        ...
                        {
                            role: 'assistant',
                            content: ui.sageVoiceUnavailable || 'Voz no disponible. Usa la app de escritorio.',
                        },
                    ],
                },
            });
