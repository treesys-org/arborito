...
function SageMessageBubbleImpl({
    message,
    idx,
    ui,
    isStreamingRow,
    ...
    const speakLabel = isSpeaking
        ? ui.sageVoiceStopPlayback || 'Stop playback'
        : ui.sageVoiceReadMessage || 'Listen to message';
    const speakOpacity = isSpeaking
        ? 'opacity-100'
        : 'opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100';

    const bodyContent = isStreamingRow ? (
        <span className="sage-md-line whitespace-pre-wrap">{contentForFormat}</span>
    ) : (
    ...
    return (
        ...
        >
            <div className="max-w-[85%] relative group text-left">
                <div
                    className={`p-3 rounded-2xl text-sm leading-relaxed shadow-sm sage-msg-bubble ${
                        message.role === 'user'
                            ? `${sendBtnColor} rounded-br-none`
                            : 'sage-msg-bubble--assistant rounded-bl-none'
                    }${isStreamingRow ? ' sage-msg-bubble--streaming' : ''}`}
                >
                    <div
...
/** Finished bubbles skip re-render while the last row streams. */
export const SageMessageBubble = memo(SageMessageBubbleImpl, (prev, next) => {
    return (
        prev.idx === next.idx &&
        prev.isStreamingRow === next.isStreamingRow &&
        prev.displayContent === next.displayContent &&
        prev.activeSpeakMsgIdx === next.activeSpeakMsgIdx &&
        prev.showVoiceMic === next.showVoiceMic &&
        prev.sendBtnColor === next.sendBtnColor &&
        prev.message?.role === next.message?.role &&
        prev.onSpeak === next.onSpeak &&
        prev.onPrivacy === next.onPrivacy
    );
});
