    ...
    if (quiz.hasQuiz) {
        cards.push(
            <SageGuideCard
                key="quiz"
                icon="✅"
                title={ui.sageBtnQuizStatus || 'Quiz status'}
                hint={ui.sageLessonQuizHint || 'Whether the review quiz is ready'}
                action="open-topic"
                topic="lesson-quiz"
                tone="violet"
            />
                ...
                <SageGuideHero
                    ...
                    chip={
                        quiz.hasQuiz
                            ? quiz.complete
                                ? quiz.isExam
                                    ? ui.sageGuideExamReady || 'Exam quizzes ready'
                                    : ui.sageGuideQuizReady || 'Quiz ready'
                                : ui.sageGuideQuizDraft || 'Quiz incomplete'
                            : ui.sageGuideNoQuiz || 'No quiz'
                    }
                    compact
                />
...
function buildTopicLessonQuiz(ui, nav, node) {
    const quiz = getSageQuizContent(node);
    const features = quiz.hasQuiz
        ? quiz.complete
            ? [
                  { icon: '✅', title: ui.sageTopicLessonQuizS1t, text: ui.sageTopicLessonQuizS1b },
                  { icon: '🎮', title: ui.sageTopicLessonContinueS2t, text: ui.sageTopicLessonContinueS2b },
              ]
            : [{ icon: '⏳', title: ui.sageTopicLessonQuizS2t, text: ui.sageTopicLessonQuizS2b }]
        : [{ icon: '📄', title: ui.sageTopicLessonQuizS3t, text: ui.sageTopicLessonQuizS3b }];
    return (
        <SageGuideTopicRich
            ui={ui}
            nav={nav}
            lead={ui.sageTopicLessonQuizLead}
            features={features}
        />
    );
}
...
function buildTopicScreen(ui, ctx, nav, learning) {
    ...
    switch (topicId) {
        ...
        case 'lesson-quiz':
            return buildTopicLessonQuiz(ui, nav, ctx.node);
        case 'lesson-summary':
...
function buildTipScreen(ui, nav) {
    ...
    const empty = ui.sageGuideTipEmpty || 'No extra information to show here.';
