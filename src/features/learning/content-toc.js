...
function tocLabelForDisplay(raw) {
...
export function annotateTocWithQuizSections(blocks, toc) {
    ...
    return toc.map((item, i) => {
        ...
        const hasQuiz = slice.some((b) => b.type === 'quizv2');
...
export function isTocSectionCompleted(sectionIndex, toc, blocks, visitedSections, getQuizState) {
    ...
    return quizzes.every((q) => {
        const st = getQuizState(q.id || 'quiz-v2-meta');
        if (!st || !st.finished) return false;
        if (st.v2Correct != null) return !!st.v2Correct;
...
export function buildTocListMarkup(toc, filteredToc, activeSectionIndex, visitedSections, opts = {}) {
    ...
    return filteredToc.map((item) => {
        ...
        const tickHtml = !construct
            ? `${(opts.isSectionCompleted ? opts.isSectionCompleted(idx) : visitedSections.has(idx))
