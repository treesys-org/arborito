...
function resolveLessonMediaBranchId(activeSource) {
    ...
    return String(activeSource?.id || '').trim();
}

function LessonImage({ src, caption, ui, blocked, onRetry, branchId }) {
    const resolved = useResolvedLessonMediaSrc(src, branchId);
    if (blocked) return <ExternalMediaPlaceholder b={{ type: 'image', src }} ui={ui} onRetry={onRetry} />;
    return (
        <figure className="my-10 flex flex-col items-center">
            <img
                src={resolved}
                className="rounded-xl shadow-lg w-full max-w-2xl max-h-[32rem] h-auto object-contain mx-auto"
                loading="lazy"
                alt=""
            />
            {caption ? (
                <figcaption className="text-center text-sm text-slate-500 mt-2 max-w-2xl">
                    <AuthorInline text={caption} />
                </figcaption>
            ) : null}
        </figure>
    );
...
function LessonAudio({ src, ui, blocked, onRetry, branchId }) {
    const resolved = useResolvedLessonMediaSrc(src, branchId);
...
function ExternalMediaPlaceholder({ b, ui, onRetry }) {
...
export function ContentBlock({
    ...
    const { ui, activeSource } = useLearning();
    const branchId = resolveLessonMediaBranchId(activeSource);
    const b = block;
    if (!b) return null;
    ...
    switch (b.type) {
        ...
        case 'section':
            return (
                <h1
                    ...
                    className={
                        compact
                            ? `text-base font-bold text-slate-800 dark:text-slate-100 mb-2 tracking-tight${al}`
                            : `text-3xl md:text-5xl font-black text-slate-900 dark:text-white mb-6 md:mb-8 pb-4 border-b border-slate-200 dark:border-slate-800 tracking-tight${al}`
                    }
                >
        ...
        case 'subsection':
            return (
                <h2
                    ...
                    className={
                        compact
                            ? `text-sm font-bold text-slate-800 dark:text-sky-100 mt-3 mb-1.5${al}`
                            : `text-2xl md:text-3xl font-bold text-slate-800 dark:text-sky-100 mt-10 md:mt-12 mb-6 group flex items-center gap-3${al}`
                    }
                >
                    <AuthorInline text={b.text} />
                </h2>
        ...
        case 'h3':
            return (
                <h3
                    ...
                    className={
                        compact
                            ? `text-sm font-semibold text-slate-700 dark:text-slate-200 mt-3 mb-1${al}`
                            : `text-xl font-bold text-slate-700 dark:text-slate-200 mt-8 mb-4 flex items-center gap-2${al}`
                    }
                >
                    {compact ? null : <span className="w-2 h-2 bg-sky-500 rounded-full" />}
                    <span>
                        <AuthorInline text={b.text} />
                    </span>
                </h3>
        ...
        case 'h4':
            return (
                <h4
                    ...
                    className={
                        compact
                            ? `text-sm font-semibold text-slate-700 dark:text-slate-300 mt-2 mb-1${al}`
                            : `text-lg font-bold text-slate-700 dark:text-slate-300 mt-6 mb-3 pl-2 border-l-2 border-sky-400/70${al}`
                    }
                >
                    <AuthorInline text={b.text} />
                </h4>
        ...
        case 'h5':
            return (
                <h5
                    ...
                    className={
                        compact
                            ? `text-xs font-semibold text-slate-600 dark:text-slate-400 mt-2 mb-1${al}`
                            : `text-base font-bold text-slate-600 dark:text-slate-400 mt-5 mb-2 pl-2 border-l border-slate-300 dark:border-slate-600${al}`
                    }
                >
                    <AuthorInline text={b.text} />
                </h5>
        ...
        case 'h6':
            return (
                <h6
                    ...
                    className={
                        compact
                            ? `text-xs font-semibold text-slate-600 dark:text-slate-500 mt-2 mb-1${al}`
                            : `text-sm font-bold text-slate-600 dark:text-slate-500 mt-4 mb-2 pl-2${al}`
                    }
                >
                    <AuthorInline text={b.text} />
                </h6>
        ...
        case 'p':
            return (
                <p
                    className={
                        compact
                            ? `mb-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300${al}`
                            : `mb-6 text-slate-600 dark:text-slate-300 leading-8 text-base md:text-lg${al}`
                    }
                >
                    <AuthorInline text={b.text} />
                </p>
        ...
        case 'blockquote':
            return (
                <blockquote
                    className={
                        compact
                            ? 'bg-yellow-50 dark:bg-yellow-900/10 border-l-4 border-yellow-400 px-3 py-2 my-3 rounded-r-lg italic text-sm text-slate-700 dark:text-yellow-100/80'
                            : 'bg-yellow-50 dark:bg-yellow-900/10 border-l-4 border-yellow-400 p-6 my-8 rounded-r-xl italic text-slate-700 dark:text-yellow-100/80'
                    }
                >
                    &ldquo;
                    <AuthorInline text={b.text} />
        ...
        case 'math':
            return <MathFormula latex={b.latex} display={b.display} />;
        case 'code':
            return (
        ...
        case 'table': {
            ...
            return (
                <div className={`arborito-lesson-table-wrap my-8 overflow-x-auto not-prose${al}`}>
                    <table className="arborito-lesson-table w-full text-left border-collapse">
                        {headers.length ? (
                            <thead>
                                <tr>
                                    {headers.map((h, i) => (
        ...
        case 'video':
            if (blocked) return <ExternalMediaPlaceholder b={b} ui={ui} onRetry={onMediaRetry} />;
            return <LessonVideoPlayer src={b.src} branchId={branchId} />;
        case 'audio':
            return (
                <LessonAudio src={b.src} ui={ui} blocked={blocked} onRetry={onMediaRetry} branchId={branchId} />
            );
        case 'game': {
            ...
            const topicMap = new Map((topicCatalog || []).map((t) => [String(t.id), String(t.text || t.id)]));
            const topicLabels = topics.map((id) => topicMap.get(String(id)) || id);
            return (
                <div className="not-prose my-10 rounded-3xl border border-orange-200/60 dark:border-orange-900/35 bg-gradient-to-br from-orange-50 to-white dark:from-slate-900 dark:to-slate-900/60 p-6 shadow-xl overflow-hidden relative">
                    ...
                    <div className="relative z-10 flex flex-col gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                            <button
                                type="button"
                                className={`btn-game-launch arborito-cta-emerald px-5 py-3 rounded-xl font-black uppercase tracking-wider text-sm hover:opacity-90 active:scale-[0.99] transition ${!url ? 'opacity-50 cursor-not-allowed' : ''}`}
                                disabled={!url}
                            ...
                            {url ? (
                                <div className="flex flex-col gap-1">
                                    ...
                                    {topicLabels.length > 0 ? (
                                        <div className="text-[11px] text-slate-500 dark:text-slate-400 break-words select-text">
                                            {ui.editorBlockGameTopics || 'Topics'}:{' '}
        ...
        case 'list':
            if (b.ordered) {
                return (
                    <ol className={`space-y-2 my-6 pl-6 list-decimal${al}`}>
                        {b.items.map((item, i) => (
                            <li
                                key={i}
                                className="text-slate-700 dark:text-slate-300 leading-relaxed pl-1"
                            >
