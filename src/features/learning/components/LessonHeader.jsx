...
import { canIssueOptionalFolderAchievement } from '../../tree-graph/api/logic/graph-node-actions.js';

function findParentBranch(node, findNode) {
    if (!node?.parentId || typeof findNode !== 'function') return null;
    let cur = node;
    while (cur?.parentId) {
        const parent = findNode(cur.parentId);
        if (!parent) break;
        if (parent.type === 'branch') return parent;
        cur = parent;
    }
    return null;
}

function LessonDiplomaBadge({ ui, node, constructEdit, findNode }) {
    const [graphTick, setGraphTick] = useState(0);
    useEffect(() => {
        const bump = () => setGraphTick((n) => n + 1);
        const store = getArboritoStore();
        store?.addEventListener('graph-update', bump);
        window.addEventListener('graph-update', bump);
        return () => {
            store?.removeEventListener('graph-update', bump);
            window.removeEventListener('graph-update', bump);
        };
    }, []);
    const moduleNode = useMemo(
        () => findParentBranch(node, findNode),
        [node, findNode, graphTick]
    );
    if (!moduleNode || !canIssueOptionalFolderAchievement(moduleNode)) return null;
    const on = !!moduleNode.isCertifiable;
    if (!on && !constructEdit) return null;
    const earned = !constructEdit && isFolderAchievementEarned(moduleNode);

    const badgeStateClass = earned
        ? ' arborito-lesson-diploma-badge--earned'
        : on
          ? constructEdit
              ? ' arborito-lesson-diploma-badge--on arborito-lesson-diploma-badge--info'
              : ' arborito-lesson-diploma-badge--locked'
          : '';

    const label = earned
        ? ui.graphAchievementEarnedHint || 'Achievement earned'
        : on
          ? ui.lessonAchievementFolderOn ||
            ui.graphAchievementPendingHint ||
            'This folder has an achievement. Configure it from the folder on the map.'
          : ui.lessonAchievementFolderOff ||
            'No achievement on this folder. Enable it from the folder on the map.';

    return (
        <span
            className={`arborito-lesson-diploma-badge arborito-lesson-diploma-badge--read${badgeStateClass}`}
            role="note"
            aria-label={label}
            title={label}
        >
            <ChromeEmoji emoji="🏆" size={18} className="arborito-emoji-glyph" />
        </span>
    );
}
...
function ReadTools({
    ...
    const sageFullLabel = ui.navSage || 'Preguntar al Sabio';
            ...
            <div className="min-w-0 flex-1 flex flex-col gap-0.5">
                <div className="flex items-start gap-1.5 min-w-0">
                    <input
                        ...
                        defaultValue={lessonHeaderTitleValue}
                        autoComplete="off"
                        spellCheck
                ...
                <input
                    ...
                    defaultValue={lessonHeaderDescValue}
...
export function LessonHeader({
    ...
    const diplomaBadge = (
        <LessonDiplomaBadge ui={ui} node={node} constructEdit={constructEdit} findNode={findNode} />
    );

    const sageHeadSlot =
        !isExam && isMobile ? (
            <div className="arborito-lesson-sage-head-slot shrink-0 flex items-center gap-2">
                {diplomaBadge}
                <button type="button" id="btn-ask-sage" className="arborito-lesson-mtool arborito-lesson-mtool--sage arborito-lesson-mtool--compact" data-arbor-tip={ui.navSage || 'Preguntar al Sabio'} aria-label={ui.navSage || 'Preguntar al Sabio'} onClick={() => onAskSage?.()}>
                    <ChromeEmoji emoji="🦉" size={18} />
                </button>
            </div>
        ) : null;
        ...
        <div className="arborito-lesson-head-trail arborito-lesson-head-trail--desktop">
            {constructEdit ? (
                <div className="arborito-lesson-actions arborito-lesson-actions--construct arborito-lesson-actions--construct-compact" role="toolbar" aria-label={lessonToolsLabel} data-arbor-tour="lesson-edit-toolbar">
                    <div className="arborito-lesson-construct-row">
                        {!isExam && !isMobile ? (
                            <button type="button" id="btn-ask-sage" className="arborito-lesson-mtool arborito-lesson-mtool--sage" data-arbor-tip={ui.navSage || 'Preguntar al Sabio'} aria-label={ui.navSage || 'Preguntar al Sabio'} onClick={() => onAskSage?.()}>
                                <ChromeEmoji emoji="🦉" size={18} />
                            </button>
                        ) : null}
                        <div className="arborito-lesson-construct-bar">
