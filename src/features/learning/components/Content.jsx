...
import { getQuizState, getEffectiveQuizState, sectionHasLiveBlockSession } from '../api/content-panel-quiz.js';
...
const panelApi = useContentPanel({
    ...
    const editorHealForRef = useRef(null);

    /*
     * Web desktop: first open often leaves the construct editor unable to take a caret
     * until the user shrinks/enlarges the window. That resize runs viewport relayout
     * (classes + reflow + content.scheduleUpdate + editor shell rebind). Replay it once.
     */
    useEffect(() => {
        if (!isLessonConstructEdit() || !panel.currentNode) {
            editorHealForRef.current = null;
            return undefined;
        }
        const nodeId = panel.currentNode.id;
        if (editorHealForRef.current === nodeId) return undefined;
        ...
        const raf1 = requestAnimationFrame(() => {
            raf2 = requestAnimationFrame(() => {
                if (cancelled) return;
                editorHealForRef.current = nodeId;
                try {
                    document.documentElement.classList.add('arborito-lesson-open');
                } catch {
                    /* ignore */
                }
                const host = rootRef.current;
                const aside = host?.querySelector?.('.arborito-lesson-aside');
                const area = contentAreaRef.current;
                if (aside) void aside.offsetHeight;
                if (area) void area.offsetHeight;
                if (host) void host.offsetHeight;
                const app = document.getElementById('app');
                if (app) void app.offsetHeight;
                /* Remount drops any keystrokes typed in the heal window — skip if already dirty. */
                const ed = editorRef?.current;
                const skipRemount =
                    ed?.dataset?.arboritoEditorDirty === '1' ||
                    panel.lessonUserHasEdited ||
                    panel.lessonConstructDraft;
                if (!skipRemount) setEditorRemountKey((n) => n + 1);
                scheduleUpdate(true);
                try {
    ...
    const effectsCtx = useMemo(
        () => ({
            ...
            editorShellEpoch: editorRemountKey,
        }),
        [panelApi, apiRef, editorRemountKey]
    );
    ...
    const examSectionOpts = isExam
        ...
        : {};
    const quizStateForSection = (sectionIdx) => (id) =>
        sectionHasLiveBlockSession(panel.blockSessions, node.id, sectionIdx)
        ...
        <div ref={rootRef} data-arborito-panel="content" data-embed={embed ? '1' : undefined} className="w-full h-full">
            ...
            <aside className={`${ASIDE_CLASSES}${constructEdit ? ' arborito-lesson-aside--construct-edit' : ` ${ASIDE_TRANSFORM_CLASSES}`}${asideModeClass}`}>
                <LessonHeader
                    ...
                    lessonUserHasEdited={isLessonDirty()}
                />
                    ...
                    >
                        ...
                        >
                            <LessonBody
                                ...
                                constructSectionId={constructEditorSeed?.sectionId}
                                constructSeedKey={constructEditorSeed?.seedKey}
                                activeSectionIndex={panel.activeSectionIndex}
                                editorRemountKey={editorRemountKey}
