...
/** Editor-only effects (construct toolbar scroll, insert-block context). */
export function useContentEffects(rootRef, ctx) {
    ...
    useLayoutEffect(() => {
        ...
        const canScroll = area && !isLessonConstructEdit();
        if (sectionChanged) {
            if (canScroll) {
        ...
        } else if (canScroll && contentScrollSnapshotRef.current != null) {
    ...
    useLessonConstructDnD({
        ...
        constructApiRef,
        isLessonConstructEdit,
        constructActive: isLessonConstructEdit(),
        editorShellEpoch,
        lessonEditor
    });
