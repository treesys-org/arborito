...
async function initialize(modelName) {
    ...
    try {
        ...
        /* Cap inference threads so a single AI generation never saturates the
           machine: using `navigator.hardwareConcurrency` directly pinned 100% CPU
           on every core and froze the whole browser while a game lesson loaded.
           We leave half the logical cores free for the main UI thread (and any
           other tab), clamped to 1..4 — Llama 3.2 1B scales poorly above 4 anyway. */
        const cores = (typeof navigator !== 'undefined' && navigator.hardwareConcurrency) || 4;
        const nThreads = Math.max(1, Math.min(4, Math.floor(cores / 2) || 1));

        const hasSharedArrayBuffer = typeof SharedArrayBuffer !== 'undefined';
        console.log(`[AI Worker] Threads: ${nThreads}/${cores}, SharedArrayBuffer: ${hasSharedArrayBuffer}`);
...
async function generate(messages, config) {
    ...
    try {
        ...
        const gemmaStyle = /gemma/i.test(String(currentModel || ''));
        ...
        if (microMode) {
            ...
            // Prepend context strings to system or first user msg
            const prependContext = (content) => {
                let text = content;
            ...
            if (firstUser) {
                firstUser.content = prependContext(firstUser.content);
            }
            
            // If system format is not natively supported or needs folding
            if (gemmaStyle) {
                // Gemma doesn't use system, treated in buildGemmaPrompt
                fullPrompt = buildGemmaPrompt(chatMessages);
            } else if (llama32Style) {
                fullPrompt = buildLlama3InstructPrompt(chatMessages);
            } else {
                // Default to ChatML (SmolLM2, etc)
                fullPrompt = buildChatMLPrompt(chatMessages);
            }
        }
