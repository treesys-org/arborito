...
import { ForumThreads } from './ForumThreads.jsx';
...
export function ForumGateNoTree({ ui, embedded, mobile, onClose, onOpenTrees }) {
    const body = (
        <div className="p-6 sm:p-8 flex flex-col items-center gap-4 text-center max-w-md mx-auto">
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed m-0">{ui.forumNoTree || 'Load a tree first.'}</p>
            <button
                type="button"
                className="forum-open-trees arborito-cta-emerald w-full sm:w-auto min-h-12 px-6 py-3 rounded-2xl font-bold shadow-lg shadow-emerald-900/20"
                onClick={onOpenTrees}
            >
    ...
    return (
        <DockModalShell
            ...
            onBackdropClick={embedded ? undefined : onClose}
            hero={
                <ModalHubHero
                    ui={ui}
                    mobile={mobile}
...
export function ForumGateLogin({ ui, embedded, mobile, onClose, onOpenProfile }) {
    const body = (
        <div className="flex flex-1 flex-col items-center justify-center p-5 sm:p-6 text-center min-h-0 overflow-y-auto">
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100 m-0 max-w-md leading-snug">
                {ui.forumParticipationNeedLoginTitle || ui.publishNeedLoginTitle || 'Sign in required'}
            </p>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-2.5 leading-relaxed max-w-md m-0">
                {ui.forumParticipationNeedLoginBody ||
                    ui.publishNeedLoginBody ||
                    'Sign in from Profile to use the forum on public courses.'}
            </p>
            <div className="arborito-action-row arborito-action-row--stack-mobile mt-5 max-w-sm shrink-0 w-full">
                <button
                    type="button"
                    className="forum-gate-open-profile arborito-cta-emerald min-h-10 rounded-xl px-4 py-2.5 text-sm font-bold shadow-md shadow-emerald-900/15 w-full"
    ...
    if (embedded) {
        return (
            <div
                ...
                data-embed="1"
                className="arborito-forum-embed-root flex flex-col flex-1 min-h-0 w-full min-w-0 overflow-hidden"
            >
                {mobNavOpen ? (
                    <>
                        {mobNavHero}
