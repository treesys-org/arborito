...
function placeTypeFallbackIcon(t) {
    return t === 'leaf' ? '📄' : t === 'exam' ? '📝' : t === 'root' ? '🌳' : '🗂️';
}

function placeIconFromNode(n, t) {
    const raw = n && n.icon != null ? String(n.icon).trim() : '';
    return raw ? folderDisplayIcon(raw) : placeTypeFallbackIcon(t);
}

/** Tree nodes may use `name` or compact `n`. */
function placeNodeName(n, fallback = 'Untitled') {
    const raw = n && (n.name != null ? n.name : n.n);
...
/** Normalize place/thread selection after collapse, filter, or stale thread ids. */
function resolveForumNavState({
    placeId,
    threadId,
    mobilePanel,
    replyParentId,
    ...
    let nextPlaceId = placeId;
    let nextThreadId = threadId;
    let nextMobilePanel = mobilePanel;
    let nextReplyParentId = replyParentId;

    const snapped = snapForumPlaceIdIfCollapsed(nextPlaceId, collapsedForumPlaceIds, placeById);
    if (snapped !== undefined) {
        nextPlaceId = snapped;
        nextThreadId = null;
        nextReplyParentId = null;
        nextMobilePanel = 'threads';
    }

    const filterSet = forumPlaceFilterMatchSet(places, forumPlaceFilterQ);
    if (filterSet && !isGeneral(nextPlaceId) && !filterSet.has(String(nextPlaceId))) {
        nextPlaceId = null;
        nextThreadId = null;
        nextReplyParentId = null;
        nextMobilePanel = 'threads';
    ...
    if (nextThreadId) {
        const meta = (allThreads || []).find((x) => x.id === nextThreadId);
        if (meta && !threadsFor(allThreads, nextPlaceId).some((x) => x.id === nextThreadId)) {
            nextPlaceId = threadNode(meta);
        }
    }

    let here = threadsFor(allThreads, nextPlaceId);
    if (nextThreadId && !here.some((t) => t.id === nextThreadId)) {
        nextThreadId = null;
    }
    if (!nextThreadId) {
        nextMobilePanel = 'threads';
    }
    here = sortByActivity(here, allMessages);

    const msgs = nextThreadId ? (allMessages || []).filter((m) => m.threadId === nextThreadId) : [];
    if (nextReplyParentId && !msgs.some((m) => m.id === nextReplyParentId)) {
        nextReplyParentId = null;
    }
    ...
    const changed =
        ...
        nextMobilePanel !== mobilePanel ||
        nextReplyParentId !== replyParentId;

    return {
        placeId: nextPlaceId,
        threadId: nextThreadId,
        mobilePanel: nextMobilePanel,
        replyParentId: nextReplyParentId,
        places,
        placeById,
        here,
        msgs,
...
function forumPlaceLabel(places, placeId, ui) {
    const p = places.find((x) => (isGeneral(placeId) ? x.isGeneral : String(x.id) === String(placeId)));
