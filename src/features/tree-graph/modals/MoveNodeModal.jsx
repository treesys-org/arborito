...
function findParentId(root, nodeId) {
...
function collectMoveTargets(root, movingNode) {
    ...
    const currentParentId =
        movingNode.parentId != null && movingNode.parentId !== ''
            ? movingNode.parentId
            : findParentId(root, movingNode.id);
    ...
    const walk = (n) => {
        if ((n.type === 'root' || n.type === 'branch') && !invalid.has(n.id)) {
            out.push({
                node: n,
                isCurrentParent:
                    currentParentId != null && String(n.id) === String(currentParentId),
            });
        }
...
export function ModalMoveNode() {
    ...
    const decorated = rawTargets
        .map(({ node: t, isCurrentParent }) => ({
            node: t,
            isCurrentParent,
            crumb: breadcrumbPath(treeRoot, t),
        }))
    ...
    const onSelectTarget = async (id, isCurrentParent) => {
        dismissModal();
        if (!id) return;
        const ok = await moveNode(moving, id);
        if (!ok) return;
        if (isCurrentParent) {
            queueMicrotask(() =>
                alert(
                    ui.moveReorderToEnd ||
                        'Kept in the same folder and moved to the end of its siblings.'
                )
            );
        }
    };
    ...
    const listBody = (
        <>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-2 custom-scrollbar" id="move-node-target-list">
                {visible.map(({ node: t, crumb, isCurrentParent }) => {
                        ...
                        >
                            ...
                            <span className="min-w-0 flex-1 flex flex-col gap-0.5">
                                ...
                                <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 break-words leading-snug">
                                    {isCurrentParent
                                        ? ui.moveCurrentFolderReorderHint ||
                                          `${crumb} · move to end of this folder`
                                        : crumb}
                                </span>
                            </span>
                        </button>
