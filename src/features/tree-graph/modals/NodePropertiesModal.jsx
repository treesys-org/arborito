...
export function ModalNodeProperties() {
    ...
    useEffect(() => {
        ...
        async function loadMeta() {
            const targetPath = getNodeMetaTargetPath(node, contentLang);
    ...
    const save = async () => {
        ...
        setSaving(true);
        try {
            /* Capture live construct body — do not run leave/Discard (that wipes drafts). */
            const bodyForSave =
    ...
    const saveBtnContent = saved ? (
        <>
            <ChromeEmoji emoji="✅" size={18} /> Saved
        </>
    ) : saving ? (
        <LoadingButtonContent label="Saving..." />
    ) : (
        <>
            <ChromeEmoji emoji="💾" size={18} /> Save Changes
        </>
    );
    ...
    const formBody = (
        <div className="p-6 space-y-4 arborito-mob-scroll-pane custom-scrollbar">
            ...
            <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">
                    {ui.nodePropertiesDescription || ui.editorLabelDesc || 'Description'}
                </label>
                <textarea
                    id="inp-desc"
