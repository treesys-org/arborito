...
import { consumePendingCurriculumSwitcher } from '../../editor/api/curriculum-switcher-pending.js';
import { scheduleSearchIndexRebuild } from '../../search/api/search-index-service.js';
import { maybeNotifyCareDue } from '../../garden-progress/api/care-reminders.js';
import { runThrottledBackgroundTask } from '../../../shared/lib/background-task-gate.js';
import { maybeNotifyRemoteVersionUpdate } from '../../version-updates/api/version-update-notify.js';
...
export const DataProcessor = {
    ...
    process(store, json, finalSource, options = {}) {
        ...
        /** Root-only JSON: no `languages` wrapper with entries (single root node shape). */
        ...
        if (json && hasLanguageMap) {
            ...
            /* Normalise to ASCII uppercase + drop region separators so `pt_BR`, `pt-BR`,
             * `PT_br` etc. all collide on the same bucket. This is intentional: the user's
             * UI lang is `ES`, `PT_BR`, etc. and a tree may declare any close variant. We
             * still keep the original keys for the actual lookup. */
            const norm = (c) => String(c || '').trim().toUpperCase().replace(/[-_]/g, '');
            /** Base of a code (`PT_BR` → `PT`) used for cross-region fallback. */
            const base = (c) => norm(c).slice(0, 2);
            const keyByNorm = new Map(availableLangs.map((k) => [norm(k), k]));
            const keyByBase = new Map();
            for (const k of availableLangs) {
                const b = base(k);
                if (b && !keyByBase.has(b)) keyByBase.set(b, k);
            }

            const uiLang = store.state.lang;
            const editLang = store.state.curriculumEditLang;

            /* Resolution order:
             *   1) Construction mode + an explicit edit-lang the user picked, never
             *      override it, the user is actively translating that variant.
             *   2) Exact match on the user's UI language (normalised). This is the
             *      main "show the tree in the user's language" path.
             *   3) Same base language, different region (`UI = pt_BR` ↔ `tree = pt`).
             *   4) First declared language as last-ditch fallback (no silent error). */
            let resolvedKey = null;
            if (
                store.state.constructionMode &&
                editLang &&
                keyByNorm.has(norm(editLang))
            ) {
                resolvedKey = keyByNorm.get(norm(editLang));
            } else if (uiLang && keyByNorm.has(norm(uiLang))) {
                resolvedKey = keyByNorm.get(norm(uiLang));
            } else if (uiLang && keyByBase.has(base(uiLang))) {
                resolvedKey = keyByBase.get(base(uiLang));
        ...
        const processNode = (node) => {
            ...
            if ((node.type === 'branch' || node.type === 'root') && node.content) {
                try {
                    const folderMeta = parseFolderReadme(String(node.content));
                    if (folderMeta.icon) node.icon = folderMeta.icon;
                    if (folderMeta.description != null) node.description = folderMeta.description;
                    if ('certifiable' in folderMeta) node.isCertifiable = !!folderMeta.certifiable;
                } catch {
                    /* ignore malformed README */
                }
            }
        ...
        if (!suppressReadmeAutoOpen && !store.state.constructionMode) {
            setTimeout(() => {
                void runThrottledBackgroundTask(
                    `care-reminder:${store.state.activeSource?.id || 'tree'}`,
                    () => maybeNotifyCareDue(store),
                    { oncePerSession: true, minIntervalMs: 12000 }
                );
            }, 3000);
        }

        // Modals: only on full loadData (not internal refresh). Priority: versions after snapshot change > readme intro.
        if (!suppressReadmeAutoOpen && store.state.activeSource && !store.state.modal && !store.state.treeHydrating) {
            ...
            if (pendingSwitch) {
                ...
                const skipReleasesAuto =
                    ...
                    ((finalSource && finalSource.url) && String(finalSource.url).startsWith('branch://'));
                if (!skipReleasesAuto) {
                    setTimeout(() => {
                        if (store.state.modal || store.state.treeHydrating || store.state.constructionMode) return;
                        store.dispatchEvent(new CustomEvent('open-curriculum-switcher', { detail: { preferTab: 'version' } }));
                    }, 450);
                }
            } else {
                setTimeout(() => {
                    maybeNotifyRemoteVersionUpdate(store);
                }, 900);
            }
            /* Tree intro modal no longer auto-opens after load. The same content (description +
             * intro markdown) is the tree's own card in `tree-info`, reachable on-demand from the
             * map chrome ℹ️ and from each row in the tree picker. Auto-opening it on every load
             * felt intrusive and duplicated the description already visible elsewhere. */
        }
    },
    ...
    hydrateCompletionState(store, rootNode) {
        ...
        if (!(completedSet instanceof Set)) return;
        let hydratedCount = 0;
        ...
        const traverse = (node) => {
            ...
            if (completedSet.has(node.id)) {
                ...
                /* Also expand leafIds when children are only partially loaded. */
                if (node.leafIds && Array.isArray(node.leafIds)) {
