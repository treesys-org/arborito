    ...
    _debounceTimer = setTimeout(() => {
        ...
        const mobilePath =
            Array.isArray(fromStore) && fromStore.length > 0
                ? fromStore.map(String)
                : [String(root.id)];
        const sig = `${source.id}|${store.state.lang}|${mobilePath.join('>')}`;
        if (sig === _lastPersistSig) return;
        _lastPersistSig = sig;

        try {
            /* Path-only merge — no O(n) tree walk; keep prior expandedIds from last full persist. */
            const key = storageKey(source.id, store.state.lang);
            let expandedIds = [];
            try {
                const prev = JSON.parse(localStorage.getItem(key) || 'null');
                if (prev?.v === STORAGE_VERSION && Array.isArray(prev.expandedIds)) {
                    expandedIds = prev.expandedIds;
                }
            } catch {
                /* ignore */
            }
