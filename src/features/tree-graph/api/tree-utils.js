import { mergeSearchEntriesById } from '../../search/api/search-index-core.js';
            ...
            if (node.type === 'branch' || node.type === 'root') {
                 ...
                 if (total > 0 || node.type === 'branch') {
                     ...
                     if (completed.has(node.id)) {
                         ...
                         completedCount = leafIdsList.filter((id) => completed.has(id)).length;
                     }
                     const isComplete = completed.has(node.id) || (total > 0 && completedCount >= total);
                     if (node.type !== 'root') {
                        modules.push({
    ...
    async search(query, activeSource, lang, cache, getLocalOverlay) {
        ...
        return shard.filter((n) => {
            ...
            const hay = TreeUtils.cleanString(
                `${n.name || ''} ${n.description || ''} ${n.path || ''} ${n.searchBody || ''}`
            );
            if (hay.includes(cq)) return true;
            const tokens = cq.split(' ').filter((t) => t.length >= 2);
