...
/**
 * Per-username account record (sub-typed via `cid` tag):
 *   - `sync-login-v1`: hashed sync secret (login challenge).
 *   - `identity-v1`: signed username ↔ DID claim.
 */
export const KIND_USER_ACCOUNT_RECORD = 30241;

/** Forum v3 bucket storage (threads, pages, bans, pending). Replaced legacy
 * per-message / per-thread / per-mod events with a single replaceable blob. */
