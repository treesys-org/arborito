...
export function NostrRelayEditor({ ui, initialUrls, disabled = false, onSave, onRestoreSuggested }) {
    ...
    /* Empty list + Online: start from the recommended bundle so toggles match what Save will store. */
    useEffect(() => {
        if (disabled) return;
        if (normalizeNostrRelayUrls(initialUrls).length) return;
        setDraft((prev) => (prev.length ? prev : [...SUGGESTED_NOSTR_RELAYS]));
    }, [initialUrls, disabled]);
