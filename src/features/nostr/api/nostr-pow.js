...
export async function solveAppPow(kind, ownerPub, universeId, bucket, actorPub, bits) {
    ...
    for (let i = 0; i < maxIters; i++) {
        ...
        /* Yield often enough that optimistic UI (likes, toasts) stays responsive
         * on mobile while solving ~2^bits hashes. */
        if ((i & 0x7ff) === 0x7ff) await new Promise((r) => setTimeout(r, 0));
    }
