 ...
 *   - `arborito:account:network-pub:<user>`, username ↔ network user pubkey (for invites).
 */
...
/**
 * Account recovery blob: the sync secret encrypted under the user's recovery
 * answers (see `account-recovery.js`), stored per-username so a user who lost
 * their code can recover it by answering their questions. Replaceable.
 */
...
/**
 * Max UTF-8 bytes per NIP-44 encrypt slice for private-tree account sync.
 * NIP-44 caps plaintext at 65535 B; slice smaller so ciphertext fits relay limits.
 */
...
/** Replaceable `d` for a main JSON bundle chunk (index in header `chunkCount`). */
...
/** Replaceable `d` for public username → network user pubkey index (per-username). */
export function accountNetworkPubDTag(username) {
    return `arborito:account:network-pub:${String(username || '').trim().toLowerCase()}`;
}

/** Replaceable `d` for the per-username account recovery blob. */
...
/** Replaceable `d` for one encrypted slice of a chunked private tree (per-username + tree id + part index). */
export function privateTreePartDTag(username, treeId, partIndex) {
    const u = String(username || '').trim().toLowerCase();
    const id = String(treeId || '');
    const i = Math.max(0, Math.floor(Number(partIndex)) || 0);
    return `arborito:user:privtree:${u}:${id}:p:${i}`;
}
