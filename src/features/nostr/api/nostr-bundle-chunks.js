...
import { flattenTreeSearchEntriesWithLessonBody } from '../../search/api/search-index-core.js';

/** Messages per node under chunks.forum (avoids huge puts). */
const NOSTR_FORUM_MESSAGE_CHUNK = 200;
...
/** @param {string} nodeId */
function nostrMainLessonChunkKey(nodeId) {
    return `m__${safeKeyPart(nodeId)}`;
}

/** @param {string} snapshotId @param {string} nodeId */
function nostrSnapshotLessonChunkKey(snapshotId, nodeId) {
    return `s__${safeKeyPart(snapshotId)}__${safeKeyPart(nodeId)}`;
}

/** @param {string} snapshotId */
function nostrSnapshotGraphChunkKey(snapshotId) {
 ...
 */
function splitForumForNostrChunks(forumObj) {
 ...
 * }}
 */
export function prepareNostrSplitBundleV2(bundle, { includeForum = true } = {}) {
    ...
    delete slimBundle.forum;

    const forumSplit = includeForum
        ? splitForumForNostrChunks(bundle.forum && typeof bundle.forum === 'object' ? bundle.forum : {})
        : { meta: { messageParts: 0 }, threads: [], messageParts: [], moderationLog: [] };
