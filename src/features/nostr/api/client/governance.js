...
export const governanceMixin = {
    ...
    async putTreeLegalReport(opts) {
        ...
        const payload = {
            ...
            entityName: truncateUtf8(String(entityName || ''), 200),
            euAddress: truncateUtf8(String(euAddress || ''), 400),
            vatId: truncateUtf8(String(vatId || ''), 64),
            notifierName: truncateUtf8(String(notifierName || ''), 160),
            notifierEmail: truncateUtf8(String(notifierEmail || ''), 160),
            legalGround: truncateUtf8(String(legalGround || ''), 600),
            whereInTree: truncateUtf8(String(whereInTree || ''), 1500),
            whatWork: truncateUtf8(String(whatWork || ''), 1500),
            description: truncateUtf8(String(description || ''), 5000),
            declaration: !!declaration,
            links: safeLinks.map((u) => truncateUtf8(u, 400)),
            caseId: truncateUtf8(String(caseId || ''), 32),
            powBucket: bucket,
            powBits: pow.powBits,
    ...
    async putTreeDirectoryOwnerAppeal(opts) {
        ...
        const payload = {
            ...
            ownerPub: String(ownerPub),
            universeId: String(universeId),
            statement: truncateUtf8(String(statement || ''), 2000),
    ...
    async putTreeDelist({ pair, ownerPub, universeId, action = 'delist', reason = 'spam', note = '' }) {
        const payload = {
            ...
            ownerPub: String(ownerPub),
            universeId: String(universeId),
            action: truncateUtf8(String(action || 'delist'), 32),
            reason: truncateUtf8(String(reason || ''), 120),
            note: truncateUtf8(String(note || ''), 2000),
            at: new Date().toISOString()
        };
        const ev = await this._finalize(pair, {
            kind: KIND_APP_SIGNED_PAYLOAD,
