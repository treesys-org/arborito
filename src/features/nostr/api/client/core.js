...
import { QUERY_MS, assertNostrContentSize, isNostrContentTooLargeError, isNostrNetworkAvailable } from './_shared.js';
 ...
 *
 * Default (`firstHit: false`): wait for every peer or the deadline so the newest
 * replaceable can win (headers / escrow / share-codes).
 *
 * `firstHit: true`: return as soon as any relay yields an event — used for
 * auth existence / post-publish confirm so a dead peer cannot burn 6s+.
 *
 * @param {import('./index.js').NostrClient} pool
 * @param {string[]} relays
 * @param {import('nostr-tools').Filter} filter
 * @param {number} ms
 * @param {{ firstHit?: boolean }} [opts]
 * @returns {Promise<import('nostr-tools').Event|null>}
 */
function relayGetRace(pool, relays, filter, ms, opts = {}) {
    return new Promise((resolve) => {
        let best = null;
        let settled = 0;
        let done = false;
        const firstHit = !!opts.firstHit;
        ...
        const consider = (ev) => {
            ...
            if (firstHit && best) finish(best);
        };
        const step = () => {
            settled++;
            if (settled >= n) finish(best);
        };
...
export const coreMixin = {
    ...
    async _publishBurst(events, concurrency = 5) {
                    ...
                    try {
                        ...
                        if (this._unpauseAllRelaysIfAllCoolingDown()) {
                            try {
                                ...
                                if (isNostrContentTooLargeError(e2)) throw e2;
                                failed.push(batch[i]);
                                continue;
                            }
                        }
        ...
        for (let pass = 0; pass < 2 && pending.length; pass++) {
            ...
            if (pending.length && pass === 0) {
                this._unpauseAllRelaysIfAllCoolingDown();
            }
        }
        if (pending.length) {
            throw new Error(`Nostr burst incomplete: ${pending.length}/${list.length} events failed`);
        }
    },
    ...
    async _getFast(filter, ms = QUERY_MS, opts = {}) {
        this._assertNetworkConsent('get');
        const ordered = this._relaysFastFirst();
        if (!ordered.length) return null;
        await this._acquireQuerySlot();
        try {
            let out = await relayGetRace(this._pool, ordered, filter, ms, opts);
            if (!out && this._unpauseAllRelaysIfAllCoolingDown()) {
                out = await relayGetRace(this._pool, this._relays(), filter, ms, opts);
            }
            return out;
        } finally {
    ...
    async _get(filter, ms = QUERY_MS, opts = {}) {
        return this._getFast(filter, ms, opts);
    }
};
