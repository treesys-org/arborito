...
export const metricsMixin = {
    ...
    async putTreeVote({ pair, ownerPub, universeId, vote = true }) {
        ...
        if (!pow?.powNonce) {
            throw new Error('tree_vote_v1 PoW failed');
        }
        const payload = {
            kind: 'tree_vote_v1',
            ownerPub: String(ownerPub),
    ...
    async countTreeVotesOnce({ ownerPub, universeId, max = 2500 } = {}) {
         ...
         * bot replaying old versions could inflate the tally. One pubkey, one
         * vote. */
    ...
    async putTreeFork({ pair, parentOwnerPub, parentUniverseId, forkOwnerPub, forkUniverseId }) {
        ...
        const childPub = String(forkOwnerPub || '').trim();
        const childId = String(forkUniverseId || '').trim();
        if (!parentPub || !parentId || !childPub || !childId) return;
        if (!pair?.priv || String(pair.pub) !== childPub) return;
        const bucket = `fork:${childPub}/${childId}`;
        const pow = await this._solvePow(
            ...
            bucket,
            pair.pub,
            this._powBits('tree_fork_v1')
        );
        const payload = {
            kind: 'tree_fork_v1',
            parentOwnerPub: parentPub,
        ...
        const ev = await this._finalize(pair, {
            kind: KIND_APP_SIGNED_PAYLOAD,
            tags: [
                arbRootTag(parentPub, parentId),
                ['m', this.metricKindName('fork')],
                ['d', `fork:${parentPub}:${parentId}:${childPub}:${childId}`],
            ],
            content: JSON.stringify(payload),
        });
        await this._publish(ev);
    ...
    async verifyTreeFork(record) {
        ...
        if (!v || String(v.kind) !== 'tree_fork_v1') return false;
        const by = String(ev.pubkey || '');
        const parentPub = String(v.parentOwnerPub || record.parentOwnerPub || '');
        const parentId = String(v.parentUniverseId || record.parentUniverseId || '');
        const childPub = String(v.forkOwnerPub || record.forkOwnerPub || '');
        /* Signer must be the fork child owner — otherwise anyone can mint fork rows. */
        if (!childPub || by !== childPub) return false;
        return (
            parentPub &&
            parentId &&
            String(v.forkOwnerPub || '') === String(record.forkOwnerPub || v.forkOwnerPub) &&
            String(v.forkUniverseId || '') === String(record.forkUniverseId || v.forkUniverseId) &&
    ...
    async countTreeForksOnce({ ownerPub, universeId, max = 2500 } = {}) {
        ...
        for (const ev of broad) {
            ...
            const childKey = `${String(o?.forkOwnerPub || '')}/${String(o?.forkUniverseId || '')}`;
            if (childKey === '/' || byChild.has(childKey)) continue;
            if (
                await this.verifyTreeFork({
                    ...o,
                    parentOwnerPub: ownerPub,
                    parentUniverseId: universeId,
                    sig: ev,
                })
            ) {
                byChild.add(childKey);
                n += 1;
            }
        }
        return n;
    ...
    async putTreeReport({ pair, ownerPub, universeId, reason, note = '' }) {
        ...
        const payload = {
            ...
            note: truncateUtf8(String(note || ''), 4000),
            powBucket: bucket,
            powBits: pow.powBits,
            powNonce: pow.powNonce,
            at: new Date().toISOString()
    ...
    async listTreeReportsOnce({ ownerPub, universeId, max = 600 } = {}) {
        ...
        for (const ev of evs) {
            ...
            /* Attach `by` from the VERIFIED event pubkey: downstream unique-
             * reporter aggregation (`computeReportSignalsFromRows`) dedupes by
             * this field, and without it every report was silently discarded
             * (the directory hide-on-reports signal never fired). */
            if (await this.verifyTreeReport({ ...o, sig: ev })) out.push({ ...o, by: String(ev.pubkey || '') });
    ...
    async putTreeUrgentUserMessage({ pair, ownerPub, universeId, message, contactLine = '' }) {
        const bucket = 'urgent';
        const pow = await this._solvePow('tree_urgent_user_message_v1', ownerPub, universeId, bucket, pair.pub, this._powBits('tree_urgent_user_message_v1'));
        const payload = {
            kind: 'tree_urgent_user_message_v1',
            ownerPub: String(ownerPub),
            universeId: String(universeId),
            message: truncateUtf8(String(message || ''), 6000),
            contactLine: truncateUtf8(String(contactLine || ''), 400),
            powBucket: bucket,
            powBits: pow.powBits,
            powNonce: pow.powNonce,
            at: new Date().toISOString()
    ...
    async verifyTreeUrgentUserMessage(record) {
        ...
        if (!v || String(v.kind) !== 'tree_urgent_user_message_v1') return false;
        const by = String(ev.pubkey || '');
        return (
            String(v.ownerPub) === String(record.ownerPub || v.ownerPub) &&
            String(v.universeId) === String(record.universeId || v.universeId) &&
            (await this._verifyPow(
                'tree_urgent_user_message_v1',
                v.ownerPub,
                v.universeId,
                v.powBucket,
                by,
    ...
    async listTreeUrgentUserMessagesOnce({ ownerPub, universeId, max = 200 } = {}) {
        ...
        for (const ev of evs) {
            ...
            if (await this.verifyTreeUrgentUserMessage({ ...o, sig: ev })) out.push({ ...o, by: String(ev.pubkey || '') });
            if (out.length >= max) break;
        }
        return out;
    }
