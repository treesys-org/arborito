...
export const coreMixin = {
    ...
    _installRelayCircuitBreaker() {
        ...
        const COOLDOWN_LADDER_MS = [15_000, 60_000, 300_000, 600_000];
        ...
        this._pool.ensureRelay = async (url, params) => {
            ...
            if (h && h.until > now) {
                /* Short-circuit: skip the new WebSocket. Throwing here is the
                   ...
                   subscribeMany / publish just record "this relay didn't
    ...
    _unpauseAllRelaysIfAllCoolingDown() {
        ...
        const now = Date.now();
        const allPaused = relays.every((url) => {
            const h = this._relayHealth.get(url);
            return h && h.until > now;
        });
        if (!allPaused) return false;
        for (const url of relays) this._relayHealth.delete(url);
        console.warn('[arborito] every relay was cooling down — unpausing for one fresh attempt');
        return true;
    },
    ...
    async _query(filter, ms = QUERY_MS) {
        ...
        try {
            const out = await this._pool.querySync(relays, filter, { maxWait: ms });
            if ((!out || out.length === 0) && this._unpauseAllRelaysIfAllCoolingDown()) {
                return this._pool.querySync(this._relays(), filter, { maxWait: ms });
            }
            return out;
        } finally {
    ...
    async _get(filter, ms = QUERY_MS) {
        ...
        await this._acquireQuerySlot();
        try {
            const out = await this._pool.get(relays, filter, { maxWait: ms });
            if (!out && this._unpauseAllRelaysIfAllCoolingDown()) {
                return this._pool.get(this._relays(), filter, { maxWait: ms });
            }
            return out;
        } finally {
            this._releaseQuerySlot();
