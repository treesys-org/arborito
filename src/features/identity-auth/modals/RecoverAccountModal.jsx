...
export function ModalRecoverAccount() {
    ...
    const close = useCallback(() => {
        ...
        dismissModal();
    }, [dismissModal, m, setModal, returnToOnboarding]);
    ...
    const doSetup = useCallback(async () => {
        ...
        try {
            ...
            if (m.fromOnboarding) {
                /* OnboardingModal unmounts while this sheet is open — pass secretSaved on return. */
                window.dispatchEvent(new CustomEvent('arborito-onboarding-recovery-setup'));
                returnToOnboarding({ secretSaved: true });
                return;
            }
            close();
        } catch (e) {
    ...
    }, [passphrase, passphrase2, setAccountRecovery, close, ui, m.fromOnboarding, returnToOnboarding]);
