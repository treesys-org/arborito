 ...
 * Loads `./vendor/qrcode-davidshimjs.min.js` on first use.
 *
 * @param {string} text
 * @param {{ size?: number, dark?: string, light?: string }} [opts]
 * @returns {Promise<string>} data:image/png;base64,… or '' if unavailable
 */
let qrcodeVendorPromise = null;

function ensureQrcodeVendor() {
    if (typeof QRCode !== 'undefined') return Promise.resolve();
    if (qrcodeVendorPromise) return qrcodeVendorPromise;
    qrcodeVendorPromise = new Promise((resolve, reject) => {
        const el = document.createElement('script');
        el.src = './vendor/qrcode-davidshimjs.min.js';
        el.async = true;
        el.onload = () => resolve();
        el.onerror = () => reject(new Error('QRCode vendor failed to load'));
        document.head.appendChild(el);
    });
    return qrcodeVendorPromise;
}

export async function qrTextToDataUrl(text, opts = {}) {
    try {
        await ensureQrcodeVendor();
    } catch {
        return '';
    }
