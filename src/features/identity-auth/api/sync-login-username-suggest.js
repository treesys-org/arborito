...
export async function suggestUsernamesFor(base) {
    ...
    for (const n of SUGGESTION_OFFSETS) {
        ...
        try {
            const rec = nostr ? await nostr.loadSyncLoginRecordOnce(cand) : null;
            if (!rec || !rec.hash) candidates.push(cand);
        } catch {
            // Treat lookup errors as "probably free" — better than nothing.
...
export async function checkUsernameAvailability(name) {
    ...
    const rec = await nostr.loadSyncLoginRecordOnce(target);
    if (rec && rec.hash) {
