...
export function humanizeAuthError(err, ui = {}) {
    ...
    if (
        (err && typeof err === 'object' && err.code === 'nostr_relays_required') ||
        low.includes('nostr_relays_required') ||
        low.includes('relays required') ||
        low.includes('configure at least one relay')
    ) {
        return (
            ui.nostrRelaysRequired ||
            ui.syncLoginRelaysRequired ||
            'Enable the recommended network in Privacy & data (or save at least one relay) before registering.'
        );
    }
    if (low.includes('wrong username') || low.includes('wrong secret') || low.includes('incorrecto')) {
        return ui.syncLoginWrongSecret || 'Wrong username or password.';
    }
    if (low.includes('wrong passphrase') || (low.includes('passphrase') && low.includes('could not recover'))) {
        return ui.recoveryWrongPassphrase || 'Wrong recovery passphrase. Try again.';
    }
    if (low.includes('recovery blob') || low.includes('recovery data') || low.includes('recovery phrase')) {
        return ui.recoveryNotConfigured || 'No recovery passphrase set for that account.';
    }
