...
/** Flush open lesson + persist live draft before leaving for an archive/snapshot view. */
export async function flushBeforeLocalVersionSwitch() {
...
export function curriculumTreeDisplayName(ui = {}) {
    ...
    const lang =
        ...
        store.state?.lang;
    if (raw?.languages && lang && raw.languages[lang]?.name) {
        const langTitle = String(raw.languages[lang].name || '').trim();
...
export function resolveActiveSourceVersionLabel(ui = {}, state, opts = {}) {
    ...
    const demoLabel = () => t('releasesDemoShort', 'Demo');
    const localSnapLabel = (localId) => {
        if (isBundledDemoBranchId(localId) || isArboritoDemoTree(st)) {
            return demoLabel();
        }
...
export function applyLocalDraftSwitch() {
    ...
    const returnComposedId = String(activeSource?.returnComposedTreeId || '').trim();
    let branchId = '';
    if (url.startsWith('branch://')) {
        branchId = url.slice('branch://'.length).split('/')[0];
    } else if (activeSource?.type === 'composed-tree') {
        branchId = String(fileSystem.localGardenTreeId() || '').trim();
    }
    if (!branchId && !returnComposedId) return undefined;
    if (
        activeSource.type !== 'archive' &&
        activeSource.localArchiveReleaseId == null &&
    ...
    /* Archive opened from a composed tree — remount the playlist wrapper. */
    if (returnComposedId) {
        markPendingCurriculumSwitcher();
        return st.loadComposedTree?.(returnComposedId, true);
    }
    /* Composed trees: leave archive view of an embedded branch by remounting the tree. */
    if (activeSource?.type === 'composed-tree') {
        const treeId = fileSystem.composedTreeId();
        if (!treeId) return undefined;
        markPendingCurriculumSwitcher();
        return st.loadComposedTree?.(treeId, true);
    }
    const newSource = {
        ...activeSource,
        id: branchId,
    ...
    delete newSource.returnComposedTreeId;
    markPendingCurriculumSwitcher();
    return st.loadData(newSource, true);
}
