...
} from './global-directory-torrent-runtime.js';
import { hasGdprNetworkConsent } from '../../../shared/lib/connected-services/index.js';

export { usesGlobalDirectoryPointerForTorrent } from './global-directory-torrent-runtime.js';
 ...
 */
async function resolveGlobalDirectoryTorrentSpec(store) {
...
/**
 * @param {{ ownerPub?: string, universeId?: string, title?: string, shareCode?: string, updatedAt?: string, description?: string, authorName?: string }[]} nostrRows, rows from Nostr relays
...
/**
 * Normalises a torrent JSON entry into a directory row (same shape as Nostr rows).
 * @param {unknown} e
 * @returns {{ ownerPub: string, universeId: string, title: string, shareCode: string, updatedAt: string, description?: string, authorName?: string, titles?: Record<string, string> }|null}
 */
function normalizeTorrentEntry(e) {
    ...
    if (e.titles && typeof e.titles === 'object' && !Array.isArray(e.titles)) {
        const titles = Object.fromEntries(
            Object.entries(e.titles)
                .map(([k, v]) => [
                    String(k || '')
                        .trim()
                        .toUpperCase(),
    ...
    /* Languages: optional. Mirror the wire format (uppercase ISO codes) and cap to keep the
     * torrent payload bounded. Empty arrays collapse to "field absent" so older entries don't
     * appear different on the wire vs. equivalent newer ones. */
    ...
    const icon = String(e.icon || '').trim().slice(0, 32);
    if (icon) out.icon = icon;
    const contentKind = String(e.contentKind || '').trim();
...
export async function loadGlobalDirectoryRowsFromHttp(opts = {}) {
    ...
    if (!url) return [];
    /* GDPR: the operator can point `window.ARBORITO_GLOBAL_DIRECTORY_JSON_URL`
     * at a cross-origin host, which would leak the visitor's IP / UA to a
     * third party. Without explicit consent we silently skip, callers already
     * fall back to the Nostr-only path. */
    let crossOrigin = false;
    try {
        const u = new URL(url, typeof window !== 'undefined' ? window.location.href : 'http://localhost/');
        crossOrigin = typeof window !== 'undefined' && u.origin !== window.location.origin;
    } catch {
        /* relative URL, treat as same-origin */
    }
    if (crossOrigin && !hasGdprNetworkConsent()) return [];
