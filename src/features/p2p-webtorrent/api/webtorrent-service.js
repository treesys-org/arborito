...
export class WebTorrentService {
    ...
    /**
     * Reflects "can this device join the WebTorrent swarm RIGHT NOW?".
     * Two AND-ed conditions:
     *   1. The vendor script was loaded (deferred to idle in `main.js`).
     *   2. The user has accepted the GDPR privacy policy. WebTorrent
     *      connects to trackers + DHT + remote peers, all of which see
     *      the user's IP, exactly the kind of disclosure GDPR requires
     *      an opt-in for.
     * Callers throughout the code already pattern-match `available()` and
     * fall back to non-torrent paths when it's `false`, so flipping this
     * off when consent is missing degrades cleanly.
     */
    available() {
        return !!globalThis.WebTorrent && hasGdprNetworkConsent();
    }
