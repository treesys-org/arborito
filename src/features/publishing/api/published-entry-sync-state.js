...
export function getBranchSyncState(branch, { getNostrPublisherPair } = {}) {
    ...
    const dataNodes = Array.isArray(data?.nodes) ? data.nodes.length : 0;
    const snapNodes = Array.isArray(snap?.nodes) ? snap.nodes.length : 0;

    if (!snapHash || (dataNodes === 0 && snapNodes > 0)) {
        return { mode: 'repair', isOwner: true, canAct: true };
    }
