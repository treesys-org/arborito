...
export async function autoMaintainPublishedComposedTree(store, treeId) {
    ...
    if (state.mode === 'repair' || state.mode === 'update') {
        await store.publishComposedTreeToNostr?.({ treeId: id });
        return true;
    }
    return false;
}
...
/**
 * Repair a published branch (local curriculum). Network sync runs when the branch is active.
...
export async function autoMaintainPublishedBranch(store, branchId) {
    ...
    if (state.mode === 'upToDate') return false;
    ...
    const onBranch =
        active &&
        (String(active.id) === id || String(active.url || '') === `branch://${id}`);
    if ((state.mode === 'repair' || state.mode === 'update') && onBranch && store.state.rawGraphData) {
        const raw = store.state.rawGraphData;
        const url = String(branch.publishedNetworkUrl || '').trim();
        if (url) {
            await store.publishActiveTreeToNostrUniverse?.({
                reuseNostrTreeUrl: url,
                includeForum: !!raw.meta?.forumEnabled,
                listInDiscover: raw.meta?.listInDiscover !== false,
            });
        }
        return true;
    }
