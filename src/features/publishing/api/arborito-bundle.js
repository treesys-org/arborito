 ...
 */
/**
 * Curriculum-only snapshot for `.arborito` export: map + lessons only.
 * Forum is stored separately (`ForumStore`); the search index is rebuilt from
 * the tree after import; optional `versions/*.arborito` nests saved snapshots
 * when exporting with scope “all”.
 * @param {object} treeData
 * @returns {object}
 */
export function sanitizeCurriculumForArboritoArchive(treeData) {
...
function collectNodeIdsFromRawTree(rawGraphData) {
    const ids = new Set();
    const walk = (node) => {
 ...
 * @returns {object}
 */
function filterProgressForTree(persistence, allowedIds) {
...
export function mergeBundleMetaIntoTree(tree, meta) {
    ...
    const next = { ...cur };
    /* Plain text fields. */
    for (const k of ['description', 'authorName', 'authorAbout', 'license', 'licenseUrl']) {
        if (meta[k] != null && String(meta[k]).length > 0 && (next[k] == null || String(next[k]).length === 0)) {
            next[k] = String(meta[k]);
        }
    }
    ...
    if (meta.attribution && typeof meta.attribution === 'object') {
        applyAttributionToTreeData(tree, meta.attribution);
    } else {
        applyAttributionToTreeData(tree, attributionFromPresentation(meta, { contentKind: 'branch' }));
    }
    if (meta.forumEnabled === false || meta.forumEnabled === true) {
 ...
 * @param {Record<string, string>|null} [opts.collaboratorRoles]
 */
export function buildArboritoBundle({
    ...
    const up =
        (rawGraphData && rawGraphData.universePresentation) && typeof rawGraphData.universePresentation === 'object'
            ...
            : {};
    let collaborators = Array.isArray(up.collaborators) ? [...up.collaborators] : [];
    if (!collaborators.length && collaboratorRoles && typeof collaboratorRoles === 'object') {
        collaborators = Object.entries(collaboratorRoles)
            .filter(([, role]) => role === 'editor' || role === 'proposer')
            .map(([pub, role]) => ({ pub: String(pub), role: String(role) }));
    }
    const attribution = attributionFromPresentation(up, {
    ...
    return {
        ...
        meta: {
            ...
            supportUrl: typeof up.supportUrl === 'string' ? up.supportUrl : '',
            icon:
                (typeof up.icon === 'string' && up.icon.trim()) ||
                (typeof rawGraphData?.icon === 'string' && rawGraphData.icon.trim()) ||
                '',
            license: typeof up.license === 'string' ? up.license : 'CC-BY-SA-4.0',
            licenseUrl: typeof up.licenseUrl === 'string' ? up.licenseUrl : 'https://creativecommons.org/licenses/by-sa/4.0/',
            attribution,
        },
 ...
 */
function parseArboritoBundle(data) {
