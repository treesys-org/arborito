...
class ArboritoModalExportPdf extends HTMLElement {
    ...
    render() {
        ...
        if (this.state.isGeneratingPdf) {
            innerContent = `
             <div class="p-12 flex flex-col items-center justify-center text-center">
                 ...
                 <p class="text-xs text-slate-400 mt-2">${ui.exportPdfProcessing || 'Processing lessons...'}</p>
             </div>`;
        } else {
            innerContent = `
        ...
        const body = `
                ${modalHeroHtml(ui, { title: ui.exportTitle, tagClass: 'btn-close' })}
                <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar">${innerContent}</div>`;
        this.innerHTML = modalShellHtml({
            bodyHtml: body,
            panelSize: 'auto-h',
            panelClass: 'arborito-float-modal-card--lg transition-all duration-300',
        });
