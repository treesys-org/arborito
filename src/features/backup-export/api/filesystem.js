...
class FileSystemService {
    ...
    activeComposedBranchId() {
        ...
        const ctx = getArboritoStore().state.treeContext;
        ...
        const entry = getArboritoStore().userStore.getTree(this.composedTreeId());
    ...
    _reloadAfterBranchWrite(branchId) {
        if (this.isLocalComposedTree()) {
            const composedId = this.composedTreeId();
            if (composedId) void getArboritoStore().loadComposedTree(composedId);
            return;
        }
        const updatedSource = getArboritoStore().userStore.getBranchData(branchId);
        getArboritoStore().processLoadedData(updatedSource);
    }
    ...
    async saveFile(node, content, meta, commitMsg) {
        if (this.isLocalBranch()) {
            const treeId = this.localGardenTreeId();
            const success = getArboritoStore().userStore.updateBranchNode(treeId, node.id, content, meta);

            if (success) {
                this._reloadAfterBranchWrite(treeId);
        ...
        if (this.isLocalComposedTree()) {
            ...
            if (success) {
                this._reloadAfterBranchWrite(branchId);
                return { success: true, mode: 'instant' };
            }
            throw new Error(getArboritoStore().ui.saveFailedLocal || 'Local save failed. Node not found.');
        }
    ...
    async createNode(parentPath, name, type, explicitParentId = null) {
        const nativeParentId = explicitParentId ? this.resolveNativeNodeId(explicitParentId) : null;
        ...
        if (this.isLocalBranch() || this.isLocalComposedTree()) {
            ...
            if (treeEntry) {
                ...
                if (explicitParentId) {
                    for (const langKey in treeEntry.data.languages) {
                        parentNode = findNodeById(treeEntry.data.languages[langKey], nativeParentId);
                        if (parentNode) break;
                    }
                }
                ...
                if (parentNode) {
                    ...
                    getArboritoStore().userStore.createBranchNode(treeId, parentPath, name, type, parentNode.id);

                    const updatedSource = getArboritoStore().userStore.getBranchData(treeId);
            ...
            getArboritoStore().userStore.createBranchNode(treeId, parentPath, name, type, nativeParentId || null);
            this._reloadAfterBranchWrite(treeId);
            return true;
        }
    ...
    async deleteNode(path, type) {
        if (this.isLocalBranch() || this.isLocalComposedTree()) {
            ...
            getArboritoStore().userStore.deleteBranchNodeByPath(treeId, path);
            this._reloadAfterBranchWrite(treeId);
            return true;
        }
        if (this.isNostrTreeSource()) {
            const ok = getArboritoStore().nostrDeleteNodeByPath(path);
            if (ok) return true;
            throw new Error(
                (getArboritoStore().ui.graphErrorWithMessage || 'Error: {message}').replace('{message}', 'Could not delete this node.')
            );
        }
        throw new Error(getArboritoStore().ui.treeReadOnlyHint || 'Read-only tree.');
    }
    ...
    async moveNode(oldPath, newParentPath) {
        ...
        if (this.isNostrTreeSource()) {
            throw new Error(getArboritoStore().ui.nostrStructureEditHint || 'Move nodes in a local garden, then re-publish.');
        }
        throw new Error(getArboritoStore().ui.treeReadOnlyHint || 'Read-only tree.');
    }
