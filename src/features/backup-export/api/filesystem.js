...
class FileSystemService {
    ...
    _reloadAfterBranchWrite(branchId) {
        if (this.isLocalComposedTree()) {
            ...
            if (composedId) void getArboritoStore().loadComposedTree(composedId, false);
            return;
        }
    ...
    async saveFile(node, content, meta, commitMsg, opts = {}) {
        ...
        if (this.isLocalBranch()) {
            ...
            if (success) {
                if (!skipGraphReload) this._reloadAfterBranchWrite(treeId);
        ...
        if (this.isLocalComposedTree()) {
            ...
            if (success) {
                if (!skipGraphReload) this._reloadAfterBranchWrite(branchId);
                return { success: true, mode: 'instant' };
            }
            throw new Error(getArboritoStore().ui.saveFailedLocal || 'Local save failed. Node not found.');
        }
    ...
    async createNode(parentPath, name, type, explicitParentId = null) {
        ...
        if (this.isLocalBranch() || this.isLocalComposedTree()) {
            ...
            if (treeEntry) {
                ...
                if (explicitParentId && langs && typeof langs === 'object') {
                ...
                if (parentNode) {
                    ...
                    const created = getArboritoStore().userStore.createBranchNode(
                        treeId,
                        parentPath,
                        name,
                        type,
