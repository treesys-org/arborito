...
/* Invisible prefix used to neutralise an `@quiz` / `@/quiz` line that the
   teacher typed by mistake in a regular paragraph. The character renders as
   nothing in the editor, so the teacher still sees `@quiz`, but the markdown
   fence regex no longer matches because the line no longer starts with `@`. */
const QUIZ_TEXT_GUARD = '\u200B';

/* `escAttr` is used everywhere here because some templates inject the result
 * inside `'...'` attribute values; it is a strict superset of `escHtml`. */
...
/** Authoring mode: the outline only lives in the syllabus; in the body these are discreet markers. */
...
/**
 * Read the optional leading `@info … @/info` block plus an optional first
 * `@quiz` fence (used as the lesson's static challenge schema), and return
 * `{ meta, body }`. The rest of the file is preserved verbatim as `body` so
 * any further `@quiz` blocks, headings, or paragraphs keep their original
 * positions.
 */
export function parseArboritoFile(content) {
    ...
    if (i < lines.length && lines[i].trim() === '@info') {
        ...
        if (i < lines.length) i++; // consume @/info
    ...
    /* Capture the first `@quiz` fence so the editor can edit it as a static
       challenge. Subsequent `@quiz` blocks stay in the body for the renderer. */
...
export function markdownToVisualHTML(md, opts = {}) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Quiz V2 fenced block: @quiz ... @/quiz
        if (isQuizBlockOpen(line)) {
            ...
            html += BLOCKS.quizv2(parseQuizBlock(body));
...
export function visualHTMLToMarkdown(rootElement) {
    ...
    for (const node of rootElement.childNodes) {
        ...
        if (node.tagName === 'P') { 
            ...
            /* If the teacher typed `@quiz` / `@/quiz` as plain prose, neutralise
               the fence by prefixing an invisible zero-width-space so the parser
               does not mistake it for the start/end of a quiz block. The user
               still sees `@quiz` in the editor — the prefix renders as nothing. */
            text = text.replace(/^@(\/?)quiz(\s*)$/gim, `${QUIZ_TEXT_GUARD}@$1quiz$2`);
...
/**
 * Rebuild a lesson .md from its in-memory pieces. Renders one optional
 * `@info … @/info` block at the top (skipped entirely when no field is set),
 * then the static challenge fence (when present), then the body.
 */
export function reconstructArboritoFile(meta, bodyMD) {
    const fields = [];
    /* `title:` is only persisted when the desired display has chars the
       filesystem cannot keep — otherwise the filename IS the display name. */
