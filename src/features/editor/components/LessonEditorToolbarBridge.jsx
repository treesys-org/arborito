...
function LessonEditorFormatMenu({ ui, onToolCmd, toolbarHandlers }) {
    ...
    const runCmd = (cmd, val) => (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        /* Pin already set on Aa open / option pointerdown — re-stash here
           would overwrite a good range with a collapsed caret. */
        onToolCmd?.(cmd, val);
        close();
    };

    /* Mouse preventDefault keeps selection; touch must not cancel the click. */
    const keepEditorSel = {
        onMouseDown: (e) => {
            e.preventDefault();
            stashSel();
...
function LessonEditorFormatRow({ ui, undoButtonId, includeHeadingStructure, toolbarHandlers, showInlineLists }) {
    ...
    const redoLabel = ui.editorToolbarRedo || ui.conRedoDockLabel || 'Redo';
    const { onUndo, onRedo, onToolCmd, undoDisabled, redoDisabled } = toolbarHandlers || {};
    ...
    const stashSel = () => toolbarHandlers?.stashFormatSelection?.();
    /* Mouse preventDefault keeps selection; touch stash only — preventDefault kills click. */
    const keepEditorSel = {
        onMouseDown: (e) => {
            e.preventDefault();
            stashSel();
    ...
    return (
        <>
            ...
            <ToolbarTipButton
                ...
                label={ui.editorToolbarBold || 'Bold'}
                data-cmd="bold"
                onMouseDown={keepEditorSel.onMouseDown}
                onPointerDown={keepEditorSel.onPointerDown}
                onTouchStart={keepEditorSel.onTouchStart}
                onClick={cmd('bold')}
            >
                B
            </ToolbarTipButton>
            <ToolbarTipButton
                ...
                label={ui.editorToolbarItalic || 'Italic'}
                data-cmd="italic"
                onMouseDown={keepEditorSel.onMouseDown}
                onPointerDown={keepEditorSel.onPointerDown}
                onTouchStart={keepEditorSel.onTouchStart}
...
function LessonEditorQuizShortcut({ ui, layout }) {
    ...
    const quizBtnClass = mobileKeyActions
        ...
        : 'lesson-editor-toolbar-quiz lesson-editor-tool-chip lesson-editor-toolbar-quiz--featured';

    return (
        <div className="lesson-editor-quiz-wrap" data-arbor-tour="lesson-edit-quiz">
            <ToolbarTipButton
                id="btn-insert-quiz"
            ...
            >
                {mobileKeyActions ? (
                    <ChromeEmoji emoji={quizEmoji} size={18} />
                ) : (
                    <>
                        <ChromeEmoji emoji={quizEmoji} size={20} className="lesson-editor-toolbar-quiz__emoji" />
                        <span className="lesson-editor-toolbar-quiz__label">{quizLabel}</span>
                    </>
                )}
            </ToolbarTipButton>
        </div>
...
function LessonEditorInsertBlock({ ui, layout }) {
    ...
    const blockOpts = [
        ...
        ['table', '📊', ui.editorBlockTable || 'Table'],
        ['video', '🎬', vidLabel],
    ...
    return (
        <div
            className={`lesson-editor-insert-wrap${mobileKeyActions ? ' lesson-editor-insert-wrap--solo' : ''}`}
    ...
    if (layout === 'mobile-split') {
        return (
            <div className="arborito-lesson-toolbar-clusters" role="group" aria-label={toolbarLabel}>
                ...
                {includeQuizShortcut ? (
                    <div className="arborito-lesson-toolbar-cluster arborito-lesson-toolbar-cluster--quiz">
                        <LessonEditorQuizShortcut ui={ui} layout={layout} />
                    </div>
                ) : null}
    ...
    return (
        <div className="lesson-editor-toolbar-rows" role="group" aria-label={toolbarLabel}>
            <div className="lesson-editor-toolbar-row lesson-editor-toolbar-row--primary">
                ...
                />
                {includeQuizShortcut ? (
                    <div className="lesson-editor-toolbar-group lesson-editor-toolbar-group--content">
                        <LessonEditorQuizShortcut ui={ui} layout={layout} />
                        <LessonEditorInsertBlock ui={ui} layout={layout} />
                    </div>
                ) : (
