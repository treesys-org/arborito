 ...
 * @param {{ includeQuizShortcut?: boolean, undoButtonId?: string, includeHeadingStructure?: boolean }} [opts]
 */
export function getEditorToolbarInnerHtml(ui, opts = {}) {
    const includeHeadingStructure = opts.includeHeadingStructure !== false;
    const includeQuizShortcut = opts.includeQuizShortcut !== false;
    const undoId = opts.undoButtonId || 'btn-undo';
    const undoAria = escToolbarAttr(ui.conUndoDockLabel || ui.editorToolbarUndo || 'Undo');
    const quizLabel = ui.editorToolbarQuizShortcut || ui.lessonQuizV2Label || 'Quiz';
    const quizHint = ui.editorToolbarQuizShortcutHint || quizLabel;
    const quizBtn = includeQuizShortcut
        ? `<button type="button" id="btn-insert-quizv2" data-arbor-tour="lesson-edit-quiz" class="lesson-editor-toolbar-quiz shrink-0 min-h-[2.75rem] sm:min-h-0 px-2.5 py-2 sm:px-3 sm:py-1.5 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/30 rounded-lg text-[10px] sm:text-xs font-bold uppercase hover:bg-indigo-500/20 inline-flex items-center justify-center gap-1 transition-colors" title="${escToolbarAttr(quizHint)}" aria-label="${escToolbarAttr(quizLabel)}">
                    <span aria-hidden="true">📋</span><span class="inline ml-0.5">${escToolbarAttr(quizLabel)}</span>
                </button>`
        : '';
                    ...
                    <div class="lesson-editor-format-wrap relative shrink-0 self-center">
                        ...
                        <div id="lesson-editor-format-panel" class="lesson-editor-format-panel hidden absolute left-0 top-[calc(100%+0.35rem)] z-[90] flex flex-col gap-2 min-w-[12rem] max-w-[min(18rem,calc(100vw-2rem))] p-2 rounded-xl border border-slate-200/80 dark:border-slate-700/70 bg-white dark:bg-slate-900 shadow-xl" role="menu" aria-label="${fmtAria}">
                            <div class="px-1 pt-0.5">
                                <p class="arborito-eyebrow m-0">${escToolbarAttr(sizeLabel)}</p>
                            ...
                            <div class="px-1 pb-0.5">
                                <p class="arborito-eyebrow m-0">${escToolbarAttr(alignLabel)}</p>
    ...
    return `
            <div class="lesson-editor-toolbar-rows flex flex-col gap-1 min-w-0 flex-1" role="group" aria-label="${escToolbarAttr(ui.lessonToolbar || 'Lesson editor')}">
                <div class="lesson-editor-toolbar-row lesson-editor-toolbar-row--primary flex flex-nowrap items-stretch sm:items-center gap-1 min-w-0 overflow-x-auto pb-0.5 -mx-0.5 px-0.5 sm:overflow-visible sm:pb-0 sm:mx-0 sm:px-0 sm:flex-wrap">${rowFmt}
                    <div class="lesson-editor-insert-wrap relative shrink-0 self-center" data-arbor-tour="lesson-edit-insert">
                        <button type="button" class="lesson-editor-insert-toggle lesson-editor-tool-chip min-h-[2.25rem] px-1.5 rounded-lg border border-amber-400/55 dark:border-amber-600/45 bg-amber-100/50 dark:bg-amber-950/35 text-[9px] sm:text-xs font-black uppercase tracking-wide text-amber-900 dark:text-amber-100 hover:bg-amber-200/60 dark:hover:bg-amber-900/40 transition-colors whitespace-nowrap" aria-expanded="false" aria-haspopup="true" aria-controls="lesson-editor-insert-panel" title="${insertMenuAria}"><span aria-hidden="true" class="sm:hidden">${insertMenuLabel}</span><span class="hidden sm:inline">${insertMenuLabel}</span> <span aria-hidden="true" class="opacity-70">▾</span></button>
                    ...
                    <span class="lesson-editor-toolbar-spacer flex flex-1 min-w-[0.1rem] sm:min-w-[0.25rem]" aria-hidden="true"></span>
                    ${quizBtn}
                </div>
            </div>`;
}
