...
import { extractTocSectionMarkdown } from '../../../learning/api/lesson-section-slices.js';
import { markdownToVisualHTML, replaceEditorHtml, parseArboritoFile } from '../editor-engine.js';
import { visualHTMLToMarkdown } from './editor-serialize.js';
import { isDraftBodyUsable } from './lesson-draft-persist.js';
import { getArboritoStore as store } from '../../../../core/store-singleton.js';

const MISPLACED_BLOCK_SEL =
    '.edit-block-wrapper, .arborito-quiz-edit, .arborito-game-edit, .arborito-media-edit, .arborito-callout-edit, .arborito-math-edit';
...
/** Section markdown for the visual construct editor. */
export function buildConstructEditorSeed(node, panel, sectionIndex) {
    if (!node) return { sectionMd: '' };
    const liveContent = store.findNode(node.id)?.content || node.content || '';
    const parsed = parseArboritoFile(liveContent);
    const usingDraft =
        panel.lessonConstructDraft &&
        panel.lessonDraftLessonId === node.id &&
        panel.lessonBodyMarkdown !== null &&
        isDraftBodyUsable(panel.lessonBodyMarkdown, liveContent);
    const fullBodyMd = usingDraft ? panel.lessonBodyMarkdown : parsed.body;
