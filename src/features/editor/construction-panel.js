...
import { iconLanguageSvg } from '../shell-chrome/sidebar-utils.js';
...
class ArboritoConstructionPanel extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const repaintIfActive = () => {
            if (store.value.constructionMode) this._repaint();
            else this.lastRenderKey = null;
        };
        this._undoHistoryListener = repaintIfActive;
        store.addEventListener('construction-undo-changed', this._undoHistoryListener);
        this._viewportListener = repaintIfActive;
    ...
    disconnectedCallback() {
        ...
        if (this._undoHistoryListener) {
            store.removeEventListener('construction-undo-changed', this._undoHistoryListener);
        }
    ...
    /** Force a repaint past the cheap render-key cache. Used by every mutation
     * that flips a local UI flag (`moreToolsOpen`, `publishingPublic`, …)
     * which is not part of the global store and therefore wouldn't be
     * picked up by the render-key fingerprint on its own. */
    _repaint() {
        this.lastRenderKey = null;
        this.updateView();
    }
    ...
    updateView() {
        ...
        const canRedoCon =
            typeof store.getConstructionRedoDepth === 'function' ? store.getConstructionRedoDepth() > 0 : false;
        const historyHasSteps = canUndoCon || canRedoCon;
        ...
        const redoL = ui.conRedoTooltip || ui.conRedoAria || 'Redo last map edit';
        const redoShort = ui.conRedoDockLabel || 'Redo';
        ...
        const curriculumTabBtn =
            showCurriculumTools ?
                `<button type="button" id="btn-cp-curriculum" data-arbor-tour="con-lang" class="cp-dock-tab${constructionLangModalOpen ? ' cp-dock-tab--accent' : ''}" title="${escHtml(langDockL)}" aria-label="${escHtml(langDockL)}" aria-expanded="${constructionLangModalOpen}" aria-haspopup="dialog">
        ...
        if (isContributor && !useCompactDock) {
            ...
            contributorTabs.push(`
                <button type="button" id="btn-construction-history" data-arbor-tour="con-undo" class="cp-dock-tab" title="${escHtml(undoL)}" aria-label="${escHtml(undoL)}">
        ...
        if (useCompactDock && isContributor) {
            ...
            <div class="cp-construct-wrap" role="region" aria-label="${escHtml(ui.constructionDockAriaFallback || constructTitle)}">
                ...
                <nav class="cp-construct-mob-dock" aria-label="${escHtml(ui.constructionDockAriaFallback || 'Tools')}">
                    <button type="button" id="btn-back-construct" data-arbor-tour="con-exit" class="arborito-mob-tab" title="${escHtml(exitConstructL)}" aria-label="${escHtml(exitConstructL)}">
                    ...
                    <button type="button" id="btn-construction-history" data-arbor-tour="con-undo" class="arborito-mob-tab cp-construct-mob-tab--undo" title="${escHtml(undoL)}" aria-label="${escHtml(undoL)}">
                        <span class="arborito-mob-tab__icon" aria-hidden="true">🕒</span>
                        <span class="arborito-mob-tab__label">${escHtml(shortDockLabel(undoShort))}</span>
                    </button>
                    <button type="button" id="btn-construction-sage" data-arbor-tour="con-ai" class="arborito-mob-tab" title="${escHtml(ui.navSage || 'Sage')}" aria-label="${escHtml(ui.navSage || 'Sage')}">
                        <span class="arborito-mob-tab__icon" aria-hidden="true">🦉</span>
                        <span class="arborito-mob-tab__label">${escHtml(shortDockLabel(ui.navSageDock || ui.navSage || 'Sage'))}</span>
                    </button>
                    ${pubDockBtn}
            ...
            const backBtn = dockExitlessDesktop
                ? ''
                : `<button type="button" id="btn-back-construct" data-arbor-tour="con-exit" class="cp-dock-tab cp-dock-tab--edge" title="${escHtml(exitConstructL)}" aria-label="${escHtml(exitConstructL)}">
        ...
        const openHistory = () => {
            this.moreToolsOpen = false;
            store.setModal({ type: 'construction-history', dockUi: true });
            this._repaint();
        };
        bind(dockMain, '#btn-construction-history', openHistory);
        bind(dockMain, '#btn-construction-sage', () => {
            this.moreToolsOpen = false;
            const lessonOpen = !!(store.value.selectedNode || store.value.previewNode);
            const node = store.value.selectedNode || store.value.previewNode;
            const inLesson =
                lessonOpen && node && (node.type === 'leaf' || node.type === 'exam');
            store.setModal({
                type: 'sage',
                mode: 'context',
        ...
        bind(dockMain, '#cp-more-close', closeMoreAndRepaint);
        bind(dockMain, '#cp-construct-more-backdrop', closeMoreAndRepaint);
