...
export function isLessonEditTourContextReady() {
    ...
    const host = getPanelRef('content');
    if (!host || !host.currentNode) return false;
    return !!(
        host.querySelector('[data-tour-id="lesson-edit-meta"]') ||
        host.querySelector('[data-arbor-tour="lesson-edit-meta"]')
    );
}
