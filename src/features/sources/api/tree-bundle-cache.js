...
function canonicalTreeCacheUrl(urlStr) {
...
/**
 * Lookup cached bundle by canonical tree URL (e.g. when sourceId differs across devices).
 * @param {string} url
 */
export async function getTreeBundleCacheByUrl(url) {
    const want = canonicalTreeCacheUrl(url);
    if (!want || typeof indexedDB === 'undefined') return null;
    try {
        const db = await openDb();
        try {
            ...
            for (const row of rows) {
                ...
                if (canonicalTreeCacheUrl(row.url) === want) return row;
            }
            return null;
        } finally {
            db.close();
        ...
        console.warn('[Arborito] tree bundle cache url lookup failed', e);
        return null;
    }
}
