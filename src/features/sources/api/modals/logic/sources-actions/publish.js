...
import { pinVoteCountAfterToggle } from '../sources-vote-persist.js';
...
function applyVoteMetricsDelta(ctx, metricKey, delta) {
    ...
    if (typeof setMetrics !== 'function') {
        console.warn('[Arborito] vote metrics setter missing');
        return;
    }
    try {
        setMetrics((prevMetrics) => {
            const prev = prevMetrics && typeof prevMetrics === 'object' ? prevMetrics : {};
            const cur = prev[metricKey] || {};
            const base = Number.isFinite(Number(cur.votes)) ? Number(cur.votes) : 0;
            return {
                ...prev,
        ...
        console.warn('[Arborito] vote metrics update failed', e);
    }
}
...
export async function runPublishAction(ctx, action, fields = {}) {
    ...
    if (action === 'global-vote') {
        ...
        /* Prefer explicit liked from the optimistic UI toggle (avoids double-tap races). */
        const finalVote =
        ...
        if (typeof fields.votes === 'number') {
            pinVoteCountAfterToggle(ownerPub, universeId, fields.votes);
        } else {
            const curBase = Number(ctx?.directoryState?.()?.globalDirMetrics?.[metricKey]?.votes) || 0;
            pinVoteCountAfterToggle(ownerPub, universeId, Math.max(0, curBase + delta));
        }
        /* Do not rerank on like — reordering remounts rows and fights the optimistic UI. */
        try {
            ctx?.bump?.();
        } catch {
            /* modal unmounted */
