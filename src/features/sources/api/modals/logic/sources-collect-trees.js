...
import { resolveBranchRefDisplayNames } from '../../../../trees/api/tree-branch-labels.js';
...
import { getRowMetricsFromMap } from './sources-directory-row-state.js';
 ...
 */
export function collectTreesTabItems(ctx, ui, state, activeSource, { scope, q }) {
    ...
    if (sc === 'all' || sc === 'internet') {
        ...
        for (let ri = 0; ri < rows.length; ri++) {
            ...
            const s = scoreSourcesMatch(q2, r?.title, r?.shareCode, r?.ownerPub, r?.description, r?.authorName);
            if (q2 && s <= 0) continue;
            const hash = String(r?.branchSetHash || '').trim();
            const m = getRowMetricsFromMap(r, metricsMap);
            const score =
                25 + s + (Number(m?.votes) || 0) * 0.5 + (Number(m?.used7) || 0) * 0.3 - ri * 0.01;
