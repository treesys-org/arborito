...
import {
    ...
    buildNetworkSourceExportArchiveBytes,
...
export { shareTreeLink, shareComposedTree } from '../../share-tree-link.js';
...
async function importBranchFromAnalysis(modal, analysis) {
    ...
    try {
        ...
        if (archive.media && newTree?.id && !reused) {
            try {
                await importLessonMediaFiles(newTree.id, archive.media);
            } catch (e) {
                console.warn('[Arborito] import local media skipped', e);
            }
        }
        const source = {
            id: newTree.id,
...
export async function exportNetworkSource(id, name, { lang = '*', scope = 'current' } = {}) {
    const archiveBytes = await buildNetworkSourceExportArchiveBytes(store, {
        sourceId: id,
        lang,
        scope,
    });
    if (!archiveBytes) {
        const ui = store.ui || {};
        throw new Error(ui.sourcesExportFailed || 'Export failed.');
    }
    const label = String(name || 'network-branch').trim() || 'network-branch';
    await downloadArboritoBlob(archiveBytes, `arborito-branch-${safeFileName(label)}.arborito`);
}
