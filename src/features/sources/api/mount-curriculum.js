...
export async function mountCurriculum(store, source, forceRefresh = true) {
    ...
    try {
        ...
        if (nextUrl.startsWith('tree://')) {
            const { mountComposedTree } = await import('../../trees/api/mount-composed-tree.js');
        ...
        if (nextUrl.startsWith('branch://')) {
            ...
            if (!graphJson) {
                const cached = sourceId ? await getTreeBundleCache(sourceId) : null;
        ...
        const { parseArboritoTreeBundle } = await import('../../trees/api/arborito-tree-bundle.js');
        if (graphJson && parseArboritoTreeBundle(graphJson)) {
            const { parseNostrTreeUrl } = await import('../../nostr/api/nostr-refs.js');
            const treeRef = parseNostrTreeUrl(String((finalSource && finalSource.url) || source.url || ''));
            const { importComposedTreeFromBundle } = await import('../../trees/api/import-composed-tree-bundle.js');
