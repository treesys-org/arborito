...
const CURRICULUM_LOCALE_BY_CODE = Object.fromEntries(
...
/** Preset metadata for a curriculum language code (flag, label, …). */
export function getCurriculumLangPreset(code) {
    return CURRICULUM_LOCALE_BY_CODE[String(code)] || null;
}

/** Plain label for native `<select>` options (no emoji — OS fonts often tofu flags). */
export function curriculumLangOptionText(code) {
    const p = getCurriculumLangPreset(code);
    if (p) return `${p.code} — ${p.label}`;
    return String(code);
}
