...
import { normalizeLoadedTreeJson } from '../../tree-graph/tree-load-pipeline.js';
...
export const mountBundleMethods = {
    ...
    isSourcesDismissBlocked() {
        /* Keep first-run gate while a tree is mounting (data cleared mid-hydrate). */
        if (this.state.treeHydrating && !this.state.data) return true;
        const m = this.state.modal;
        const sourcesOpen =
            m && (m === 'sources' || (typeof m === 'object' && m.type === 'sources'));
        /* Keep first-run gate + fullbleed while Sources is open until curriculum is mounted. */
        if (sourcesOpen && (!this.state.data || this.state.treeHydrating)) return true;
    ...
    /**
     * Lightweight reload for CRUD operations: refreshes current source data without treeHydrating
     * (avoids UI blocking and preserves construction undo history).
     */
    async reloadCurrentSource() {
        const source = this.state.activeSource;
        if (!source) return;
        if (source.url?.startsWith('local://')) {
            const { json } = this.sourceManager.readLocalTreeSync(source);
            if (json) this.processLoadedData(json);
        } else if (typeof this.isNostrTreeSource === 'function' && this.isNostrTreeSource()) {
            // Nostr: re-fetch from memory/network (simplified; full reload needed for now)
            return this.loadData(source, false);
        } else {
            // HTTPS: fetch fresh
            try {
                const out = await this.sourceManager.loadData(source, this.state.lang, false, this.state.rawGraphData);
                if (out.json) this.processLoadedData(out.json);
            } catch (e) {
