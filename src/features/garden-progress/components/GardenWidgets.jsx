    ...
    return (
        ...
        >
            ...
            <div className="garden-vitality__head">
                <span className="garden-vitality__label">{ui.todayGoal || 'Fotosíntesis'}</span>
            ...
            <p className="garden-vitality__meta">
                {g.dailyXP || 0} / {goal} {ui.xpUnit || 'lúmenes'}
                {' · '}
                <span className="garden-vitality__balance">
                    {balance} {ui.gardenShopBalance || 'disponibles'}
                </span>
            </p>
        </div>
    );
...
export function GardenPlot({ ui }) {
    ...
    return (
        <div
            ...
            aria-label={ui.gardenPlotTitle || ui.gardenTitle || 'Mi parcela'}
        >
            {visible.map((item) => (
                <div
                    key={item.id}
    ...
    return (
        <section className="garden-shop" aria-labelledby="garden-shop-heading">
            <h4 id="garden-shop-heading" className="garden-shop__title">
                {ui.gardenShopTitle || 'Tienda del jardín'}
            </h4>
            <p className="garden-shop__lead">{ui.gardenShopDesc || ''}</p>
            <p className="garden-shop__hint">
                {ui.gardenShopLegend || 'Compra una decoración y la verás moverse en el fondo de la app.'}
            </p>
                        ...
                        >
                            ...
                            {owned ? (
                                <div className="garden-shop__actions">
                                    ...
                                    >
                                        {equipped
                                            ? ui.gardenShopEquipped || 'Puesto'
                                            : ui.gardenShopEquip || 'Poner'}
                                    </button>
                                    {equipped ? (
                                        <button
                                            type="button"
        ...
        >
            <div className="garden-ranking__head">
                <h4 id="garden-ranking-heading" className="garden-ranking__title">
                    {ui.gardenRankingTitle || 'Vecinos del árbol'}
                </h4>
                {consentOk ? (
                    <button
                        type="button"
                    ...
                    >
                        {rankingOn
                            ? ui.gardenRankingLeave || 'Salir'
                            : ui.gardenRankingJoin || 'Participar'}
                    </button>
                ) : null}
            </div>
