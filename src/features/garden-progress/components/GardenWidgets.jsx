        ...
        <section className="garden-shop" aria-labelledby="garden-shop-heading">
            ...
            <div className="garden-shop__grid">
                {GARDEN_SHOP_ITEMS.map((item) => {
                    ...
                    const ownedCount = isConsumable ? getConsumableOwnedCount(g, item) : 0;
                    const maxOwned = Math.max(1, Number(item.maxOwned) || 1);
                    const owned = isConsumable ? false : inventory.has(item.id);
                    const atMax = isConsumable && ownedCount >= maxOwned;
                    const equipped = !isConsumable && g.gardenDecor?.[item.slot] === item.id;
                    const label = ui[item.labelKey] || item.id;
                    const canBuy = isConsumable
                        ? !atMax && balance >= item.cost
                        : !owned && balance >= item.cost;
                    return (
                        <article
                            key={item.id}
                            className={`garden-shop__item${owned || atMax ? ' garden-shop__item--owned' : ''}${equipped ? ' garden-shop__item--equipped' : ''}`}
                        >
                            <span className="garden-shop__icon" aria-hidden="true">
                                <ChromeEmoji emoji={item.icon} size={28} />
                            </span>
                            <span className="garden-shop__name">{label}</span>
                            {isConsumable ? (
                                <span className="garden-shop__owned-count">
                                    {(ui.gardenShopShieldOwned || '{n}/{max}')
                                        .replace('{n}', String(ownedCount))
                                        .replace('{max}', String(maxOwned))}
                                </span>
                            ) : null}
                            {owned ? (
                                ...
                                <span className="garden-shop__max">
                                    {ui.gardenShopMaxShields || 'Maximum owned'}
                                </span>
                            ) : (
                                <button
                                    type="button"
