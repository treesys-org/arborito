...
import { getStoreTreeRoot } from '../../../shared/lib/store-facade.js';
...
export function getVitalityLabel(pct, ui) {
    ...
    if (pct >= 50) return ui.gardenVitalityWarm || 'Warm';
...
export function buildGardenPlotItems(store, ui) {
    ...
    return seeds.map((seed) => {
        ...
        let name = node?.name;
        if (!name || String(name).trim() === '' || String(name) === String(seed.id)) {
            const path = node?.path ? String(node.path) : '';
            const leaf = path.split('/').filter(Boolean).pop();
            name = leaf || store.state.activeSource?.name || seed.id;
        }
        return { ...seed, ...st, name: String(name) };
    });
}
