...
import { ARBORITO_ROOT_LOGO_DATA_URL } from '../../shared/ui/arborito-logo-root.js';
...
/** Root knot mark — `ArboritologoVERSION2RAIZPlain1SVG.svg`. Served as an
 *  `<img>` so the original artwork (bitmap + vector overlays) renders 1:1 to
 *  the source. The full SVG is inlined as a base64 data URL via
 *  `ARBORITO_ROOT_LOGO_DATA_URL` (see `shared/ui/arborito-logo-root.js`) so
 *  there is no separate HTTP request and the asset is not exposed as a static
 *  file under any stable URL. */
export function iconArboritoRootSvg({ className = 'arborito-icon-mark arborito-icon-mark--color', size = 22 } = {}) {
    ...
    return `<img src="${ARBORITO_ROOT_LOGO_DATA_URL}" width="${sz}" height="${sz}" class="${cls}" alt="" aria-hidden="true" draggable="false" style="display:block;width:100%;height:100%;object-fit:contain;object-position:center bottom"/>`;
}
