...
function ImpressumDetailsPre({ text, ui }) {
    const processed = String(text || '')
        .replace(/\{operatorName\}/g, ui?.operatorName || getBuiltInOperatorName())
        .replace(/\{operatorPhone\}/g, ui?.operatorPhone || getBuiltInOperatorPhone())
        .replace(/\{operatorAddress\}/g, ui?.operatorAddress || getBuiltInOperatorAddress());
    ...
    return (
        <div>
            ...
            <div className="bg-slate-50 dark:bg-slate-950/50 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 mb-6">
                <p className="text-slate-600 dark:text-slate-300 mb-4 leading-relaxed text-sm select-text">{ui.impressumText || ''}</p>
