 ...
 */
function getSearchResultsListHTML(state, ui, isBookmarks = false, lightChrome = false) {
    ...
    if (results.length === 0) {
        if (isBookmarks) {
            return `<div class="arborito-empty ${statePad} flex flex-col items-center gap-2">
                    <span class="text-3xl opacity-50">📖</span>
                    <span>${escHtml(ui.searchBookmarksEmpty || 'Start reading to save your place here.')}</span>
                 </div>`;
        }
        return `<div class="arborito-empty ${statePad} flex flex-col items-center gap-2"><span class="text-2xl opacity-50">🍃</span><span>${escHtml(ui.noResults)}</span></div>`;
    }
    ...
    const header = isBookmarks
        ? lightChrome
            ? `<div class="arborito-eyebrow arborito-eyebrow--sky px-4 py-2 bg-slate-100/90 dark:bg-slate-800/70 border-b border-slate-200 dark:border-slate-700">${recentHeaderText}</div>`
            : `<div class="arborito-eyebrow arborito-eyebrow--sky px-4 py-2 bg-slate-800/50 border-b border-slate-700">${recentHeaderText}</div>`
        : '';
