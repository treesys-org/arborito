...
function getWorker() {
    ...
    try {
        worker = new Worker(new URL('./search-index.worker.js', import.meta.url), { type: 'module' });
    } catch (e) {
 ...
 */
function fingerprintRawGraph(rawGraphData, hashFn) {
    try {
...
/**
 * @param {import('../../../stores/shell-store.js').ShellStore} store
 */
...
async function runSearchIndexRebuild(store) {
    ...
    const liveRoot = store.state.data;
    if (!(src && src.id)) {
        try {
    ...
    let languageRoots = null;
    if (raw && raw.languages && typeof raw.languages === 'object' && !Array.isArray(raw.languages)) {
        languageRoots = raw.languages;
    } else if (liveRoot && typeof liveRoot === 'object') {
        const code =
            store.state.lang && String(store.state.lang).length >= 2
                ? String(store.state.lang).toUpperCase().slice(0, 8)
                : 'EN';
        languageRoots = { [code]: liveRoot };
    } else {
        try {
            store.update({ searchIndexStatus: 'idle', searchIndexError: null });
        } catch {
    ...
    const preferLiveIndex = !!store.state.constructionMode;
    const rawFp = fingerprintRawGraph(raw, hashFn);
    const fp = preferLiveIndex
        ? rawFp
        : nostrSearchPackEntries != null
          ? hashFn(JSON.stringify(nostrSearchPackEntries) + '|' + rawFp)
          : torrentPackEntries != null
            ? hashFn(JSON.stringify(torrentPackEntries) + '|' + rawFp)
            : rawFp;

    try {
        const prev = await SearchIndexStore.getMeta(treeKey);
        if (prev && prev.fingerprint === fp && store.state.searchIndexStatus === 'ready') {
            return;
        }
        ...
        /** @type {object[]} */
        let allEntries;
        if (!preferLiveIndex && nostrSearchPackEntries != null) {
            allEntries = nostrSearchPackEntries;
        } else if (!preferLiveIndex && torrentPackEntries != null) {
            allEntries = torrentPackEntries;
        } else {
            allEntries = [];
            const langs = Object.keys(languageRoots);
            for (let i = 0; i < langs.length; i++) {
                if (i > 0) await yieldToMainThread();
                const langCode = langs[i];
                const root = languageRoots[langCode];
            ...
            /* Construction: merge hydrated lesson bodies from the live graph. */
            if (preferLiveIndex && store.state.data) {
                const liveLang = String(
                    store.getCurrentContentLangKey?.() || store.state.lang || 'EN'
                )
                    .toUpperCase()
                    .slice(0, 8);
                const liveEntries = flattenTreeSearchEntriesWithLessonBody(store.state.data, liveLang);
                allEntries = mergeSearchEntriesById(allEntries, liveEntries);
            }
        }
