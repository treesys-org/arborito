...
function sortResults(results, listKind) {
...
function ResultsList({ results, ui, listKind, onPick, onDeleteBookmark, sectionHeader, confirm, removeBookmark }) {
    const sorted = useMemo(() => sortResults(results, listKind), [results, listKind]);
...
export function SearchResultsPanel({ state, ui, lightChrome = false, onPick, onRefresh }) {
    ...
    if (query.length > 0) {
        if (results.length > 0 || query.length >= 2) {
            ...
            return (
                <>
                    <ResultsList results={results} ui={ui} onPick={onPick} confirm={confirm} removeBookmark={removeBookmark} />
