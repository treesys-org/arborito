...
const FONTS_BASE = new URL('../../../vendor/fonts/', import.meta.url);
const TWEMOJI_BASE = new URL('../../../vendor/emoji/twemoji/72x72/', import.meta.url);
...
function twemojiImgHtml(emoji, opts = {}) {
    ...
    if (!TWEMOJI_DATAURI) {
        ...
        const cls = /(^|\s)arborito-emoji-native(\s|$)/.test(extra) ? extra : `${extra} arborito-emoji-native`;
        const size = opts.size || 18;
        return (
            `<span class="${escAttr(cls)}" role="img" aria-label="${escAttr(ch)}" ` +
            `style="font-size:${size}px;line-height:1">${escHtml(ch)}</span>`
        );
    }
...
export async function initEmojiRendering() {
    ...
    if (typeof requestIdleCallback === 'function') {
        requestIdleCallback(runTwemojiPass, { timeout: 2000 });
    } else {
        setTimeout(runTwemojiPass, 800);
    }
}
