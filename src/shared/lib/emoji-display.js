 ...
 * 5. Modal hero back chevrons — SVG via React ModalHero, not emoji.
 */
 ...
 * The helpers above (chromeEmojiHtml/…) only cover call sites that opt
...
export function chromeEmojiHtml(emoji, size = 20) {
    const ch = String(emoji || '').trim() || '📄';
    const cls = 'arborito-emoji-glyph';
    const parts = ch.match(EMOJI_IN_TEXT_RE);
    if (parts && parts.length > 1) {
        return parts.map((p) => twemojiImgHtml(p, { className: cls, size })).join('');
    }
...
var SKIP_SEL='canvas,#bg-canvas,#game-canvas,#float-layer,.duel-card,.duel-card .card-face,.table-card,.table-card .card-face,.card-slot,[data-card-id]';
...
function refreshUi(){
  document.documentElement.classList.add('arborito-emoji-images');
  var roots=['.overlay','#start-overlay','#rules-overlay','#end-overlay','#start-screen','#victory-screen','#card-grid','#topic','.overlay-card','.fighter-portrait','.speech-bubble','.event-banner','.hud-top','#event-banner-stack','#start-features','.bg-icon','.memory-empty-icon','.review-empty-icon','.card-face'];
...
var UI_SEL='.overlay,.overlay-card,#start-features,#event-banner-stack,.hud-top,.fighter-portrait,.speech-bubble,.event-banner,#start-screen,#victory-screen,#card-grid,#topic,.bg-icon,.memory-empty-icon,.review-empty-icon,.card-face';
function observeUi(){
  var scheduled=false;
