...
export async function readArboritoArchive(input) {
    ...
    const attributionRaw =
        ...
        (files['ATTRIBUTION.json']
            ...
            : null);
    if (attributionRaw) {
        const { applyAttributionToTreeData } = await import('./arborito-attribution.js');
        applyAttributionToTreeData(tree, attributionRaw);
    }

    const { validateArboritoTree } = await import('./arborito-archive-validate.js');
...
export async function writeArboritoArchive(meta, treeData, bundledFiles = {}, opts = {}) {
    ...
    const attribution = opts.attribution
        ? (await import('./arborito-attribution.js')).normalizeAttribution(opts.attribution)
        : null;
    const manifest = {
