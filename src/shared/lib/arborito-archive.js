...
import {
    descriptionsFromTreeLanguages,
    pickTitleForLang,
    resolveManifestDescriptions,
    resolveManifestTitles,
    titlesFromTreeLanguages,
} from './catalog-titles.js';

const ARCHIVE_FORMAT = 'arborito';
...
async function writeZip(entries) {
    ...
    for (const { name, data } of entries) {
        ...
        /* Bit 11: UTF-8 filenames (Info-ZIP). Without it, unzip/Python show mojibake. */
        const gpFlags = 0x0800;
        const lfh = new Uint8Array(30 + nameBytes.length);
        const ldv = new DataView(lfh.buffer);
        ldv.setUint32(0, 0x04034b50, true);
        ldv.setUint16(4, 20, true);
        ldv.setUint16(6, gpFlags, true);
...
function reconstructTree(entries, manifestMeta) {
    const dec = new TextDecoder('utf-8');
    const universeId = String(manifestMeta.id || '').trim() || 'tree';
    const titles = resolveManifestTitles(manifestMeta);
    const descriptions = resolveManifestDescriptions(manifestMeta);
    ...
    const langKeys = [...byLang.keys()].map((l) =>
        String(l || '')
            .trim()
            .toUpperCase()
    );
    const fallbackLang = langKeys[0] || Object.keys(titles)[0] || 'ES';
    const courseName = pickTitleForLang(titles, fallbackLang, 'Course') || 'Course';
    const courseDescription = pickTitleForLang(descriptions, fallbackLang, '') || '';
    ...
    for (const [lang, langEntries] of byLang) {
        ...
        languages[lang] = buildLangRoot(lang, langEntries, entries, dec, {
            universeId,
            courseName: pickTitleForLang(titles, langKey, courseName) || courseName,
            courseDescription:
                pickTitleForLang(descriptions, langKey, courseDescription) || courseDescription,
            courseIcon,
        });
    }
 ...
 * @param {{ id?: string, titles?: object, descriptions?: object, icon?: string }} manifestMeta
 */
...
function* serializeTreeEntries(tree, _manifestMeta) {
    const enc = new TextEncoder();
    for (const [lang, langRoot] of Object.entries(tree?.languages || {})) {
        const prefix = `lessons/${lang}/`;
        const rootReadme = folderReadmeFromNode({
            ...langRoot,
            icon: langRoot?.icon && langRoot.icon !== '🌳' ? langRoot.icon : undefined,
        });
        if (rootReadme.trim()) {
            yield { name: `${prefix}${FOLDER_README}`, data: enc.encode(rootReadme) };
        }
        yield* serializeBranchChildren(langRoot.children || [], prefix, enc);
    }
}
 ...
 * already uses: `{ format, meta, tree, files? }` with each leaf
...
export async function readArboritoArchive(input) {
    ...
    if (manifest.format !== ARCHIVE_FORMAT) {
        throw new Error('Archive manifest has wrong format (expected format: "arborito")');
    }
    ...
    for (const [name, bytes] of entries) {
        if (name.startsWith('media/') && !name.endsWith('/')) {
            ...
            if (base && bytes instanceof Uint8Array) media[base] = bytes;
        }
    }
    ...
    const authorWarnings = validateArboritoTree(
        tree,
        Object.keys(tree.languages || {})[0] || 'ES'
    );
 ...
 * @param {{ id?: string, titles?: object, descriptions?: object, icon?: string }} meta
...
export async function writeArboritoArchive(meta, treeData, bundledFiles = {}, opts = {}) {
    ...
    const titlesFromTree = titlesFromTreeLanguages(treeData);
    const titles = {
        ...resolveManifestTitles(meta),
        ...titlesFromTree,
    };
    const descriptionsFromTree = descriptionsFromTreeLanguages(treeData);
    const descriptions = {
        ...resolveManifestDescriptions(meta),
        ...descriptionsFromTree,
    };
    const cleanMeta = { ...(meta || {}) };
    delete cleanMeta.name;
    delete cleanMeta.description;
    delete cleanMeta.language;
    const localId = String(cleanMeta.id || '').trim();
    const manifest = {
        format: ARCHIVE_FORMAT,
        contentKind: 'branch',
        meta: {
    ...
    /* Never ship empty id / leftover single-string catalog fields. */
    if (!localId) delete manifest.meta.id;
    delete manifest.meta.name;
    delete manifest.meta.description;
    delete manifest.meta.language;
    ...
    const mediaFiles = opts.mediaFiles && typeof opts.mediaFiles === 'object' ? opts.mediaFiles : {};
    for (const [name, bytes] of Object.entries(mediaFiles)) {
        const safe = String(name || '')
            .replace(/^.*[/\\]/, '')
            .replace(/[^A-Za-z0-9._-]/g, '');
        if (!safe || !(bytes instanceof Uint8Array) || !bytes.byteLength) continue;
        allEntries.push({ name: `media/${safe}`, data: bytes });
    }
