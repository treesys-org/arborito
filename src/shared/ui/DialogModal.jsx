...
export function ModalDialog() {
    ...
    const confirmLbl =
        confirmText ||
        (danger ? ui.delete : null) ||
        ui.dialogConfirmButton ||
        ui.dialogOkButton ||
        'OK';
    const cancelLbl = cancelText || ui.cancel || 'Cancel';
    const finish = (result) => {
    ...
    const dialogRootFlags = danger
        ? 'arborito-modal--dialog arborito-modal--danger'
        : 'arborito-modal--dialog';
    ...
    const confirm = () => {
        if (isPrompt) {
            ...
            if (switchLabel) {
                finish({ confirmed: true, checked: syncOn });
            } else {
                finish(true);
            }
    ...
    }, [dialogType, title, body, placeholder]);
    ...
    const sheetFooterPadding = mobile
        ? 'px-3 sm:px-4 pt-3 pb-0'
        : 'px-6 pt-3 pb-6';
    /* Mobile dock-gap: blend inherits panel surface to the piso (same as compact hubs). */
    const sheetFooterVariant = mobile ? 'blend' : 'flat';

    const sheetFooter = dialogFooterLayout ? (
        choiceConsolidated ? (
            <ModalBinaryFooter
                footerVariant={sheetFooterVariant}
                paddingClass={sheetFooterPadding}
                stackMobile={mobile}
            >
                <div
                    className={`arborito-action-row w-full ${mobile ? 'arborito-action-row--stack-mobile' : ''}`}
                >
            ...
            <ModalBinaryFooter
                ...
                paddingClass={sheetFooterPadding}
                stackMobile={mobile}
                hideCancel={isAlert || hideCancelBtn}
                cancelLabel={cancelLbl}
                onCancel={cancel}
                ...
                fullWidthConfirm={hideCancelBtn}
            />
        )
    ) : null;
    ...
    const bodyContent = (
        <div
            className={`flex flex-col min-h-0 flex-1 overflow-hidden ${sheetBodyLayout ? 'arborito-dialog-sheet-body' : ''} ${choiceConsolidated || choicePickList ? 'arborito-dialog-choice-sheet' : ''} ${mobile ? (sheetFooter ? 'px-3 sm:px-4 pt-2 pb-2' : 'px-3 sm:px-4 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]') : 'p-6 pt-2'}`}
        >
            {showBodyText ? (
                <div
            ...
            {isConfirm && switchLabel ? (
                <div className="w-full max-w-md mx-auto mb-4 px-1 shrink-0">
                    <SwitchRow
                        ...
                        checked={syncOn}
                        onChange={setSyncOn}
                        onAria={ui.publicTreeIncludeForumSwitchOn || 'Turn on'}
                        offAria={ui.publicTreeIncludeForumSwitchOff || 'Turn off'}
                    />
                </div>
            ) : null}
            ...
            {isExportSnapshots && Array.isArray(exportSnapshots) && exportSnapshots.length > 0 && (
                <div className="flex flex-col min-h-0 flex-1 mb-4">
                    ...
                    <button
                        type="button"
                        className={`${danger ? modalCtaConfirmDangerFull() : modalCtaConfirmFull(confirmTone)} shrink-0`}
                        onClick={confirm}
                    >
                        {confirmLbl}
                    </button>
            ...
            {!sheetFooter && !isExportSnapshots && !choiceConsolidated && !choicePickList && (
                <ModalBinaryFooter
                    footerVariant={sheetFooterVariant}
                    className={sheetBodyLayout ? 'mt-auto' : ''}
                    stackMobile={mobile}
                    hideCancel={isAlert || hideCancelBtn}
                    cancelLabel={cancelLbl}
                    onCancel={cancel}
