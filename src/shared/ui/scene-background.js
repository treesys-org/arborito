...
export function syncSceneBackgroundLayer({ pinBody = false } = {}) {
    ...
    if (body && body.classList.contains('arborito-app')) {
        const pinElectronBody =
            pinBody && typeof window !== 'undefined' && window.arboritoElectron;
        if (pinElectronBody) {
            body.style.width = '100%';
            body.style.minHeight = `${h}px`;
            body.style.height = `${h}px`;
        } else if (!window.arboritoElectron) {
...
function scheduleSceneBackgroundSync() {
    ...
    requestAnimationFrame(() => {
        rafPending = false;
        syncSceneBackgroundLayer({ pinBody: false });
    });
}
