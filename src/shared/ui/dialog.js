...
import { bindMobileTap, isModalBackdropEmptyTap } from './mobile-tap.js';
import { modalShellHtml } from './modal-shell.js';
import { modalHeroHtml } from './modal-hero.js';
import { escHtml as escText, escHtml as escAttr } from '../lib/html-escape.js';
...
class ArboritoModalDialog extends HTMLElement {
    ...
    render() {
        ...
        const ui = store.ui;
        const chromeRow = modalHeroHtml(ui, { title: escText(title), backTagClass: 'btn-dialog-mob-back', closeTagClass: 'btn-dialog-dismiss' });
        ...
        const dialogBody = `
                ...
                <div class="mb-6 shrink-0">
                    <input type="text" class="arborito-input border-none font-bold" placeholder="${escAttr(placeholder || '')}">
                </div>
                ` : ''}
                ...
                <div class="arborito-action-row shrink-0">
                    ${!isAlert ? `
                    <button class="btn-cancel arborito-cta-slate py-3 rounded-xl font-bold text-xs uppercase tracking-wider">
                        ${escText(cancelText || 'Cancel')}
                    </button>
                    ` : ''}
                    
                    <button class="btn-confirm py-3 ${confirmBtnClass} font-bold rounded-xl shadow-lg transition-transform active:scale-95 text-xs uppercase tracking-wider">
                        ${escText(confirmText || 'OK')}
                    </button>
                </div>
                ` : ''}
        ...
        this.innerHTML = modalShellHtml({
            ...
            enter: 'fade-fast',
            panelSize: 'auto-h',
            panelClass: 'arborito-float-modal-card--narrow animate-in zoom-in-95 duration-200 max-h-[min(90vh,640px)]',
            panelRadius: '2xl',
        });
