...
import { brandIconSvgHtml } from '../lib/brand-icon-svg.js';
...
export function renderDownloadAppPanelHtml(ui, opts = {}) {
    ...
    const platformBtns = platforms
        .map((p) => {
            ...
            return `<a class="arborito-download-app-panel__platform" data-brand="${escAttr(p.brand)}" href="${escAttr(p.url)}" target="_blank" rel="noopener noreferrer"
                title="${escAttr(`${label}, ${sub}`)}" aria-label="${escAttr(`${label} (${sub})`)}">
                <span class="arborito-download-app-panel__platform-ic" aria-hidden="true">${brandIconSvgHtml(p.brand, { size: 22 })}</span>
