...
class Store extends UIStore {
    ...
    clearConstructionUndoStack() {
        ...
        if (this._constructionRedoStack?.length) this._constructionRedoStack.length = 0;
    }
    ...
    async initialize() {
        ...
        const qrAuthDeepLink = this._peekQrAuthQueryParam();
        
        // First-run onboarding: brief intro + language picker + start button.
        if (!recoveryDeepLink && !qrAuthDeepLink) {
            let seen = true;
            try {
    ...
    /**
     * Check if opened via QR deep link (WhatsApp-style signaling auth).
     * @returns {boolean} true if this load was a QR auth deep link.
     */
    _peekQrAuthQueryParam() {
        try {
            if (typeof window === 'undefined' || window.location.protocol === 'file:') return false;
            const originalUrl = window.location.href;
            const u = new URL(originalUrl);
            if (!u.searchParams.has('auth_sid') || !u.searchParams.has('auth_pub')) return false;

            // Clean up URL
            u.searchParams.delete('auth_sid');
            u.searchParams.delete('auth_pub');
            u.searchParams.delete('auth_rel');
            u.searchParams.delete('auth_ts');
            const qs = u.searchParams.toString();
            const next = `${u.pathname}${qs ? `?${qs}` : ''}${u.hash || ''}`;
            window.history.replaceState({}, '', next);
            ...
            queueMicrotask(async () => {
                ...
                if (!isSyncAuthed || !sess?.username || !sess?.syncSecretPlain) {
                    this.notify(this.ui.syncLoginQrAuthRequired || "Sign in with your sync code first to authorize other devices.", true);
                    this.setModal({ type: 'sync-login' });
                } else {
                    if (window.confirm(this.ui.qrAuthConfirmPrompt || "Authorize a new device to sign in to your offline account?")) {
                        try {
                            await this.scanQrSignalingAndAuthorize(originalUrl);
                        } catch (err) {
    ...
    _scheduleDeferredProductTour() {
        ...
        const attempt = () => {
            ...
            /* No tree loaded: always run the “pick a tree” tour before the map/shell tour. */
            if (!this.state.data) {
