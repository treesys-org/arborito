...
**Folder / `.arborito` format** (all `@` tags, quizzes, bilingual layout): [`docs/AUTHOR-FORMAT.md`](docs/AUTHOR-FORMAT.md), also shipped as `files/AUTORIA.md` inside every export.
...
If you draft a lesson with a language model, fact-check and edit before publishing. Published lessons stay under **CC BY-SA 4.0**.
...
Optional flags inside the `@quiz` block: `skip_multiple: yes`, `skip_ordering: yes`, `modes: recall,multiple` (comma-separated list to limit modes). See [`src/features/learning/quiz-schema.js`](src/features/learning/quiz-schema.js) for validation rules.
...
> If you want the **map** of the codebase first, start with [`docs/dev-onboarding.md`](docs/dev-onboarding.md) and [`docs/CODEBASE_CONVENTIONS.md`](docs/CODEBASE_CONVENTIONS.md), layout, naming, file-size limits, and what knip reports mean.
...
### Where to put your change (junior dev cheat sheet)
...
| Change store actions for a domain | `src/features/<name>/api/actions/` |

**Rule:** In `.jsx` files, import **`useSearch`**, **`useForum`**, etc., **not** `core/store.js`. Hooks live in `features/<name>/hooks/`. See [`ARCHITECTURE.md`](ARCHITECTURE.md).
...
**Start here:** [`ARCHITECTURE.md`](ARCHITECTURE.md) · [`docs/react-architecture.md`](docs/react-architecture.md) · [`docs/jr-developer-guide.md`](docs/jr-developer-guide.md)
...
Domain actions are moving from `features/*/api/actions/` (prototype mixins) to `stores/*-store-actions.js` + hooks.
...
Split into `components/`, `hooks/`, and `api/` subfolders. Keep files under **1000 lines** (`npm run check:max-lines`). Store mixins in `api/actions/` are legacy, prefer `stores/<domain>-store-actions.js`.
