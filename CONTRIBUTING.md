...
*   `nostr-universe.js`: Public `nostr://` trees, bundles, share codes, and forum snapshots.
*   `ai.js`: AI providers (Ollama, in-browser models, etc.).
*   `filesystem.js`: Abstraction for reading/writing lesson files depending on the active source (local garden, public tree, or read-only HTTPS bundle). Components should use `fileSystem.js` rather than duplicating source logic.
...
Coordinate with the project maintainers using whatever workflow they use for this codebase (branching, reviews, and merges are project-specific).
