...
contextBridge.exposeInMainWorld('arboritoElectron', {
    ...
    llamacpp: {
        ...
        clearModels: () => ipcRenderer.invoke('arborito-llamacpp-clear-models'),
    ...
    /** Frozen trees / offline games as files under userData (Flatpak: ~/.var/app/…/config/arborito). */
    userData: {
    ...
    ...(process.platform === 'win32'
        ? {
              /** Windows: check only after privacy consent; prompt before download + NSIS silent install. */
              appUpdate: {
                  onAvailable: (cb) => on('arborito-app-update-available', cb),
                  onError: (cb) => on('arborito-app-update-error', cb),
                  check: () => ipcRenderer.invoke('arborito-app-update-check'),
                  confirm: () => ipcRenderer.invoke('arborito-app-update-confirm'),
                  dismiss: () => ipcRenderer.invoke('arborito-app-update-dismiss'),
              },
          }
        : {}),
});
