@info
icon: ⭐
description: Puntos, racha, trofeos y jardín
@/info

@section
index: 1
title: Tu progreso
@/section

Arborito te da puntos para motivarte a seguir. Es **tu** avance, no una competencia.

@image
url: ./media/11-jardin-es.png
caption: Jardín: semillas al estudiar.
@/image

@section
index: 2
title: XP, racha y paraguas
@/section

- **Puntos (XP)** al terminar lecciones y acertar.
- **Racha:** días seguidos estudiando.
- **Paraguas:** protege la racha si un día no estudias. Ganas uno cada 3 días de racha (máximo 3). También en la Tienda del jardín.
- **Semillas** al terminar un tema (en Progreso o en la mochila).

@quiz
concept: Progreso diario
items:
  - question: ¿Qué sube al estudiar y acertar?
    answer: Puntos XP
    modes: multiple,recall
    traps:
      - El volumen del móvil
      - El precio del curso
      - Los archivos .arborito
  - question: ¿Qué es la racha?
    answer: Días seguidos estudiando
    modes: recall,multiple
    traps:
      - Número de exámenes
      - Cursos importados
      - Solo puntos de Arcade
  - question: ¿Para qué sirve el paraguas?
    answer: Protege la racha si un día no estudias
    modes: multiple,recall
    traps:
      - Sube el volumen
      - Abre Cursos
      - Borra el progreso
@/quiz

@section
index: 3
title: Trofeos
@/section

En **Logros** (mochila):

1. **Curso:** terminas todo un curso.
2. **Playlist:** terminas todos los cursos de una playlist.
3. **Logro extra:** el autor lo pone en una carpeta (modo construcción).

@quiz
concept: Trofeos principales
items:
  - question: ¿Qué trofeo ganas al terminar este demo entero?
    answer: El trofeo de curso
    modes: multiple,recall
    traps:
      - Solo un examen
      - Un logro de carpeta
      - Solo puntos de Arcade
  - question: ¿Qué trofeo pide todos los cursos de una playlist?
    answer: El trofeo de playlist
    modes: multiple,recall
    traps:
      - Trofeo de curso
      - Solo racha
      - Solo Classroom
  - question: ¿Los trofeos son para competir con otras personas?
    answer: No, son para ti
    modes: multiple,recall
    traps:
      - Sí, hay ranking global
      - Solo en Arcade
      - Solo en demo
@/quiz
