...
if (isDirectRun) register('./lib/node-arborito-resolve.mjs', import.meta.url);
    ...
    const publish = async (slot, entries) => {
        ...
        if (bytes.length <= NOSTR_CHUNK_CONTENT_MAX) {
            ...
            while (i < bytes.length) {
                let end = Math.min(bytes.length, i + NOSTR_CHUNK_CONTENT_MAX);
        ...
        if (parts.length <= 1) {
            ...
            const hdr = finalizeEvent(
                {
                    kind: KIND_DIRECTORY_INDEX_SNAPSHOT,
                    created_at: Math.floor(Date.now() / 1000),
                    tags: [['d', `arborito:diridx:${slot}:v1`], ['n', String(parts.length)]],
            ...
            await Promise.all(pool.publish(relays, hdr));
            for (let i = 0; i < parts.length; i++) {
                const pev = finalizeEvent(
                    {
                        kind: KIND_DIRECTORY_INDEX_SNAPSHOT,
                        created_at: Math.floor(Date.now() / 1000),
                        tags: [['d', directoryIndexChunkDTag(slot, i)], ['i', String(i)], ['n', String(parts.length)]],
                        content: parts[i]
                    },
                    skBytes
                );
                await Promise.all(pool.publish(relays, pev));
            }
        }
        console.log(`[directory-index] ${slot}: ${payload.entries.length} entries (${parts.length} chunk(s))`);
    };
