...
function renderRoute({ mode, sageAiMode, aiStatus, hasConsent, webAiUnavailable = false }) {
    // `mode === 'settings'` is an overlay; screen routing keeps the underlying sage screen.
    if (sageAiMode !== 'dynamic') return 'context';
    if (webAiUnavailable) return 'web-gate';
...
assert.equal(renderRoute({ mode: 'settings', sageAiMode: 'dynamic', aiStatus: 'ready', hasConsent: true }), 'chat');
assert.equal(
    renderRoute({ mode: 'settings', sageAiMode: 'dynamic', aiStatus: 'ready', hasConsent: false, webAiUnavailable: true }),
    'web-gate'
);
assert.equal(
    renderRoute({ mode: 'context', sageAiMode: 'dynamic', aiStatus: 'ready', hasConsent: true, webAiUnavailable: true }),
    'web-gate'
);
assert.equal(
