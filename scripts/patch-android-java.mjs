...
if (
    patchFile(join(ANDROID, 'app', 'capacitor.build.gradle'), (s) => {
        if (/JavaVersion\.VERSION_\d+/.test(s)) {
            return s.replace(/JavaVersion\.VERSION_\d+/g, `JavaVersion.VERSION_${JAVA}`);
        }
        return s;
...
if (
    patchFile(join(ANDROID, 'gradle.properties'), (s) => {
        let out = s;
        if (!/org\.gradle\.java\.installations\.auto-download\s*=/.test(out)) {
            out = `${out.trimEnd()}\norg.gradle.java.installations.auto-download=true\n`;
        }
        if (JAVA_HOME) {
            const escaped = JAVA_HOME.replace(/\\/g, '\\\\');
            const line = `org.gradle.java.home=${escaped}`;
            if (/org\.gradle\.java\.home\s*=/.test(out)) {
                out = out.replace(/org\.gradle\.java\.home\s*=.*(?:\r?\n|$)/, `${line}\n`);
            } else {
                out = `${out.trimEnd()}\n${line}\n`;
            }
        }
