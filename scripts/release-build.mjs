...
console.log('[release-build] 1/4 : Vite web build + Flatpak assets');
npmRun('build');
run('node', ['./scripts/flatpak-sync.mjs']);
run('node', ['./scripts/ensure-build-icon.mjs']);

if (wantFlatpak) {
    console.log('[release-build] 2/4 : Flatpak');
    run('node', ['./scripts/flatpak-toolkit.mjs', 'setup']);
    run('npx', ['electron-builder', '--linux', 'flatpak', '--publish', 'never'], {
        env: { ...process.env, DEBUG: process.env.DEBUG || '@malept/flatpak-bundler' },
    });
    run('node', ['./scripts/flatpak-toolkit.mjs', 'rebundle']);
    run('node', ['./scripts/flatpak-toolkit.mjs', 'verify']);
} else {
