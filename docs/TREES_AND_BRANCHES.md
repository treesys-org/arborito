...
Arborito separates **what you study** (a branch) from **how you combine courses** (a tree). Both are managed in **Forest** (EN) / **Bosque** (ES).
...
**Achievements:** one default **tree trophy** per imported branch. Optional **diplomas** on any folder via Properties → Issue diploma. See [`ACHIEVEMENTS.md`](ACHIEVEMENTS.md).
...
- **Forest stays open** when you load a branch or tree during normal use (the list refreshes in place). The modal **closes only** during **onboarding/welcome** (`modal.fromOnboarding`) so first-run users land on the canvas. Logic: `src/features/sources/api/sources-session.js` (`finishSourcesLoadSession`).
...
- **Local composed tree:** on enter, choose **Tree** (playlist info → Forest → Trees) or **Branch** (edit lessons locked to one ref).
...
| Action | Branch | Tree |
|--------|--------|------|
| Export | Forest → Branches → Export → `.arborito` | Trees tab → Export → `.arborito` (embeds local branches when present) |
| Import | Forest → Import → Branches tab | Forest → Import → Trees tab |
...
| `src/features/forest/api/import-composed-tree-bundle.js` | Import composed `.arborito` |
| `src/features/sources/api/sources-session.js` | Forest modal session: when to close vs refresh after load; Nostr prep before network loads |
| `src/features/sources/modals/sources-logic.js` | File import UI |
| `src/features/forest/api/mount-composed-tree.js` | Load `tree://` on the graph |
...
Achievements and trophies: [`ACHIEVEMENTS.md`](ACHIEVEMENTS.md).
