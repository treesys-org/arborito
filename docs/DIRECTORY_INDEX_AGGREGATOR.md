...
Clients read snapshots first in `NostrUniverseService.listGlobalTreeDirectoryEntriesOnce` (see `src/features/nostr/nostr-universe.js`).

## Production checklist

1. **Aggregator SEA pair** — Generate a dedicated keypair for the indexer (not an author course key).
2. **`src/features/p2p-webtorrent/directory-index-config.js`** — Set `DIRECTORY_INDEX_TRUSTED_PUBLISHERS` to the aggregator’s **`pub`** string (exact match). Rebuild the app you ship.
