...
Arborito online is **static** (GitHub Pages). Scale comes from **Nostr + WebTorrent + client-side stores**, not from a central database.
...
- Static `/catalog/` shard files on Pages
- `search.arborito.org` or any search API subdomain
...
See [NOSTR_DIRECTORY_SEARCH.md](NOSTR_DIRECTORY_SEARCH.md) and [DIRECTORY_INDEX_AGGREGATOR.md](DIRECTORY_INDEX_AGGREGATOR.md).
