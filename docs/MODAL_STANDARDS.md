...
| `NestedSheetShell` | `src/shared/ui/NestedSheetShell.jsx` | Sub-dialog **inside** a hub panel (`absolute inset-0` scrim). Mount via hub `overlay` prop; never inside a scroll body |
| `ConfirmNestedSheet` | `src/shared/ui/ConfirmNestedSheet.jsx` | Binary confirm inside a hub (delete tree, snapshot, …); centered Callout + footer; dismiss via Cancel / Escape / scrim |
| `FormNestedSheet` | `src/shared/ui/FormNestedSheet.jsx` | Form prompt inside a hub (Export, forum, arcade); `ModalHubHero` + body + `ModalBinaryFooter`: same header as Language / DialogModal |
| `ModalBinaryFooter` | `src/shared/ui/ModalBinaryFooter.jsx` | Shared Cancel/OK footer (`DialogModal`, nested sheets, properties) |
...
| Forum new-topic sheet | `ForumThreads.jsx` `ForumNewTopicOverlay` + `FormNestedSheet` | Hub `DockModalShell` / embed `overlay` (not inside scroll columns) | `ModalHubHero` on form |
...
| Tree switcher | `CurriculumSwitcherModal.jsx` | mob: `DockModalShell` fullbleed; desktop: `ModalShell` bareBackdrop + right drawer | `ModalHubHero` ✓ |
...
Version picking is only via `CurriculumSwitcherModal`.
...
| `#arborito-tree-switcher-backdrop` | Tree switcher | fullbleed |
...
- **Consolidation:** the backdrop uses `--arborito-theme-modal-scrim` (translucent). The floating card sits over the live forest — never a solid wall that hides the graph.
- Search → inline in header (`search-redirect`); no modal in forest.
- Tree switcher → drawer `ModalShell` `bareBackdrop` portaled (`#arborito-tree-switcher-backdrop`).
...
Accepted examples: `panelTone: 'sage' | 'dark'`, `scrim: 'translucent'` (default — forest consolidation), `shellOpts.enter`, `ModalHubHero` `tone: 'danger'`. Do **not** use opaque full-viewport plates except immersive games (`scrim: 'black'`).
...
| Pattern | File | When |
|---------|------|------|
| `ModalBinaryFooter` | `ModalBinaryFooter.jsx` | Shared Cancel/OK footer (`DialogModal`, `ConfirmNestedSheet`, `FormNestedSheet`, properties, rename) |
| `MODAL_CTA_CANCEL` + `modalCtaConfirm()` | `modal-action-chrome.js` | Buttons inside that footer (or rare custom rows via `children`) |
| `arborito-modal-footer` padding | `arborito-cta-forms.css`, `dialog-content.css` | Do **not** zero `padding-top` on `DialogModal` footers (`store.confirm`); buttons must clear the `border-top` divider |
| `arborito-modal-footer--blend` | `arborito-cta-forms.css` | Compact construction hubs, transparent footer inheriting panel surface |
| `arborito-modal-footer--bg-flat` | same | Full-bleed sticky bars with solid bg (properties, consent); default for `ModalBinaryFooter` |
...
Eager vs lazy list: [`DEVELOPMENT.md`](DEVELOPMENT.md) § Modals.
...
| ModalHost | `src/app/components/ModalHost.jsx` | `#modal-backdrop` takeover + desktop overlays |
