...
- **Tree codes** (`share code`): the Nostr graph `arborito.codes.{code}` points to a signed record that resolves to `{ pub, universeId }`. See `resolveTreeShareCode` / `putTreeCodeClaim` in [`client/index.js`](../src/features/nostr/api/client/index.js).
