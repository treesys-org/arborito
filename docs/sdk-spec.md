...
This document describes the **logical API** for **browser cartridges** (`window.arborito`) and the **Python SDK** (`arborito-games/sdk/`) against **arborito-library** `data/`.
...
| Python (stdlib) | `arborito-games/sdk/` | Same API names as the player; reads `arborito-library/data`; Ollama for `ask.json`. |
...
[Arborito Games README — Python SDK (native tools)](../../arborito-games/README.md#python-sdk-native-tools)
