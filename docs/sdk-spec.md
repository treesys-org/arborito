...
| Surface | Where | Notes |
|--------|--------|------|
| Browser iframe | `arborito/src/features/arcade/inject-game-sdk.js` | Injected by `game-player.js`; talks to `window.parent.__ARBORITO_GAME_BRIDGE__`. |
...
| AI (`ask.json`, `ask.chat`) | ✅ Goes through the host `aiService`: native llama.cpp on desktop, wllama (WebAssembly) in the browser. Zero setup for the player; respects the user's static/dynamic mode. | ⚠️ Calls a local `llama-server` (llama.cpp) at `LLAMA_CPP_HOST` (default `http://127.0.0.1:8080`). **You** are responsible for shipping or starting it on the player's machine; if no server is reachable, dynamic mode fails and you fall back to static. |
...
Canonical reference: [`src/features/learning/quiz-v2-schema.js`](../src/features/learning/quiz-v2-schema.js) (`ALL_QUIZ_MODES`, `getPlayableModes`, `pickStudyQuizMode` — `modeIsPlayable` is the file-private predicate behind `getPlayableModes`).

### What the SDK exposes for game authors

The SDK (browser cartridge and Python) shares the same modality logic as the in-app Care/Study view. Canonical implementation: [`src/features/learning/quiz-v2-schema.js`](../src/features/learning/quiz-v2-schema.js) (imported in the host; inlined in [`inject-game-sdk.js`](../src/features/arcade/inject-game-sdk.js) for iframe cartridges).
...
Full chat; returns provider-specific result. In the browser/Electron host this goes through the same `aiService.chat()` pipeline as Sage (native llama.cpp on desktop, wllama in the browser).
...
- **`challenge.modes`:** `playable`, `pick`, `buildCard`, `buildStudyCard` — all five Quiz V2 modalities (see [Quiz V2 modalities](#quiz-v2-modalities-coverage-today)).
