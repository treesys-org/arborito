...
- **Network defaults:** `from_share_code` / `from_nostr` connect to the same Nostr relays the Arborito app uses by default (public operators in the EU and Canada). Override per-call with `relays=[..]` or globally with the `ARBORITO_NOSTR_RELAYS` environment variable. See [`NETWORK.md`](./NETWORK.md#nostr-relays).
