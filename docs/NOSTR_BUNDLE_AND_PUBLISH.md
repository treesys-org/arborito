...
1. [`prepareNostrSplitBundleV2`](../src/features/nostr/nostr-bundle-chunks.js) clones the bundle and:
...
- [`source-manager`](../src/features/sources/source-manager.js): requires `nostrBundleFormat === 2`; leaves the `bundle.forum` stub as-is (no forced forum load in the first `once`).
...
- Lessons: [`GraphLogic.loadNodeContent`](../src/features/tree-graph/graph-logic.js) + `treeLazyContent` / `treeContentKey`.
