...
- **Number of trees a single browser can list locally.** `localTrees` lives in
  `localStorage`, which is ~5–10 MB per origin in most browsers. A realistic
  ceiling is **a few hundred small local gardens**, not thousands. Browsers
  that share the same profile across windows hit this faster. For a teacher
  keeping dozens of drafts this is fine; for an institution storing every
  student's tree locally it is not.
- **Saved bookmarks (`communitySources`).** Lightweight (URLs + metadata),
  comfortably scales to thousands per browser.
- **Sources modal display cap.** `SOURCES_UNIFIED_DISPLAY_CAP = 160` rows
  visible at any time (see `src/features/p2p-webtorrent/directory-index-config.js`). The full local +
...
- **Global directory index snapshot cap.** 800 entries per signed snapshot
  ...
  [`DIRECTORY_INDEX_AGGREGATOR.md`](DIRECTORY_INDEX_AGGREGATOR.md). For
  Wikipedia-scale catalog discovery you would add a dedicated HTTP search
  service in front of this.
...
> tree from `localTrees`, drop a row from `arborito-sources`, or wipe the
> `arborito-active-source-meta` pointer that remembers what the user had open.

The pieces that enforce this:

- [`mountCurriculum`](../src/features/sources/mount-curriculum.js) snapshots the
...
- [`publishTreePublicInteractive`](../src/features/publishing/store-mixins/store-publish-revoke-methods.js)
...
- [`NostrUniverseService._installRelayCircuitBreaker`](../src/features/nostr/nostr-universe.js)
...
- A **global search across millions of trees**. The directory snapshot
  ...
  millions. Wikipedia-scale catalog browsing needs a dedicated HTTP search
  service in front of the snapshots.
