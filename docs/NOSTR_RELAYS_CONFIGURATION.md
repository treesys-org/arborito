...
Defined in [`src/features/nostr/nostr-relays-runtime.js`](../src/features/nostr/nostr-relays-runtime.js) (`DEFAULT_NOSTR_RELAYS`). In the current version:
...
If your deployment targets EU-only audiences and you need to keep traffic away from US infrastructure, override the list via `window.ARBORITO_NOSTR_RELAYS` or `localStorage['arborito-nostr-relays-v1']` (see precedence below) and update the privacy copy accordingly — the logic in `src/features/nostr/nostr-universe.js` does not depend on jurisdiction; it only calls whatever URL list you provide.
...
The read-and-normalise logic lives in that same `nostr-relays-runtime.js` module and in [`src/features/nostr/nostr-universe.js`](../src/features/nostr/nostr-universe.js).
...
Operational consequence: **no network failure destroys local data**. A tree saved in `localTrees` or a bookmark in `arborito-sources` is not deleted because a relay does not answer; relays sync, they do not destroy. That guarantee is implemented by `mountCurriculum` (`src/features/sources/mount-curriculum.js`) and `publishTreePublicInteractive` (`src/features/publishing/store-mixins/store-publish-revoke-methods.js`).
