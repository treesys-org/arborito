...
The logic lives in `discoverManifest` in [`src/features/sources/api/source-manager.js`](../src/features/sources/api/source-manager.js). For an HTTP source URL it tries, in order, candidates derived from the base URL:
...
The first valid JSON updates `availableReleases`. Nostr, `branch://`, `tree://`, and non-HTTP origins do **not** go through this discovery.
