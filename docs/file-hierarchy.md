...
│   └── modal-open.js              openModal / prefetchModal (canonical API)
│
├── core/                          State engine
...
│   ├── catalog-store.js           Branches/trees mirror (Forest)
...
└── features/                      One domain = one folder
    ...
    ├── sources/                   Forest (library), catalog
    ├── identity-auth/             Profile, password login, onboarding — see docs/AUTH_AND_ACCOUNT.md
...
| **Modal chunk** | Sources, forum, profile, publishing… | `modal-chunk-loaders.js` + `useModalChunk` / `ModalHost` |
| **Dock hub panel** | Search, arcade, forum (mobile dock) | `DockHubPanelLayer.jsx` + `modal-surface-routing.js` |
| **Store mixin defer** | forum, publish, admin | `attach-actions.js` on first use |

### Modals: eager vs lazy

Open modals via [`modal-open.js`](../src/app/modal-open.js) (`openModal`, `openModalWhenReady`, `prefetchModal`).

| Load | Types | Source |
|------|-------|--------|
| **Eager** (instant) | `dialog`, `profile`, `about`, `language`, `preview`, `certificate`, `emptyModule`, `privacy`, `celebration-prefs`, `download-app`, `accessibility-prefs`, `security-warning`, `load-warning`, `tree-info`, `move-node`, `pick-curriculum-lang`, `sync-login-qr-scanner`, `account-recovery`, `construction-about` | `eager-modals.js` |
| **Lazy** (chunk fetch) | `sources`, `search`, `certificates`, `export-pdf`, `backup`, `arcade`, `game-player`, `node-properties`, `construction-curriculum-lang`, `forum`, `construction-history`, `construction-edit-pick`, `contributor` | `modal-chunk-loaders.js` |

`construction-about` is eager (publish hub must open on first click). Other construction modals stay lazy.
