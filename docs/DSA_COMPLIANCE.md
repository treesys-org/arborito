...
| **Art. 17**: statement of reasons | ✅ | Affected publishers get an in-app statement with facts (score, threshold, unique reporters), ground (community policy), scope (in-app directory visibility only) and redress path (Courses → row ⋯ → contest / legal response): `creatorDirectoryHiddenStatement`, `creatorReportsToastProgress`, `creatorLegalDisputeToast`, owner banner in `SourcesInternetRow.jsx` |
