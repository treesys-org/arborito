...
- **Where the DSA plausibly bites:** to the extent Treesys is considered an
  intermediary/hosting-like service for the **Discover index** in the app (consultative
  listing over relays the user chose, plus build blocklist and algorithmic moderation),
  the safest posture is to satisfy the intermediary + hosting obligations that are cheap
  and to document the rest. Relay transport is user-configured; Treesys does not operate relays.
- **Size:** Treesys is a micro/small enterprise. Under **Art. 19**, the additional
  ...
  micro/small enterprises. They are therefore documented as N/A below, with the
  spirit of some of them implemented anyway where cheap.
...
| **GDPR interplay** | ✅ | Network consent gate before any relay connection (`core.js#_assertNetworkConsent`), privacy policy in `profile.json`, data-minimised directory (`docs/PUBLIC_TREE_INDEX.md`) |
