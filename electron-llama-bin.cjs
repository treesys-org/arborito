...
/** Optional GPU builds (Vulkan). macOS uses Metal in the default macOS archive. */
const LLAMA_ASSETS_VULKAN = {
  'linux-x64': { name: `llama-${LLAMA_RELEASE}-bin-ubuntu-vulkan-x64.tar.gz`, extract: 'tar' },
  'linux-arm64': { name: `llama-${LLAMA_RELEASE}-bin-ubuntu-vulkan-arm64.tar.gz`, extract: 'tar' },
  'win32-x64': { name: `llama-${LLAMA_RELEASE}-bin-win-vulkan-x64.zip`, extract: 'zip' },
};
...
function resolveUserDataDir(appRef) {
  ...
  return resolveStableUserDataDir();
}

function llamaBinRoot(userDataDir, vulkan = false) {
  return path.join(userDataDir, vulkan ? 'llama-cpp-bin-vulkan' : 'llama-cpp-bin');
}

function isLlamaVulkanSupportedPlatform(platformKey = detectPlatformKey()) {
  return !!(platformKey && LLAMA_ASSETS_VULKAN[platformKey]);
}
...
/** Delete downloaded GGUF weights (and partial downloads). Stops nothing by itself. */
function clearChatModels(userDataDir) {
  ...
  try {
    ...
    for (const name of fs.readdirSync(dir)) {
      if (!/\.gguf(\.part)?$/i.test(name)) continue;
    ...
    return { ok: false, error: String(e && e.message ? e.message : e), removed, bytes };
  }
...
async function downloadFile(url, destPath, onProgress, signal) {
  ...
  const writeChunk = (chunk) => new Promise((resolve, reject) => {
    ...
    out.write(chunk, (err) => {
      ...
      /* Abort closes the stream; ignore write-after-destroy noise. */
      if (signal?.aborted || out.destroyed || err.code === 'ERR_STREAM_DESTROYED') {
        reject(makeAbortError());
        return;
      }
      reject(err);
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      out.once('error', (err) => {
        if (signal?.aborted || err?.code === 'ERR_STREAM_DESTROYED') reject(makeAbortError());
        else reject(err);
      });
    });
...
async function ensureLlamaServerBinary(userDataDir, onProgress, signal, opts = {}) {
  const platform = detectPlatformKey();
  const useVulkan = !!(opts.vulkan && isLlamaVulkanSupportedPlatform(platform));
  const asset = useVulkan ? LLAMA_ASSETS_VULKAN[platform] : LLAMA_ASSETS[platform];
  if (!asset) throw new Error('Unsupported platform for llama.cpp binaries');
  return ensureBinaryTool({
    asset,
    urlBase: `https://github.com/ggml-org/llama.cpp/releases/download/${LLAMA_RELEASE}`,
    destDir: llamaBinRoot(userDataDir, useVulkan),
...
module.exports = {
  ...
  resolveUserDataDir,
  llamaBinRoot,
  isLlamaVulkanSupportedPlatform,
  chatModelsDir,
  clearChatModels,
